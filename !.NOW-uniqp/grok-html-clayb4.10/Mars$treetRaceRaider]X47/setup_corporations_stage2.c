...
int read_governments(Government govs[]) {
    FILE *fp = fopen("governments/gov-list.txt", "r");
    if (fp == NULL) {
        perror("Error opening governments/gov-list.txt");
        return 0;
    }
...
int read_banks(Corporation banks[]) {
    ...
    if (fp == NULL) {
        perror("Error opening corporations/martian_corporations.txt");
        return 0;
    }
    ...
    if (d) {
        while ((dir = readdir(d)) != NULL) {
            if (strstr(dir->d_name, ".txt")) {
                update_corporation_file(dir->d_name, govs, num_govs, banks, num_banks);
            }
        }
        closedir(d);
    }

    printf("Corporation files updated.\n");
