...
void display_search_results() {
    ...
    if (d) {
        while ((dir = readdir(d)) != NULL) {
            ...
            char *dot = strrchr(ticker_name, '.');
            if (dot) {
                *dot = '\0'; // Null-terminate at the dot to remove the extension
            }

            snprintf(filepath, sizeof(filepath), "corporations/generated/%s", dir->d_name);
