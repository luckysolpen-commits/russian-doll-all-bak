#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <curl/curl.h>
#include <cjson/cJSON.h>
#include <getopt.h>
#include <unistd.h> // For isatty

// Global flag for auto-confirmation
int auto_confirm = 0;

// Helper function to read a file into a string buffer
char* read_file_to_string(const char* filename) {
    FILE *file = fopen(filename, "rb");
    if (file == NULL) {
        return NULL;
    }

    fseek(file, 0, SEEK_END);
    long length = ftell(file);
    fseek(file, 0, SEEK_SET);

    char *buffer = (char*)malloc(length + 1);
    if (buffer == NULL) {
        fclose(file);
        return NULL;
    }

    fread(buffer, 1, length, file);
    buffer[length] = '\0';
    fclose(file);

    return buffer;
}

// Helper function to write a string buffer to a file
int write_file_from_string(const char* filename, const char* content) {
    FILE *file = fopen(filename, "wb");
    if (file == NULL) {
        fprintf(stderr, "Error: Could not open file '%s' for writing.\n", filename);
        return 0; // Failure
    }

    if (fputs(content, file) == EOF) {
        fprintf(stderr, "Error: Could not write to file '%s'.\n", filename);
        fclose(file);
        return 0; // Failure
    }

    fclose(file);
    return 1; // Success
}


// Struct to hold the stream buffer, full response, and tool calls
struct StreamBuffer {
    char *data; // For line buffering
    size_t size; // For line buffering
    char *full_response; // To accumulate full model response
    size_t full_response_size; // Size of full_response
    cJSON *tool_calls_array; // To accumulate tool calls
};

// Processes a single line from the SSE stream
void process_stream_line(char* line, struct StreamBuffer *mem) {
    if (strncmp(line, "data: ", 6) == 0) {
        char* json_str = line + 6;
        if (strcmp(json_str, "[DONE]") == 0) {
            return;
        }

        cJSON *json = cJSON_Parse(json_str);
        if (json != NULL) {
            const cJSON *choices = cJSON_GetObjectItemCaseSensitive(json, "choices");
            if (cJSON_IsArray(choices)) {
                cJSON *choice = cJSON_GetArrayItem(choices, 0);
                if (cJSON_IsObject(choice)) {
                    const cJSON *delta = cJSON_GetObjectItemCaseSensitive(choice, "delta");
                    if (cJSON_IsObject(delta)) {
                        const cJSON *content = cJSON_GetObjectItemCaseSensitive(delta, "content");
                        const cJSON *tool_calls = cJSON_GetObjectItemCaseSensitive(delta, "tool_calls");

                        if (cJSON_IsString(content) && content->valuestring != NULL) {
                            printf("%s", content->valuestring);
                            fflush(stdout);

                            // Accumulate full response
                            size_t content_len = strlen(content->valuestring);
                            // Remove trailing newline if present
                            if (content_len > 0 && content->valuestring[content_len - 1] == '\n') {
                                content_len--;
                            }
                            char *new_full_response = realloc(mem->full_response, mem->full_response_size + content_len + 1);
                            if (new_full_response == NULL) {
                                fprintf(stderr, "Failed to reallocate memory for full response.\n");
                            } else {
                                mem->full_response = new_full_response;
                                memcpy(&(mem->full_response[mem->full_response_size]), content->valuestring, content_len);
                                mem->full_response_size += content_len;
                                mem->full_response[mem->full_response_size] = 0;
                            }
                        } else if (cJSON_IsArray(tool_calls)) {
                            if (mem->tool_calls_array == NULL) {
                                mem->tool_calls_array = cJSON_CreateArray();
                            }
                            cJSON *tool_call_item;
                            cJSON_ArrayForEach(tool_call_item, tool_calls) {
                                cJSON_AddItemToArray(mem->tool_calls_array, cJSON_Duplicate(tool_call_item, 1));
                            }
                        }
                    }
                }
            }
            cJSON_Delete(json);
        }
    }
}


// Callback function to handle streaming data
static size_t
StreamCallback(void *contents, size_t size, size_t nmemb, void *userp) {
    size_t realsize = size * nmemb;
    struct StreamBuffer *mem = (struct StreamBuffer *)userp;

    char *ptr = realloc(mem->data, mem->size + realsize + 1);
    if(ptr == NULL) {
        fprintf(stderr, "not enough memory (realloc returned NULL) for stream buffer\n");
        return 0;
    }

    mem->data = ptr;
    memcpy(&(mem->data[mem->size]), contents, realsize);
    mem->size += realsize;
    mem->data[mem->size] = 0;

    char *line_end;
    while ((line_end = strchr(mem->data, '\n')) != NULL) {
        *line_end = '\0';
        if (strlen(mem->data) > 0 && mem->data[strlen(mem->data)-1] == '\r') {
            mem->data[strlen(mem->data)-1] = '\0';
        }
        process_stream_line(mem->data, mem);
        
        size_t line_len = strlen(mem->data) + 1;
        memmove(mem->data, mem->data + line_len, mem->size - line_len);
        mem->size -= line_len;
        mem->data[mem->size] = 0;
    }

    return realsize;
}

// Helper function for user confirmation
int ask_for_confirmation(const char* question) {
    if (auto_confirm) {
        return 1; // Auto-confirm if flag is set
    }

    if (!isatty(STDIN_FILENO)) {
        // If not interactive and not auto-confirm, default to 'no'
        fprintf(stderr, "Non-interactive mode: defaulting to 'no' for confirmation '%s'\n", question);
        return 0;
    }

    fprintf(stderr, "%s [y/N]: ", question);
    char response[10];
    if (fgets(response, sizeof(response), stdin) != NULL) {
        if (response[0] == 'y' || response[0] == 'Y') {
            return 1;
        }
    }
    return 0;
}


void print_usage(const char* prog_name) {
    fprintf(stderr, "Usage: %s [options] <prompt>\n", prog_name);
    fprintf(stderr, "Options:\n");
    fprintf(stderr, "  --prompt <prompt>    The prompt to send to the model.\n");
    fprintf(stderr, "  --model <model_name> Override the model from config.\n");
    fprintf(stderr, "  --api-url <url>      Override the API URL from config.\n");
    fprintf(stderr, "  -y, --yes            Assume 'yes' to all confirmation prompts.\n");
    fprintf(stderr, "  --help               Show this help message.\n");
}

// Helper function to expand file references in a prompt
char* expand_file_references(const char* original_prompt) {
    char *expanded_prompt = strdup(""); // Start with an empty string
    if (expanded_prompt == NULL) {
        fprintf(stderr, "Error: Failed to allocate memory for expanded prompt.\n");
        return NULL;
    }

    const char *current_pos = original_prompt;
    const char *at_sign_pos;

    while ((at_sign_pos = strstr(current_pos, "@")) != NULL) {
        // Append text before the '@'
        size_t pre_at_len = at_sign_pos - current_pos;
        char *temp = realloc(expanded_prompt, strlen(expanded_prompt) + pre_at_len + 1);
        if (temp == NULL) {
            fprintf(stderr, "Error: Failed to reallocate memory for expanded prompt.\n");
            free(expanded_prompt);
            return NULL;
        }
        expanded_prompt = temp;
        strncat(expanded_prompt, current_pos, pre_at_len);
        expanded_prompt[strlen(expanded_prompt)] = '\0'; // Ensure null termination

        // Find end of file path (space, newline, or end of string)
        const char *path_start = at_sign_pos + 1;
        const char *path_end = path_start;
        while (*path_end != ' ' && *path_end != '\n' && *path_end != '\0') {
            path_end++;
        }

        if (path_end == path_start) { // Just an '@' without a path
            strcat(expanded_prompt, "@");
            current_pos = path_start;
            continue;
        }

        size_t path_len = path_end - path_start;
        char *file_path = (char*)malloc(path_len + 1);
        if (file_path == NULL) {
            fprintf(stderr, "Error: Failed to allocate memory for file path.\n");
            free(expanded_prompt);
            return NULL;
        }
        strncpy(file_path, path_start, path_len);
        file_path[path_len] = '\0';

        // Ask for confirmation before reading file
        char question[512];
        snprintf(question, sizeof(question), "Read content from file '%s'?", file_path);
        if (!ask_for_confirmation(question)) {
            fprintf(stderr, "Skipping file '%s' due to user denial.\n", file_path);
            char *temp_append = realloc(expanded_prompt, strlen(expanded_prompt) + 1 + strlen(file_path) + 1); // +1 for '@', +1 for null terminator
            if (temp_append == NULL) {
                fprintf(stderr, "Error: Failed to reallocate memory for expanded prompt.\n");
                free(expanded_prompt);
                free(file_path);
                return NULL;
            }
            expanded_prompt = temp_append;
            strcat(expanded_prompt, "@");
            strcat(expanded_prompt, file_path);
            free(file_path);
            current_pos = path_end;
            continue; // Skip to next iteration
        }

        // Construct command to run gemini-c-readfile
        char command[512]; // Increased buffer size for command
        snprintf(command, sizeof(command), "./gemini-c-readfile \"%s\"", file_path);
        free(file_path);

        FILE *fp = popen(command, "r");
        if (fp == NULL) {
            fprintf(stderr, "Error: Failed to execute command '%s'\n", command);
            free(expanded_prompt);
            return NULL;
        }

        char read_buffer[1024];
        size_t bytes_read;
        while ((bytes_read = fread(read_buffer, 1, sizeof(read_buffer), fp)) > 0) {
            char *temp_content = realloc(expanded_prompt, strlen(expanded_prompt) + bytes_read + 1);
            if (temp_content == NULL) {
                fprintf(stderr, "Error: Failed to reallocate memory for expanded prompt content.\n");
                pclose(fp);
                free(expanded_prompt);
                return NULL;
            }
            expanded_prompt = temp_content;
            strncat(expanded_prompt, read_buffer, bytes_read);
            expanded_prompt[strlen(expanded_prompt)] = '\0'; // Ensure null termination
        }

        int pclose_status = pclose(fp);
        if (pclose_status != 0) {
            fprintf(stderr, "Error: gemini-c-readfile exited with status %d for path starting at '%s'.\n", pclose_status, path_start);
            // Decide whether to return NULL or continue with partial expansion
            // For now, let's return NULL to indicate a failure in expansion
            free(expanded_prompt);
            return NULL;
        }

        current_pos = path_end;
    }

    // Append any remaining text after the last '@'
    if (*current_pos != '\0') {
        char *temp = realloc(expanded_prompt, strlen(expanded_prompt) + strlen(current_pos) + 1);
        if (temp == NULL) {
            fprintf(stderr, "Error: Failed to reallocate memory for expanded prompt.\n");
            free(expanded_prompt);
            return NULL;
        }
        expanded_prompt = temp;
        strcat(expanded_prompt, current_pos);
    }

    return expanded_prompt;
}

// chat_with_model function definition
int chat_with_model(cJSON *history_array, const char *prompt_content, const char *model, const char *api_url) {
    CURL *curl;
    CURLcode res;
    struct StreamBuffer chunk = { .data = malloc(1), .size = 0, .full_response = malloc(1), .full_response_size = 0, .tool_calls_array = NULL };
    if (chunk.data == NULL || chunk.full_response == NULL) {
        fprintf(stderr, "Error: Failed to allocate initial memory for stream buffer or full response.\n");
        return 1;
    }
    chunk.data[0] = 0;
    chunk.full_response[0] = 0;

    // Add system message to instruct on tool usage (only once at the beginning of the conversation)
    cJSON *system_message = cJSON_CreateObject();
    cJSON_AddStringToObject(system_message, "role", "system");
    
    cJSON *tools_json_def = cJSON_CreateArray();
    cJSON *read_file_tool = cJSON_CreateObject();
    cJSON_AddStringToObject(read_file_tool, "type", "function");
    cJSON *read_file_function = cJSON_CreateObject();
    cJSON_AddStringToObject(read_file_function, "name", "read_file");
    cJSON_AddStringToObject(read_file_function, "description", "Reads the content of a specified file.");
    cJSON *read_file_parameters = cJSON_CreateObject();
    cJSON_AddStringToObject(read_file_parameters, "type", "object");
    cJSON *read_file_properties = cJSON_CreateObject();
    cJSON *path_property = cJSON_CreateObject();
    cJSON_AddStringToObject(path_property, "type", "string");
    cJSON_AddStringToObject(path_property, "description", "The path to the file to read.");
    cJSON_AddItemToObject(read_file_properties, "path", path_property);
    cJSON_AddItemToObject(read_file_parameters, "properties", read_file_properties);
    cJSON *read_file_required = cJSON_CreateArray();
    cJSON_AddItemToArray(read_file_required, cJSON_CreateString("path"));
    cJSON_AddItemToObject(read_file_parameters, "required", read_file_required);
    cJSON_AddItemToObject(read_file_function, "parameters", read_file_parameters);
    cJSON_AddItemToObject(read_file_tool, "function", read_file_function);
    cJSON_AddItemToArray(tools_json_def, read_file_tool);

    // Define write_file tool
    cJSON *write_file_tool = cJSON_CreateObject();
    cJSON_AddStringToObject(write_file_tool, "type", "function");
    cJSON *write_file_function = cJSON_CreateObject();
    cJSON_AddStringToObject(write_file_function, "name", "write_file");
    cJSON_AddStringToObject(write_file_function, "description", "Writes content to a specified file.");
    cJSON *write_file_parameters = cJSON_CreateObject();
    cJSON_AddStringToObject(write_file_parameters, "type", "object");
    cJSON *write_file_properties = cJSON_CreateObject();
    cJSON *write_path_property = cJSON_CreateObject();
    cJSON_AddStringToObject(write_path_property, "type", "string");
    cJSON_AddStringToObject(write_path_property, "description", "The path to the file to write.");
    cJSON_AddItemToObject(write_file_properties, "path", write_path_property);
    cJSON *content_property = cJSON_CreateObject();
    cJSON_AddStringToObject(content_property, "type", "string");
    cJSON_AddStringToObject(content_property, "description", "The content to write to the file.");
    cJSON_AddItemToObject(write_file_properties, "content", content_property);
    cJSON_AddItemToObject(write_file_parameters, "properties", write_file_properties);
    cJSON *write_file_required = cJSON_CreateArray();
    cJSON_AddItemToArray(write_file_required, cJSON_CreateString("path"));
    cJSON_AddItemToArray(write_file_required, cJSON_CreateString("content"));
    cJSON_AddItemToObject(write_file_parameters, "required", write_file_required);
    cJSON_AddItemToObject(write_file_function, "parameters", write_file_parameters);
    cJSON_AddItemToObject(write_file_tool, "function", write_file_function);
    cJSON_AddItemToArray(tools_json_def, write_file_tool);

    char *tools_json_str = cJSON_PrintUnformatted(tools_json_def);
    cJSON_Delete(tools_json_def); // Free the tools definition object

    char *system_prompt_content = NULL;
    if (tools_json_str != NULL) {
        const char *few_shot_examples = 

When asked to perform an action that requires a tool, respond ONLY with a JSON object in the following format:
{"tool_calls": [{"function": {"name": "tool_name", "arguments": {"arg1": "value1", "arg2": "value2"}}}]}
Do NOT include any other text or explanation when using a tool.

Example 1:
User: Read the content of my_file.txt
Assistant: {"tool_calls": [{"function": {"name": "read_file", "arguments": {"path": "my_file.txt"}}}]}

Example 2:
User: Save "Hello there!" to greeting.txt
Assistant: {"tool_calls": [{"function": {"name": "write_file", "arguments": {"path": "greeting.txt", "content": "Hello there!"}}}]}

If no tool is needed, respond with a normal text message.;

        size_t sys_prompt_len = strlen("You are a helpful AI assistant. You have access to the following tools:\n\n") + strlen(tools_json_str) + strlen(few_shot_examples) + 1;
        system_prompt_content = (char*)malloc(sys_prompt_len);
        if (system_prompt_content != NULL) {
            snprintf(system_prompt_content, sys_prompt_len,
                     "You are a helpful AI assistant. You have access to the following tools:\n\n%s%s",
                     tools_json_str, few_shot_examples);
        }
        free(tools_json_str);
    }

    if (system_prompt_content != NULL) {
        cJSON_AddStringToObject(system_message, "content", system_prompt_content);
        free(system_prompt_content);
    } else {
        cJSON_AddStringToObject(system_message, "content", "You are a helpful AI assistant.");
    }
    
    // Prepend system message to messages_for_request if not already present
    // This check is simplified; a more robust solution might check for role "system"
    // at the beginning of the history_array.
    if (cJSON_GetArraySize(history_array) == 0 || strcmp(cJSON_GetObjectItem(cJSON_GetArrayItem(history_array, 0), "role")->valuestring, "system") != 0) {
        cJSON_InsertItemInArray(history_array, 0, system_message);
    } else {
        cJSON_Delete(system_message); // If system message already exists, free the newly created one
    }

    int loop_count = 0; // To prevent infinite loops in case of model errors
    int max_loops = 5; // Max tool-use turns

    do {
        loop_count++;
        if (loop_count > max_loops) {
            fprintf(stderr, "Warning: Exceeded max tool-use loops. Terminating conversation.\n");
            break;
        }

        // Reset chunk for new response
        free(chunk.data);
        free(chunk.full_response);
        if (chunk.tool_calls_array) cJSON_Delete(chunk.tool_calls_array);
        chunk.data = malloc(1);
        chunk.size = 0;
        chunk.full_response = malloc(1);
        chunk.full_response_size = 0;
        chunk.tool_calls_array = NULL;
        if (chunk.data == NULL || chunk.full_response == NULL) {
            fprintf(stderr, "Error: Failed to reallocate memory for stream buffer or full response in loop.\n");
            return 1;
        }
        chunk.data[0] = 0;
        chunk.full_response[0] = 0;

        curl = curl_easy_init();
        if(!curl) {
            fprintf(stderr, "Error: Failed to initialize curl in loop.\n");
            return 1;
        }

        cJSON *root = cJSON_CreateObject();
        cJSON_AddStringToObject(root, "model", model);
        cJSON_AddBoolToObject(root, "stream", 1);
        
        // Add cloned history to the request
        cJSON *messages_for_request = cJSON_Duplicate(history_array, 1); // Deep copy
        cJSON_AddItemToObject(root, "messages", messages_for_request);
        cJSON_AddStringToObject(root, "tool_choice", "auto"); // Allow model to choose tools

        char *json_payload = cJSON_PrintUnformatted(root);
        cJSON_Delete(root); // Delete root and its children (including messages_for_request)

        if (json_payload != NULL) {
            struct curl_slist *headers = NULL;
            headers = curl_slist_append(headers, "Content-Type: application/json");
            curl_easy_setopt(curl, CURLOPT_URL, api_url);
            curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);
            curl_easy_setopt(curl, CURLOPT_POSTFIELDS, json_payload);
            curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, StreamCallback);
            curl_easy_setopt(curl, CURLOPT_WRITEDATA, (void *)&chunk);

            res = curl_easy_perform(curl);
            free(json_payload);
            curl_slist_free_all(headers);

            if(res != CURLE_OK) {
                fprintf(stderr, "curl_easy_perform() failed: %s\n", curl_easy_strerror(res));
                curl_easy_cleanup(curl);
                return 1;
            } else {
                printf("\n"); // Newline after streamed response
            }
        }
        curl_easy_cleanup(curl);

        // Process tool calls or final response
        if (chunk.tool_calls_array != NULL && cJSON_GetArraySize(chunk.tool_calls_array) > 0) {
            // Model wants to call tools
            cJSON *tool_message = cJSON_CreateObject();
            cJSON_AddStringToObject(tool_message, "role", "assistant");
            cJSON_AddItemToObject(tool_message, "tool_calls", cJSON_Duplicate(chunk.tool_calls_array, 1));
            cJSON_AddItemToArray(history_array, tool_message);

            cJSON *tool_call_item;
            cJSON_ArrayForEach(tool_call_item, chunk.tool_calls_array) {
                const cJSON *function = cJSON_GetObjectItemCaseSensitive(tool_call_item, "function");
                if (cJSON_IsObject(function)) {
                    const cJSON *name = cJSON_GetObjectItemCaseSensitive(function, "name");
                    const cJSON *arguments = cJSON_GetObjectItemCaseSensitive(function, "arguments");

                    if (cJSON_IsString(name) && name->valuestring != NULL && cJSON_IsObject(arguments)) {
                        char *tool_name = name->valuestring;
                        char *tool_output_content = NULL;
                        int tool_success = 0;

                        if (strcmp(tool_name, "read_file") == 0) {
                            const cJSON *path_json = cJSON_GetObjectItemCaseSensitive(arguments, "path");
                            if (cJSON_IsString(path_json) && path_json->valuestring != NULL) {
                                char *path = path_json->valuestring;
                                char question[512];
                                snprintf(question, sizeof(question), "Model wants to read file '%s'. Confirm?", path);
                                if (ask_for_confirmation(question)) {
                                    tool_output_content = read_file_to_string(path);
                                    if (tool_output_content != NULL) {
                                        tool_success = 1;
                                    } else {
                                        tool_output_content = strdup("Error: Could not read file.");
                                    }
                                } else {
                                    tool_output_content = strdup("User denied file read.");
                                }
                            } else {
                                tool_output_content = strdup("Error: Missing 'path' argument for read_file.");
                            }
                        } else if (strcmp(tool_name, "write_file") == 0) {
                            const cJSON *path_json = cJSON_GetObjectItemCaseSensitive(arguments, "path");
                            const cJSON *content_json = cJSON_GetObjectItemCaseSensitive(arguments, "content");
                            if (cJSON_IsString(path_json) && path_json->valuestring != NULL &&
                                cJSON_IsString(content_json) && content_json->valuestring != NULL) {
                                char *path = path_json->valuestring;
                                char *content = content_json->valuestring;
                                char question[512];
                                snprintf(question, sizeof(question), "Model wants to write to file '%s'. Confirm?", path);
                                if (ask_for_confirmation(question)) {
                                    if (write_file_from_string(path, content)) {
                                        tool_output_content = strdup("File written successfully.");
                                        tool_success = 1;
                                    } else {
                                        tool_output_content = strdup("Error: Could not write file.");
                                    }
                                } else {
                                    tool_output_content = strdup("User denied file write.");
                                }
                            } else {
                                tool_output_content = strdup("Error: Missing 'path' or 'content' argument for write_file.");
                            }
                        } else {
                            tool_output_content = strdup("Error: Unknown tool.");
                        }

                        // Add tool_output message to history
                        cJSON *tool_output_message = cJSON_CreateObject();
                        cJSON_AddStringToObject(tool_output_message, "role", "tool");
                        cJSON_AddStringToObject(tool_output_message, "tool_call_id", cJSON_GetObjectItemCaseSensitive(tool_call_item, "id")->valuestring);
                        cJSON_AddStringToObject(tool_output_message, "content", tool_output_content);
                        cJSON_AddItemToArray(history_array, tool_output_message);
                        free(tool_output_content); // Free the dynamically allocated tool output
                    }
                }
            }
        } else if (chunk.full_response_size > 0) {
            // Model provided a final text response
            cJSON *assistant_message = cJSON_CreateObject();
            cJSON_AddStringToObject(assistant_message, "role", "assistant");
            cJSON_AddStringToObject(assistant_message, "content", chunk.full_response);
            cJSON_AddItemToArray(history_array, assistant_message);

            // Save updated history to file
            char *history_json_str = cJSON_PrintUnformatted(history_array);
            if (history_json_str != NULL) {
                FILE *fp = fopen("history.json", "wb");
                if (fp != NULL) {
                    fputs(history_json_str, fp);
                    fclose(fp);
                } else {
                    fprintf(stderr, "Warning: Failed to save history.json\n");
                }
                free(history_json_str);
            }
            break; // Exit loop as conversation is complete
        } else {
            // No tool calls and no content, possibly an empty response or error
            fprintf(stderr, "Warning: Received empty response from model.\n");
            break;
        }

    } while (1); // Loop indefinitely until a break condition is met

    free(chunk.data);
    free(chunk.full_response);
    if (chunk.tool_calls_array) cJSON_Delete(chunk.tool_calls_array);

    return 0;
}


int main(int argc, char *argv[]) {
    curl_global_init(CURL_GLOBAL_ALL); // Initialize curl globally once

    // --- Argument Parsing ---
    char *prompt_arg = NULL;
    char *model_arg = NULL;
    char *api_url_arg = NULL;

    struct option long_options[] = {
        {"prompt",  required_argument, 0, 'p'},
        {"model",   required_argument, 0, 'm'},
        {"api-url", required_argument, 0, 'u'},
        {"yes",     no_argument,       0, 'y'}, // New option
        {"help",    no_argument,       0, 'h'},
        {0, 0, 0, 0}
    };

    int opt;
    int option_index = 0;
    while ((opt = getopt_long(argc, argv, "p:m:u:yh", long_options, &option_index)) != -1) {
        switch (opt) {
            case 'p': prompt_arg = strdup(optarg); break;
            case 'm': model_arg = strdup(optarg); break;
            case 'u': api_url_arg = strdup(optarg); break;
            case 'y': auto_confirm = 1; break; // Set global flag
            case 'h': print_usage(argv[0]); return 0;
            default: print_usage(argv[0]); return 1;
        }
    }

    if (optind < argc) {
        if (prompt_arg != NULL) {
            fprintf(stderr, "Error: Prompt provided with --prompt and as a bare argument. Please use only one.\n");
            print_usage(argv[0]);
            free(prompt_arg);
            if (model_arg) free(model_arg);
            if (api_url_arg) free(api_url_arg);
            return 1;
        }
        prompt_arg = strdup(argv[optind]);
    }

    if (prompt_arg == NULL) {
        // Check if stdin is a terminal or a pipe/file
        if (isatty(STDIN_FILENO)) {
            // Interactive mode
            fprintf(stderr, "Enter your prompt (Ctrl+D to finish):\n");
            char *line = NULL;
            size_t len = 0;
            ssize_t read;
            char *multiline_prompt = NULL;
            size_t total_len = 0;

            while ((read = getline(&line, &len, stdin)) != -1) {
                // Remove trailing newline from each line
                if (read > 0 && line[read - 1] == '\n') {
                    line[read - 1] = '\0';
                    read--; // Adjust read length
                }

                // Add a space between lines if not the very first line and something was read
                if (total_len > 0 && read > 0) {
                    char *space_prompt = realloc(multiline_prompt, total_len + 1 + read + 1); // +1 for space, +read for new line, +1 for null terminator
                    if (space_prompt == NULL) {
                        fprintf(stderr, "Error: Failed to allocate memory for multiline prompt.\n");
                        free(line);
                        free(multiline_prompt);
                        if (model_arg) free(model_arg);
                        if (api_url_arg) free(api_url_arg);
                        return 1;
                    }
                    multiline_prompt = space_prompt;
                    multiline_prompt[total_len] = ' ';
                    total_len++;
                    multiline_prompt[total_len] = '\0';
                }

                char *new_multiline_prompt = realloc(multiline_prompt, total_len + read + 1);
                if (new_multiline_prompt == NULL) {
                    fprintf(stderr, "Error: Failed to allocate memory for multiline prompt.\n");
                    free(line);
                    free(multiline_prompt);
                    if (model_arg) free(model_arg);
                    if (api_url_arg) free(api_url_arg);
                    return 1;
                }
                multiline_prompt = new_multiline_prompt;
                memcpy(multiline_prompt + total_len, line, read);
                total_len += read;
                multiline_prompt[total_len] = '\0';
            }
            free(line); // Free the buffer used by getline

            if (multiline_prompt == NULL || total_len == 0) {
                fprintf(stderr, "Error: No prompt entered.\n");
                if (model_arg) free(model_arg);
                if (api_url_arg) free(api_url_arg);
                return 1;
            }
            prompt_arg = multiline_prompt;
        } else {
            // Piped input mode
            char *piped_input = NULL;
            size_t piped_len = 0;
            char buffer[4096];
            size_t bytes_read;

            while ((bytes_read = fread(buffer, 1, sizeof(buffer), stdin)) > 0) {
                char *new_piped_input = realloc(piped_input, piped_len + bytes_read + 1);
                if (new_piped_input == NULL) {
                    fprintf(stderr, "Error: Failed to allocate memory for piped input.\n");
                    free(piped_input);
                    if (model_arg) free(model_arg);
                    if (api_url_arg) free(api_url_arg);
                    return 1;
                }
                piped_input = new_piped_input;
                memcpy(piped_input + piped_len, buffer, bytes_read);
                piped_len += bytes_read;
                piped_input[piped_len] = '\0';
            }

            if (piped_input == NULL || piped_len == 0) {
                fprintf(stderr, "Error: No piped input received.\n");
                if (model_arg) free(model_arg);
                if (api_url_arg) free(api_url_arg);
                return 1;
            }
            prompt_arg = piped_input;
        }
    }

    // --- Configuration Loading & Precedence ---
    const char *default_model = "gemma3:270M";
    const char *default_api_url = "http://localhost:11434/v1/chat/completions";
    char *model = NULL;
    char *api_url = NULL;

    if (model_arg) model = model_arg;
    if (api_url_arg) api_url = api_url_arg;

    if (model == NULL || api_url == NULL) {
        char* config_buffer = read_file_to_string("config.json");
        if (config_buffer != NULL) {
            cJSON *config_json = cJSON_Parse(config_buffer);
            free(config_buffer);
            if (config_json != NULL) {
                if (model == NULL) {
                    const cJSON *model_json = cJSON_GetObjectItemCaseSensitive(config_json, "model");
                    if (cJSON_IsString(model_json) && model_json->valuestring != NULL) {
                        model = strdup(model_json->valuestring);
                    }
                }
                if (api_url == NULL) {
                    const cJSON *api_url_json = cJSON_GetObjectItemCaseSensitive(config_json, "api_url");
                    if (cJSON_IsString(api_url_json) && api_url_json->valuestring != NULL) {
                        api_url = strdup(api_url_json->valuestring);
                    }
                }
                cJSON_Delete(config_json);
            }
        }
    }

    if (model == NULL) model = (char*)default_model;
    if (api_url == NULL) api_url = (char*)default_api_url;

    // --- File Reference Expansion ---
    char *original_prompt_to_free = prompt_arg; // Keep track of original prompt_arg for freeing
    char *expanded_prompt = expand_file_references(prompt_arg);
    if (expanded_prompt == NULL) {
        fprintf(stderr, "Error: Failed to expand file references in prompt.\n");
        // Cleanup resources before returning
        free(original_prompt_to_free); // Free the original prompt_arg
        if (model != default_model) free(model);
        if (api_url != default_api_url) free(api_url);
        return 1;
    }
    prompt_arg = expanded_prompt; // Use the expanded prompt

    // --- Conversation History Loading ---
    cJSON *history_array = NULL;
    char* history_buffer = read_file_to_string("history.json");
    if (history_buffer != NULL) {
        history_array = cJSON_Parse(history_buffer);
        free(history_buffer);
        if (history_array == NULL || !cJSON_IsArray(history_array)) {
            fprintf(stderr, "Warning: history.json is invalid or not an array. Starting new history.\n");
            if (history_array) cJSON_Delete(history_array);
            history_array = cJSON_CreateArray();
        }
    } else {
        history_array = cJSON_CreateArray();
    }
    if (history_array == NULL) {
        fprintf(stderr, "Error: Failed to initialize conversation history.\n");
        free(prompt_arg); // Free the expanded prompt
        if (model != default_model) free(model);
        if (api_url != default_api_url) free(api_url);
        return 1;
    }

    // Add current user prompt to history
    cJSON *user_message = cJSON_CreateObject();
    cJSON_AddStringToObject(user_message, "role", "user");
    cJSON_AddStringToObject(user_message, "content", prompt_arg);
    cJSON_AddItemToArray(history_array, user_message);


    // --- Main Logic ---
    int result = chat_with_model(history_array, prompt_arg, model, api_url);

    // --- Cleanup ---
    cJSON_Delete(history_array); // Free the history array and its contents
    free(original_prompt_to_free); // Free the original prompt_arg
    if (model != default_model) free(model);
    if (api_url != default_api_url) free(api_url);

    curl_global_cleanup(); // This should only be called once in main
    return result;
}
