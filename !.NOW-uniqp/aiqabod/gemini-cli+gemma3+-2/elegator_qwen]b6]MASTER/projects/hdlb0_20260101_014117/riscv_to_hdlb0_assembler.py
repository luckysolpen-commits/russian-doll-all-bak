#!/usr/bin/env python3
"""
RISC-V to HDLb0 assembler with proper memory addressing
Converts RISC-V assembly programs to HDLb0 binary format
Addresses are automatically shifted to start from 16 and above for HDLb0 emulator compatibility
Addresses 0-15 are reserved for special purposes in the HDLb0 emulator
"""
...
def parse_instruction(instr):
    ...
    # Define simple opcodes for our processor
    opcodes = {
        'add': 0b000,
        'sub': 0b001,
        'and': 0b010,
    ...
    if opcode_str in ['add', 'sub', 'and', 'or', 'xor', 'slt', 'sltu']:
        ...
        # Convert to addi rd, x0, adjusted_immediate
        # [15:13] opcode [12:10] rs1 [9:4] immediate [3:1] funct3 [0] unused
        funct3 = 0  # addi
        imm_part = adjusted_immediate & 0x3F  # Use lower 6 bits of adjusted immediate
        ...
        # Convert to addi rd, x0, adjusted_address (load address)
        # [15:13] opcode [12:10] rs1 [9:4] immediate [3:1] funct3 [0] unused
        funct3 = 0  # addi
...
def assemble_file(input_file, output_file):
    ...
    with open(output_file, 'w') as f:
        f.write("# HDLb0 binary format for RISC-V processor\n")
        f.write("# Generated from: " + input_file + "\n")
        f.write("# Memory addresses automatically shifted to start from 16 and above for HDLb0 emulator compatibility\n")
        f.write("# Addresses 0-15 are reserved for special purposes in the HDLb0 emulator\n")
        for binary in binary_lines:
            f.write(binary + '\n')

def main():
    if len(sys.argv) != 3:
        print("Usage: python3 riscv_to_hdlb0_assembler.py <input_asm_file> <output_bin_file>")
        sys.exit(1)
