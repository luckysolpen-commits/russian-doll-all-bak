...
#include "stb_image.h"
 unsigned char target_r = 0;   // Green screen color
    ...
    int tolerance = 80; // Tolerance for color matching
// Function prototype
...
static int RGB_SIZE; // Initialized in main (now RGBA)
...
static unsigned char *rgb_data = NULL; // Dynamic allocation (now RGBA)
...
static void yuyv_to_rgb(unsigned char *yuyv, unsigned char *rgb, int size) {
    int i, j;
    int max_pixels = (RGB_SIZE / 4); // Now RGBA (4 bytes per pixel)
...
static void yuv420_to_rgb(unsigned char *yuv, unsigned char *rgb, int width, int height) {
    ...
    for (int j = 0; j < height; j++) {
        for (int i = 0; i < width; i++) {
            ...
            int rgb_idx = (j * width + i) * 4; // Now RGBA
            rgb[rgb_idx + 0] = r;
            rgb[rgb_idx + 1] = g;
            rgb[rgb_idx + 2] = b;
...
static int try_format(int width, int height, unsigned int pixelformat) {
    ...
    if (fmt.fmt.pix.width != width || fmt.fmt.pix.height != height) {
        ...
        RGB_SIZE = WIDTH * HEIGHT * 4; // Now RGBA
        // Reallocate rgb_data
...
static void save_png(void) {
    ...
    printf("Saving %s with resolution %dx%d, buffer size %d bytes\n", filename, WIDTH, HEIGHT, RGB_SIZE);
...
void display() {
    ...
    glPixelZoom(1.0, -1.0);
    glDrawPixels(WIDTH, HEIGHT, GL_RGBA, GL_UNSIGNED_BYTE, rgb_data); // Now RGBA
    glPixelZoom(1.0, 1.0);
    glutSwapBuffers();
}
