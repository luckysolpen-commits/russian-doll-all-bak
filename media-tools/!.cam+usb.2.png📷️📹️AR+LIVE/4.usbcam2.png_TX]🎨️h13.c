...
int tolerance = 29; // Tolerance for color matching
...
static unsigned char target_r = 0;   // Green screen target color, default green
static unsigned char target_g = 255;
static unsigned char target_b = 0;
...
void mouse(int button, int state, int x, int y) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        ...
        int img_x = x;
        int img_y = HEIGHT - y; // Flip Y-axis (OpenGL bottom-left origin)

        if (img_x >= 0 && img_x < WIDTH && img_y >= 0 && img_y < HEIGHT) {
            // Read pixel color from rgb_data
            int idx = (img_y * WIDTH + img_x) * 4; // RGBA
            unsigned char pixel[4] = {rgb_data[idx + 0], rgb_data[idx + 1], rgb_data[idx + 2], rgb_data[idx + 3]};
            ...
            printf("  CSS: rgb(%d, %d, %d)\n", pixel[0], pixel[1], pixel[2]);
            if (pixel[3] < 255) {
                printf("  CSS with alpha: rgba(%d, %d, %d, %.2f)\n", 
                       pixel[0], pixel[1], pixel[2], pixel[3] / 255.0f);
            }
            printf("Swatches:\n");
            printf("\033[48;2;%d;%d;%dm    \033[0m Background\n", pixel[0], pixel[1], pixel[2]);
            printf("\033[38;2;%d;%d;%dm████\033[0m Foreground\n", pixel[0], pixel[1], pixel[2]);

            // Update green screen target color
            target_r = pixel[0];
            target_g = pixel[1];
            target_b = pixel[2];
            printf("Updated green screen target to RGB: (%d, %d, %d)\n", target_r, target_g, target_b);
        } else {
            printf("Click outside image bounds (%d, %d)\n", x, y);
        }
    }
}
...
int main(int argc, char **argv) {
    ...
    printf("  g/G: Toggle green screen effect (default green: RGB 0, 255, 0, ±20 tolerance)\n");
    printf("  Left-click: Pick a color for green screen transparency\n");
