...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        // Check if this command contains the CHTML renderer executable
        char* modified_cmd = NULL;
        if (strstr(original_cmd, "chtml_d6") != NULL || strstr(original_cmd, "chtml") != NULL) {
            // This is a renderer command, append the file path to it if we have one
            if (has_file_arg) {
            ...
            // This is not a renderer command, just add the redirect
            const char* redirect = " 2>&1 | tee -a gl_cli_out.txt";
            size_t len = strlen(original_cmd) + strlen(redirect) + 1;
            modified_cmd = malloc(len);
            if (!modified_cmd) {
