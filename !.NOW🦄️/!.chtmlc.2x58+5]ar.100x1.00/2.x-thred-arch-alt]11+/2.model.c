...
// Function declaration for multiple modules
...
// Multi-module support - function to initialize multiple modules
void init_multiple_modules(const char** module_paths, int count) {
    // For the multi-threaded version, we need to maintain the original single-module design
    // This is because the current threading implementation supports only one module at a time
    // In a real multi-module system, we'd need to create separate pipes and threads for each module
    if (count > 0 && module_paths[0] != NULL) {
        // For now, initialize with the first module as the primary module
        // This maintains compatibility with the existing threading model
        printf("Initializing with first module: %s\n", module_paths[0]);
        init_model(module_paths[0]);
    } else {
        // Initialize without a module
        printf("Initializing without any modules\n");
        init_model(NULL);
    }
}
