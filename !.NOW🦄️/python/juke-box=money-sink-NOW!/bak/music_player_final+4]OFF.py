...
from PySide6.QtWidgets import (QApplication, QFrame, QGroupBox,
                              ...
                              QTabWidget, QCheckBox, QDialog, QGridLayout)
import pygame
import os
import sys
import time
...
class AudioVisualizer(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.distortion_amount = 5  # Range 0-10
        self.eq_curve = [0] * 100  # Custom EQ curve for each frequency band
    ...
    def set_eq_curve(self, curve):
        """Set the custom EQ curve for visualization"""
        self.eq_curve = curve[:]  # Copy the curve
        self.update()
    
    def set_distortion_amount(self, amount):
        """Set the distortion amount effect in visualization"""
    ...
    def _draw_bars(self, painter, width, height, data):
        ...
        for i, value in enumerate(data):
            ...
            if self.is_playing:
                ...
                # Apply the EQ curve adjustment for this specific frequency
                curve_adjustment = 0
                if i < len(self.eq_curve):
                    curve_adjustment = self.eq_curve[i] / 5.0  # Scale to reasonable adjustment amount
                
                # Combine both adjustments
                total_adjustment = adjustment + curve_adjustment
                
                adjusted_value = max(0, min(100, value + total_adjustment))
                
                # Apply distortion effect if enabled
                if self.distortion_enabled:
                    # Distortion effect makes peaks more exaggerated
                    adjusted_value = self.apply_distortion_effect(adjusted_value)
            ...
            if self.is_playing:
                ...
                if self.distortion_enabled:
                    ...
                    sat_adjustment = min(255, int(sat_adjustment * (1.0 + self.distortion_amount / 10.0)))
            ...
            if self.distortion_enabled and self.is_playing:
                border_alpha = min(255, 50 + self.distortion_amount * 15)  # Increase alpha based on distortion amount
            painter.setPen(QColor(255, 255, 255, border_alpha))
            painter.drawRect(x, y, bar_width - 1, bar_height)
    
    def apply_distortion_effect(self, value):
        """Apply a distortion effect to the value"""
        # Simple distortion algorithm that exaggerates peaks
        if value > 70:  # Only apply to higher values
            # The higher the distortion_amount, the more exaggerated the effect
            exaggeration = 1.0 + (self.distortion_amount / 10.0)
            return min(100, value * exaggeration)
        return value
    ...
    def _draw_waveform(self, painter, width, height, data):
        ...
        for i, value in enumerate(data):
            ...
            if self.is_playing:
                ...
                adjusted_value = max(-100, min(100, value + total_adjustment))
        ...
        if len(points) > 1:
            ...
            pen_thickness = 2
            if self.distortion_enabled:
                pen_thickness = 2 + self.distortion_amount / 3  # Thickness based on distortion amount
            waveform_color = QColor(0, 200, 255)
            if self.distortion_enabled:
                # Color changes based on distortion amount
                red_level = min(255, 100 + self.distortion_amount * 15)
                green_level = max(50, 200 - self.distortion_amount * 15)
                waveform_color = QColor(red_level, green_level, 100)  # Dynamic color for distortion
            ...
            if self.is_playing:
                if self.distortion_enabled:
                    red_level = min(255, 150 + self.distortion_amount * 10)
                    green_level = max(50, 150 - self.distortion_amount * 10)
                    gradient.setColorAt(0, QColor(red_level, green_level, 100, 120))
                    gradient.setColorAt(1, QColor(red_level//2, green_level//2, 50, 60))
                else:
        ...
        for i, value in enumerate(data):
            ...
            if self.is_playing:
                ...
                if self.distortion_enabled:
                    ...
                    saturation_boost = min(255, saturation + self.distortion_amount * 15)
                    value_brightness = min(255, value_brightness + self.distortion_amount * 5)
                    color = QColor.fromHsv(int(hue), saturation_boost, value_brightness)
            else:
            ...
            if self.is_playing:
                ...
                if self.distortion_enabled:
                    pen_color = QColor(255, 255, 220)  # Bright white-yellow for distortion
                    # Add additional glow based on distortion amount
                    pen = QPen(pen_color, 1 + self.distortion_amount / 5)
                    painter.setPen(pen)
                    painter.drawLine(x, y, x + bar_width - 1, y)  # Top line
                    painter.setPen(QColor(255, 255, 255, 100))  # Add outline
                    painter.drawRect(x, y, bar_width - 1, bar_height)
                    painter.setPen(pen_color)  # Reset pen to original
                else:
    ...
    def _draw_particles(self, painter, width, height, data):
        ...
        for i, value in enumerate(data):
            if value > 5:  # Only draw particles for significant values
                ...
                if self.distortion_enabled:
                    size *= 1.0 + (self.distortion_amount / 10.0)  # Size based on distortion amount
                
                # Color based on frequency band
                hue = (i * 3.6) % 360
                if self.is_playing:
                    if self.distortion_enabled:
                        saturation = min(255, 255)
                        brightness = min(255, int(200 + adjusted_value))
                        saturation += int(self.distortion_amount * 10)  # Boost saturation with distortion
                        color = QColor.fromHsv(int(hue), saturation, brightness)
                    else:
                ...
                # Add alpha transparency for particle effect
                ...
                if self.is_playing and i > 0 and data[i-1] > 5:
                    ...
                    # Apply curve adjustment to previous point too
                    prev_curve_adjustment = 0
                    if i-1 < len(self.eq_curve):
                        prev_curve_adjustment = self.eq_curve[i-1] / 5.0
                    
                    prev_total_adjustment = prev_adjustment + prev_curve_adjustment
                    
                    prev_adjusted_value = max(0, min(100, data[i-1] + prev_total_adjustment))
                    
                    if self.distortion_enabled:
                        prev_adjusted_value = self.apply_distortion_effect(prev_adjusted_value)
                    ...
                    if self.distortion_enabled:
                        ...
                        line_width = 1 + (self.distortion_amount / 5.0)  # Width based on distortion amount
...
class Ui_MainWindow(QMainWindow):
    def setupUi(self, MainWindow):
        ...
        self.eq_settings = {
            ...
            'lows': 0,
            'distortion': False,
            'distortion_amount': 5,  # Range 0-10
            'pitch_shift': 0,  # Range -12 to 12 semitones
            'eq_curve': [0] * 100    # Custom EQ curve for each frequency band
        }
        ...
        # Connect progress bar for seeking
        # Create a separate slider for seeking since progress bar doesn't support mouse events directly
        self.seek_slider = QSlider(Qt.Horizontal, self.controls_frame)
        self.seek_slider.setRange(0, 100)
        self.seek_slider.setValue(0)
        self.seek_slider.valueChanged.connect(self.seek_position_changed)
        self.controls_layout.addWidget(self.seek_slider)
        
        # Keep progress bar as a display only
        self.progress_bar = QProgressBar(self.controls_frame)
        self.progress_bar.setTextVisible(True)
        self.progress_bar.setRange(0, 100)
        ...
        # Actually, let's use a slider for seeking, and progress bar as display only
        # We'll just have both visible, with the slider for seeking and bar for display
        ...
        self.shuffle_button.setCheckable(True)  # Toggle button
        ...
        self.distortion_layout = QHBoxLayout()
        self.distortion_label = QLabel("Distortion Amount:", self.eq_tab)
        self.distortion_slider = QSlider(Qt.Horizontal, self.eq_tab)
        self.distortion_slider.setRange(0, 10)
        self.distortion_slider.setValue(5)
        self.distortion_value_label = QLabel("5", self.eq_tab)
        self.distortion_slider.valueChanged.connect(lambda v: self.distortion_value_label.setText(str(v)))
        
        self.distortion_layout.addWidget(self.distortion_label)
        self.distortion_layout.addWidget(self.distortion_slider)
        self.distortion_layout.addWidget(self.distortion_value_label)
        
        # Pitch Shift Slider
        self.pitch_layout = QHBoxLayout()
        self.pitch_label = QLabel("Pitch Shift:", self.eq_tab)
        self.pitch_slider = QSlider(Qt.Horizontal, self.eq_tab)
        self.pitch_slider.setRange(-12, 12)  # Semitones range
        self.pitch_slider.setValue(0)
        self.pitch_value_label = QLabel("0", self.eq_tab)
        self.pitch_slider.valueChanged.connect(lambda v: self.pitch_value_label.setText(f"{v:+d}"))
        
        self.pitch_layout.addWidget(self.pitch_label)
        self.pitch_layout.addWidget(self.pitch_slider)
        self.pitch_layout.addWidget(self.pitch_value_label)
        
        # Custom EQ Curve Button
        self.eq_curve_button = QPushButton("Custom EQ Curve...", self.eq_tab)
        self.eq_curve_button.clicked.connect(self.open_eq_curve_dialog)
        
        # Reset EQ Button
        self.reset_eq_button = QPushButton("Reset EQ", self.eq_tab)
        self.reset_eq_button.clicked.connect(self.reset_eq_settings)
        ...
        self.eq_layout.addLayout(self.distortion_layout)
        self.eq_layout.addLayout(self.pitch_layout)  # Add pitch controls
        self.eq_layout.addWidget(self.eq_curve_button)
        self.eq_layout.addWidget(self.reset_eq_button)
        ...
        self.pitch_slider.valueChanged.connect(self.update_pitch_settings)
        
        # Connect playlist item click to play the selected track
        self.playlist.itemClicked.connect(self.on_playlist_item_clicked)
        
        # Connect seek slider to seeking function
        self.seek_slider.valueChanged.connect(self.seek_position_changed)
    ...
    def set_visualization_mode(self, mode):
        ...
        self.shuffled_indices = []  # To track shuffled order when shuffle is active
        self.shuffle_active = False  # Flag to track if shuffle is active
        
        # Initialize EQ settings after UI is created
        self.update_eq_settings()
    ...
    def load_directory(self):
        ...
        directory = QFileDialog.getExistingDirectory(
            self, "Select Directory with MP3 Files", self.load_last_directory())
        
        if directory:
            # Save the directory to config file
            self.save_current_directory(directory)
            
            # Load the files
            self.load_directory_from_path(directory)
    
    def load_directory_from_path(self, directory):
        """Load MP3 files from a specific directory path without prompting user"""
        if directory and os.path.exists(directory):
            ...
            # Update status
            self.statusbar.showMessage(f"Loaded {len(mp3_files)} MP3 files from: {os.path.basename(directory)}")
    ...
    def next_track(self):
        """Play the next track - if shuffle is active, play a random track"""
        if self.shuffle_active:
            # Get all non-hidden (visible) items for random selection
            visible_items = []
            for i in range(self.playlist.count()):
                item = self.playlist.item(i)
                if not item.isHidden():  # Only consider non-hidden items
                    visible_items.append(i)
            
            if visible_items:
                import random
                current_row = self.playlist.currentRow()
                # Pick a random item that's not the current one (if possible)
                possible_items = [i for i in visible_items if i != current_row]
                if possible_items:
                    # Choose a random different track
                    random_row = random.choice(possible_items)
                    self.playlist.setCurrentRow(random_row)
                else:
                    # If only one item is available, play it
                    random_row = random.choice(visible_items)
                    self.playlist.setCurrentRow(random_row)
                self.play_track()
        else:
    ...
    def toggle_shuffle(self):
        ...
        self.shuffle_active = self.shuffle_button.isChecked()
        if self.shuffle_active:
            self.statusbar.showMessage("Shuffle mode enabled")
            # Update button appearance
            self.shuffle_button.setStyleSheet("QPushButton { background-color: #4CAF50; color: white; }")
        else:
            self.statusbar.showMessage("Shuffle mode disabled")
            # Update button appearance
            self.shuffle_button.setStyleSheet("")  # Reset to default
    ...
    def open_eq_curve_dialog(self):
        """Open a dialog to adjust custom EQ curve"""
        from PySide6.QtWidgets import QDialog, QGridLayout, QSlider
        from PySide6.QtCore import Qt
        
        # Create a dialog with sliders for each frequency band
        dialog = QDialog(self)
        dialog.setWindowTitle("Custom EQ Curve")
        dialog.resize(800, 300)
        
        grid_layout = QGridLayout(dialog)
        
        # Create 20 sliders for different frequency bands
        for i in range(20):
            label = QLabel(f"{i*5}-{(i+1)*5}%", dialog)
            slider = QSlider(Qt.Vertical, dialog)
            slider.setRange(-10, 10)  # Range from -10dB to +10dB
            slider.setValue(self.eq_settings['eq_curve'][i * 5] if i * 5 < len(self.eq_settings['eq_curve']) else 0)
            
            # Store reference to slider for later retrieval
            slider.setProperty("band_index", i)
            slider.valueChanged.connect(lambda value, idx=i: self.update_eq_curve(idx, value))
            
            col = i % 10
            row = i // 10
            grid_layout.addWidget(label, row * 2, col)
            grid_layout.addWidget(slider, row * 2 + 1, col)
        
        # Reset curve button
        reset_curve_btn = QPushButton("Reset Curve", dialog)
        reset_curve_btn.clicked.connect(lambda: self.reset_eq_curve(dialog))
        grid_layout.addWidget(reset_curve_btn, 2, 10, 1, 2)
        ...
        grid_layout.addLayout(btn_layout, 3, 0, 1, 12)
        
        dialog.exec()
    
    def update_eq_curve(self, band_idx, value):
        """Update a specific band in the EQ curve"""
        # Update the corresponding section of the EQ curve array
        start_idx = band_idx * 5
        end_idx = min((band_idx + 1) * 5, 100)
        for i in range(start_idx, end_idx):
            if i < len(self.eq_settings['eq_curve']):
                self.eq_settings['eq_curve'][i] = value
        
        # Update visualization
        self.visualizer.set_eq_curve(self.eq_settings['eq_curve'])
    
    def reset_eq_curve(self, dialog):
        """Reset the EQ curve to flat"""
        self.eq_settings['eq_curve'] = [0] * 100
        
        # Update visualization
        self.visualizer.set_eq_curve(self.eq_settings['eq_curve'])
    
    def reset_eq_settings(self):
        """Reset all EQ settings to default"""
        self.highs_slider.setValue(0)
        self.mids_slider.setValue(0)
        self.lows_slider.setValue(0)
        ...
        # Update visualization
        self.visualizer.set_eq_values(0, 0, 0)
        self.visualizer.set_eq_curve([0] * 100)
        self.visualizer.set_distortion_enabled(False)
    ...
    def update_eq_settings(self):
        ...
        self.eq_settings['mids'] = self.mids_slider.value()
        self.eq_settings['lows'] = self.lows_slider.value()
        self.eq_settings['distortion_amount'] = self.distortion_slider.value()
        ...
        self.visualizer.set_distortion_amount(self.eq_settings['distortion_amount'])
    
    def on_playlist_item_clicked(self, item):
        """Handle when a playlist item is clicked by the user"""
        # Play the selected track
        self.play_track()


    def load_last_directory(self):
        """Load the last directory used from config file"""
        try:
    ...
    def save_current_directory(self, directory):
        ...
        try:
            ...
            pass  # Silently fail if we can't write config file
...
if __name__ == "__main__":
    ...
    # Load and pre-populate playlist with last directory if available
    last_dir = ui.load_last_directory()
    if last_dir:
        ui.load_directory_from_path(last_dir)  # Use a new method that doesn't prompt user
