...
// This is an example game module demonstrating the new enhanced IPC system with texture support
int main() {
    ...
    float pulse_alpha = 0.5f;
    float wave_alpha = 1.0f;
    ...
    while (1) {
        ...
        if (fgets(command, sizeof(command), stdin) != NULL) {
            ...
            // Update alpha values for animation effects
            player_alpha = 0.5f + 0.5f * sin((float)(score) / 5.0f);  // Pulsing alpha for player
            enemy_alpha = 0.3f + 0.7f * cos((float)(score) / 7.0f);   // Pulsing alpha for enemy
            pulse_alpha = 0.2f + 0.8f * fabsf(sinf((float)(score) / 3.0f));  // Pulsing alpha for powerup
            wave_alpha = 0.4f + 0.6f * sin((float)(score) / 4.0f + M_PI/2);  // Phase-shifted alpha

            // Write multiple types of messages to stdout (which is piped to the main app).
            
            // 1. Send player shape (blue moving square with variable alpha) with missingno.png texture
            printf("SHAPE;SQUARE;player;%.2f;%.2f;%.2f;20.0;20.0;5.0;0.0;0.0;1.0;%.2f;0;0;0;1;1;1;texture_src=task/!.no-glew_img.ld.gl]#/missingno.png\n", 
                   player_x, player_y, player_z, player_alpha);
            
            // 2. Send enemy shape (red square with variable alpha) with missingno.png texture
            printf("SHAPE;RECT;enemy;%.2f;%.2f;%.2f;15.0;15.0;5.0;1.0;0.0;0.0;%.2f;0;0;0;1;1;1;texture_src=task/!.no-glew_img.ld.gl]#/missingno.png\n", 
                   enemy_x, enemy_y, enemy_z, enemy_alpha);
            
            // 3. Send second enemy shape (orange square - static alpha) with missingno.png texture
            printf("SHAPE;SQUARE;enemy2;%.2f;%.2f;%.2f;12.0;12.0;5.0;1.0;0.5;0.0;1.0;0;0;0;1;1;1;texture_src=task/!.no-glew_img.ld.gl]#/missingno.png\n", 
                   enemy2_x, enemy2_y, enemy2_z);
            
            // 4. Send powerup shape (green circle with pulsing alpha) with missingno.png texture
            printf("SHAPE;CIRCLE;powerup;%.2f;%.2f;%.2f;8.0;8.0;5.0;0.0;1.0;0.0;%.2f;0;0;0;1;1;1;texture_src=task/!.no-glew_img.ld.gl]#/missingno.png\n", 
                   powerup_x, powerup_y, powerup_z, pulse_alpha);
            
            // 5. Send background element (purple pulsing square with static alpha) with missingno.png texture
            printf("SHAPE;SQUARE;bg_element;%.2f;%.2f;%.2f;%.2f;%.2f;5.0;0.5;0.0;0.5;0.7;0;0;0;1;1;1;texture_src=task/!.no-glew_img.ld.gl]#/missingno.png\n", 
                   bg_element_x, bg_element_y, bg_element_z, bg_element_size, bg_element_size);

            // 6. Send animated shapes (rotating triangle with wave alpha) with missingno.png texture
            float time = (float)score / 10.0f;
            float rot_x = 400.0f + 100.0f * cos(time);
            float rot_y = 300.0f + 100.0f * sin(time);
            printf("SHAPE;TRIANGLE;rotating_shape;%.2f;%.2f;%.2f;15.0;15.0;5.0;1.0;1.0;0.0;%.2f;0;0;0;1;1;1;texture_src=task/!.no-glew_img.ld.gl]#/missingno.png\n", 
                   rot_x, rot_y, 0.0f, wave_alpha);
