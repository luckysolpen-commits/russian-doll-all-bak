// craft.js - 3D Crafting Game for JS CLI - Multi-Window Compatible
...
class CraftApp {
    ...
    createGUI() {
        ...
        if (typeof document !== 'undefined' && !existingWindow) {
            ...
            const maxWidth = Math.min(900, window.innerWidth - 100);  // Reduced from 1000
            const maxHeight = Math.min(600, window.innerHeight - 100); // Reduced from 700
                    ...
                    <div id="${this.instanceId}-game-container" style="display: grid; grid-template-areas: 'header header' 'sidebar main'; grid-template-rows: 50px 1fr; grid-template-columns: 15% 85%; height: 100%; width: 100%;">
                        <div id="${this.instanceId}-header" style="grid-area: header; background-color: rgba(0, 0, 0, 0.5); border-bottom: 2px solid white; display: flex; align-items: center; padding-left: 10px;">
                            <div class="dropdown" style="position: relative; display: inline-block;">
                                <button id="${this.instanceId}-fileBtn" style="background: none; border: 1px solid white; color: white; padding: 5px 10px; margin-right: 10px; cursor: pointer;">File</button>
                                <div id="${this.instanceId}-fileDropdown" class="dropdown-content" style="display: none; position: absolute; background-color: rgba(0, 0, 0, 0.7); min-width: 120px; box-shadow: 0px 8px 16px 0px rgba(255,255,255,0.1); z-index: 100; border: 1px solid white;">
                                    <button id="${this.instanceId}-newBtn" style="background: none; border: none; color: white; padding: 8px 12px; width: 100%; text-align: left; cursor: pointer; font-size: 14px;">New</button>
                                    ...
                                    <button id="${this.instanceId}-saveBtn" style="background: none; border: none; color: white; padding: 8px 12px; width: 100%; text-align: left; cursor: pointer; font-size: 14px;">Save</button>
                                </div>
                            </div>
                            <button id="${this.instanceId}-textBtn" style="background: none; border: 1px solid white; color: white; padding: 5px 10px; margin-right: 10px; cursor: pointer;">text</button>
                            <button id="${this.instanceId}-platBtn" style="background: none; border: 1px solid white; color: white; padding: 5px 10px; margin-right: 10px; cursor: pointer;">plat</button>
                        ...
                        <div id="${this.instanceId}-sidebar" style="grid-area: sidebar; background-color: rgba(0, 0, 0, 0.5); border-right: 2px solid white; overflow-y: auto; padding: 10px; color: white;">
                            ...
                            <div class="sidebar-window" style="margin-bottom: 20px;">
                                ...
                                <div id="${this.instanceId}-file-viewer" style="height: 65px; overflow-y: auto; border: 1px solid #555; padding: 5px; margin-top: 10px;">
                                    <!-- File items will be added here by JavaScript -->
                                </div>
                                <!-- Hidden file input for loading maps -->
                                <input type="file" id="${this.instanceId}-mapFileInput" accept=".txt" style="display: none;">
                            </div>
                        </div>
                        
                        <div id="${this.instanceId}-main" style="grid-area: main; position: relative;">
                            <div id="${this.instanceId}-gameContainer" style="position: relative; width: 100%; height: 100%;">
                                <div id="${this.instanceId}-canvasContainer" style="width: 100%; height: 100%;"></div>
                                
                                <div id="${this.instanceId}-crosshair" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 20px; height: 20px; pointer-events: none; z-index: 10;">
                                    <div id="${this.instanceId}-horizontal" style="position: absolute; background-color: white; top: 50%; left: 50%; width: 20px; height: 2px; transform: translate(-50%, -50%);"></div>
                                    <div id="${this.instanceId}-vertical" style="position: absolute; background-color: white; top: 50%; left: 50%; width: 2px; height: 20px; transform: translate(-50%, -50%);"></div>
                                </div>
                                
                                <div id="${this.instanceId}-uiContainer" style="position: absolute; top: 20px; right: 20px; width: 200px; height: 200px; background-color: rgba(0, 0, 0, 0.5); border: 2px solid white; pointer-events: none; z-index: 10;">
                                    <canvas id="${this.instanceId}-minimapCanvas" width="200" height="200" style="width: 100%; height: 100%;"></canvas>
                                </div>
                                
                                <div id="${this.instanceId}-coordinates" style="position: absolute; top: 220px; right: 20px; color: white; font-size: 14px; background-color: rgba(0, 0, 0, 0.5); padding: 5px; pointer-events: none; z-index: 10;">X: 0.0, Y: 0.0, Z: 0.0</div>
                                
                                <div id="${this.instanceId}-controls" style="position: absolute; bottom: 20px; left: 20px; color: white; background-color: rgba(0, 0, 0, 0.5); padding: 10px; font-size: 14px; pointer-events: none; z-index: 10;">
                                ...
                                <div id="${this.instanceId}-inventory" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 300px; height: 200px; background-color: rgba(0, 0, 0, 0.8); border: 2px solid white; color: white; padding: 20px; display: none; pointer-events: all; z-index: 100;">
                                    <button class="close-btn" onclick="window.appInstances['${this.instanceId}'].toggleInventory()" style="position: absolute; top: 10px; right: 10px; background: none; border: none; color: white; font-size: 20px; cursor: pointer;">X</button>
                                    <h3>Inventory</h3>
                                    <p>Empty for now...</p>
                                </div>

                                <div id="${this.instanceId}-menu" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 300px; height: 200px; background-color: rgba(0, 0, 0, 0.8); border: 2px solid white; color: white; padding: 20px; display: none; pointer-events: all; z-index: 100;">
                                    <button class="close-btn" onclick="window.appInstances['${this.instanceId}'].toggleMenu()" style="position: absolute; top: 10px; right: 10px; background: none; border: none; color: white; font-size: 20px; cursor: pointer;">X</button>
                                    <h3>Menu</h3>
                                    <p>Game Menu</p>
                                </div>
                            </div>
        ...
        `;
            ...
            // Add focus handler to this window to ensure proper z-index and active state management
            // This replicates the same focus handling as regular terminals
            if (craftWindow) {
                const instanceId = this.instanceId; // Capture instanceId in closure
                craftWindow.addEventListener('mousedown', function(e) {
                    ...
                    if (!e.target.closest('.title-bar') && !e.target.closest('.close-btn')) {
                        const input = craftWindow.querySelector('.cmd-input-span') || 
                                     craftWindow.querySelector(`#${instanceId}-canvasContainer`);
                        if (input) {
                            input.focus();
                        }
                    }
            ...
            if (existingWindow && !existingWindow.hasAttribute('data-focus-handler-added')) {
                const instanceId = this.instanceId; // Capture instanceId in closure
                existingWindow.addEventListener('mousedown', function(e) {
                    // Remove .active class from all terminal windows
                    document.querySelectorAll('.terminal-window').forEach(w => {
                        w.classList.remove('active');
                    });
                    ...
                    existingWindow.style.zIndex = window.currentZ++;
                    
                    // Focus on input if it exists and the click was not on the title bar
                    if (!e.target.closest('.title-bar') && !e.target.closest('.close-btn')) {
                        const input = existingWindow.querySelector('.cmd-input-span') || 
                                     existingWindow.querySelector(`#${instanceId}-canvasContainer`);
                        if (input) {
                            input.focus();
                        }
                    }
                ...
                // Make the window draggable via its title bar (same as regular terminals)
                if (typeof makeDraggable === 'function') {
                    makeDraggable(existingWindow);
                }
                
                existingWindow.setAttribute('data-focus-handler-added', 'true');
            }
        }
    }
    ...
    showGUI() {
        ...
        if (this.gameWindow) {
            ...
            // Calculate dimensions that are smaller than the browser window
            const maxWidth = Math.min(900, window.innerWidth - 100);  // Leave 50px margin on each side, reduced from 1000
            const maxHeight = Math.min(600, window.innerHeight - 100); // Leave 50px margin on top/bottom, reduced from 700
            ...
            // Position in the center of the browser window
    ...
    setupCraftGame() {
        ...
        const minimapCanvas = getEl('minimapCanvas');
        if (minimapCanvas) {
        ...
        view2DCanvas.id = `${this.instanceId}-view2DCanvas`;
        ...
        function updateSelections() {
            const paletteItems = document.querySelectorAll(`#${self.instanceId}-color-palette .palette-item`);
            ...
            const status = getEl('color-status');
            if (status) {
        ...
        class ProjectManager {
            ...
            updateFileViewer() {
                ...
                const fileViewer = getEl('file-viewer');
...
0,5,1,0,4,1,1,0,0,0`;
        ...
        function setupEventListeners() {
            ...
            document.querySelectorAll(`#${self.instanceId} .project-btn`).forEach(button => {
            ...
            const fileBtn = getEl('fileBtn');
            const fileDropdown = getEl('fileDropdown');
            ...
            if (canvasContainer) {
                ...
                // Set up click handler to focus the canvas when clicked, but allow bubbling for window focus
                canvasContainer.addEventListener('click', (e) => {
                    canvasContainer.focus();
                    // Don't stop propagation to allow parent window to get focus event too
                });
