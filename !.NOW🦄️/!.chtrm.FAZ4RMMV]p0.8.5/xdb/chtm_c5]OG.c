...
void render_elements() {
            ...
            if (el->width > 0 && el->height > 0) {
                ...
                if (el->label[0] != '\0') {
                    ...
                    if (i == active_interaction_element) {
                        ...
                        printf("[^] %d. %s\n", i + 1, processed_label);  // Show interaction mode indicator
                    } else if (el->focused) {
                        // Element is just selected (navigation) - show with ">"
                        printf("[>] %d. %s\n", i + 1, processed_label);  // Show selected indicator
                    } else {
                        // Element is not focused - show with regular brackets
                        printf("[ ] %d. %s\n", i + 1, processed_label);  // Show unselected
                    }
                }
    ...
    // Display navigation input prompt at the bottom of the screen
    if (nav_input_active) {
        printf("\nNavigation: %s", nav_input_buffer);
    } else {
        printf("\nNavigation: Enter element index (1-%d) or 'q' to quit", element_count);
    }
...
bool process_input(int ch) {
    ...
    if (ch == '\r' || ch == '\n') {
        ...
        if (nav_input_active) {
            if (nav_input_pos > 0) {
                ...
                int target_index = atoi(nav_input_buffer);
                // Convert to 0-based index since elements are displayed as 1-based
                target_index = target_index - 1;
                
                // Validate range
                if (target_index >= 0 && target_index < element_count) {
                    // Check if the target element is interactive
                    UIElement* target_el = &elements[target_index];
                    if (strcmp(target_el->type, "link") == 0 || 
                        strcmp(target_el->type, "button") == 0 ||
                        strcmp(target_el->type, "canvas") == 0 ||
                        strcmp(target_el->type, "cli_io") == 0) {
                        // Set focus to the target element
                        set_focus(target_index);
                        active_interaction_element = target_index;
                    } else {
                        // If not interactive, just clear the screen and show out of range message
                        printf("\033[H\033[J"); // Clear screen
                        printf("Selected element is not interactive.\n");
                        render_elements();
                    }
                } else {
    ...
    if (isprint(ch)) {
        ...
        if (nav_input_active) {
            if (nav_input_pos < 255 && (isdigit(ch) || ch == '.')) {  // Only allow digits and period for now
