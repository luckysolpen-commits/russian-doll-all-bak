...
bool is_element_visible(UIElement* el) {
    if (el->parent_index != -1) {
        ...
        if (strcmp(parent->type, "menu") == 0) {
            ...
            // For other types of parent elements, use different logic as needed
            // For now, make them visible only when parent is active too
            return (active_interaction_element == el->parent_index);
        }
    } else {
...
bool process_input(int ch) {
    ...
    if (ch == '\r' || ch == '\n') {
        ...
        // If any element is active [^]: Enter is passed directly to the appropriate module and element stays active
            ...
            if (strcmp(el->type, "canvas") == 0) {
                ...
                // For buttons, check if this is the button that should execute on this Enter press
                if (execute_on_next_enter == active_interaction_element && strlen(el->onClick) > 0) {
                    // Execute the onClick command and then reset the execute flag
                    send_ipc_command(el->onClick);
                    execute_on_next_enter = -1; // Reset the flag so next Enter doesn't execute
                } else if (execute_on_next_enter != active_interaction_element) {
                    // This is the first Enter press - mark this element for execution on next Enter
                    execute_on_next_enter = active_interaction_element;
                }
                ...
                if (el->num_children > 0 && el->current_child_focus >= 0) {
                    // Check if the currently focused child is marked for execution
                    int child_idx = el->current_child_focus;
                    int child_element_idx = el->children[child_idx];
                    if (child_element_idx >= 0 && child_element_idx < element_count) {
                        UIElement* child_el = &elements[child_element_idx];
                        if (execute_on_next_enter == child_element_idx) {
                            // Execute the child's action (onClick or href) and reset the flag
                            if (strcmp(child_el->type, "button") == 0 && strlen(child_el->onClick) > 0) {
                                send_ipc_command(child_el->onClick);
                                execute_on_next_enter = -1; // Reset the flag
                            ...
                            // If the child is a link, navigate to its href
                                ...
                                execute_on_next_enter = -1; // Reset the flag before navigating
                            ...
                            // This is the first Enter press - mark the child for execution on next Enter
                            execute_on_next_enter = child_element_idx;
                            return true; // Stay in menu active state and trigger refresh to show [^] marker
                        }
                    }
                }
    ...
    if (ch == ESC_KEY) {
        ...
        // Check if there's a child marked for execution, if so just clear the flag and focus the child
        if (execute_on_next_enter != -1) {
            // If there was a child marked for execution, make it the current focus of its parent
            UIElement* child_el = &elements[execute_on_next_enter];
            if (child_el->parent_index != -1) {
                UIElement* parent_el = &elements[child_el->parent_index];
                // Only update focus if the parent is currently active or was the parent of the active element
                if (strcmp(parent_el->type, "menu") == 0 && parent_el == &elements[active_interaction_element]) {
                    // Find which child index corresponds to the element that was marked for execution
                    for (int j = 0; j < parent_el->num_children; j++) {
                        if (parent_el->children[j] == execute_on_next_enter) {
            ...
            execute_on_next_enter = -1;
            return true;  // Return early to avoid navigating away from the current menu
        }
        // Otherwise, handle active elements normally
        if (active_interaction_element != -1) {
        ...
        if (active_interaction_element != -1) {
            ...
            if (strcmp(el->type, "menu") == 0) {
                if (el->num_children > 0) {
                    ...
                    // Reset the execute flag when navigation occurs
                    execute_on_next_enter = -1;
                }
