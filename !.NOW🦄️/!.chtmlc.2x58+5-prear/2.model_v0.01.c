...
// Structure for managing multiple modules
typedef struct {
    pid_t pid;
...
ModuleProcess modules[MAX_MODULES];
int num_modules = 0;
...
void init_model(const char* module_path) {
    ...
    // Initialize the first module (for backward compatibility with command-line modules)
    // To handle multiple modules, we'll need to create a separate function
    if (module_path != NULL && strlen(module_path) > 0) {
        // For now, initialize a single module for backward compatibility
        // Later we'll use this as a template for multiple modules from CHTML
        modules[0].active = 1;
...
void model_send_input(const char* input) {
    for (int i = 0; i < MAX_MODULES; i++) {
        if (modules[i].active && modules[i].pid > 0) {
...
void model_send_input_to_module(const char* input, int module_idx) {
    if (module_idx >= 0 && module_idx < MAX_MODULES && modules[module_idx].active && modules[module_idx].pid > 0) {
...
int update_model() {
    ...
    for (int mod_idx = 0; mod_idx < MAX_MODULES; mod_idx++) {
        if (!modules[mod_idx].active || modules[mod_idx].pid <= 0) {
