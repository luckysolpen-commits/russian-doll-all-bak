...
// Define DOM constants (matching those in view.c)
...
// Flex sizing structures (matching those in view.c)
typedef enum {
    FLEX_UNIT_PIXEL,
    FLEX_UNIT_PERCENT
} FlexUnit;
...
// Forward declaration for DOM structures
...
// DOM Node structure (matching the one in view.c)
...
// Extern declarations for DOM functionality from view.c
extern DOMNode* dom_tree_root;
extern DOMNode* find_node_by_id(DOMNode* root, const char* id);
extern int find_nodes_by_tag_name(DOMNode* root, const char* tag_name, DOMNode** results, int max_results, int* count);
extern const char* get_attribute(DOMNode* node, const char* name);
...
// External declarations for flex size functions defined in view.c
extern int resolve_flex_size(FlexSize flex_size, int window_size);
extern void calculate_element_layout(UIElement* element);
extern void calculate_layout();
...
typedef void (*event_handler_func_ptr)();

// Function to execute an event handler by name - Send the event name directly to the module
void execute_event_handler_by_name(const char* event_handler_name) {
    // Create a command string in the format "EVENT:<handler_name>"
    char command[256];
    snprintf(command, sizeof(command), "EVENT:%s", event_handler_name);
    model_send_input(command);
    
    // Add debugging output
    printf("Event '%s' sent to module for processing\n", event_handler_name);
}
...
void handle_element_event(const char* event_handler) {
    // Use the new dynamic event system that sends events directly to the module
    execute_event_handler_by_name(event_handler);
    
    // Special case: for UI-only handlers (like view switching), handle them in the controller
    if (strcmp(event_handler, "switch_to_2d_handler") == 0) {
        switch_to_2d_view();
    ...
    if (button == 0) { // Left mouse button
        if (state == 0) { // Mouse button down
            ...
            for (int i = 0; i < num_elements; i++) {
                ...
                int abs_y = window_height - (parent_y + elements[i].y + elements[i].calculated_height); // Convert to OpenGL coordinates to match view.c
                ...
                if (strcmp(elements[i].type, "menuitem") == 0 && elements[i].parent != -1 && 
                    ...
                    int submenu_y = parent_menu_abs_y + elements[parent_menu].y + elements[parent_menu].calculated_height;
                ...
                if (x >= abs_x && x <= abs_x + elements[i].calculated_width &&
                    ...
                    if (strcmp(elements[i].type, "button") == 0) {
                        ...
                        int canvas_abs_y = window_height - (parent_y + elements[i].y + elements[i].calculated_height); // Convert to OpenGL coords
            ...
            for (int i = 0; i < num_elements; i++) {
                ...
                int abs_x = parent_x + elements[i].x;
                int abs_y = window_height - (parent_y + elements[i].y + elements[i].calculated_height); // Convert to OpenGL coordinates
        ...
        if (state == 0) { // Mouse button down
            ...
            if (generic_context_menu_idx != -1) {
                // Calculate parent offsets that will be applied during rendering
                ...
                // Fix y coordinate by compensating for parent offset (not inverted)
                ...
                int final_y_pos = parent_y_total + elements[generic_context_menu_idx].y;  // Using simpler y calculation
                ...
                printf("Context menu at mouse (%d, %d), set element to (%d, %d), parent offset (%d, %d)\\n", 
                       x, y, elements[generic_context_menu_idx].x, elements[generic_context_menu_idx].y, parent_x_total, parent_y_total);
                glutPostRedisplay();
            } else {
...
void mouse_motion(int x, int y) {
    if (active_slider_index != -1) {
        ...
        int new_value;
        // Check if this is a vertical slider by looking at the aspect ratio
        int is_vertical = (slider->calculated_height > slider->calculated_width);  // If height is greater than width, consider it vertical
