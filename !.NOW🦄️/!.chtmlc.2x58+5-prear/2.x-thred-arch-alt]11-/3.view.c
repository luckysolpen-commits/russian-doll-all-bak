...
// Structure for attribute-value pairs
typedef struct {
...
typedef void (*event_handler_func)(DOMNode* node, void* event_data);
...
typedef struct DOMNode {
    ...
    char* class_name;
    char* text_content;
    DOMAttribute attributes[MAX_ATTRIBUTES];
    int attr_count;
    struct DOMNode* parent;
    struct DOMNode* children[MAX_CHILDREN];
    int child_count;
    event_handler_func event_handlers[EVENT_TYPE_MAX];
    int event_handler_count;
    void* ui_element_ref;  // Reference to the original UIElement if applicable
} DOMNode;
...
DOMNode* create_dom_node(const char* tag_name);
void free_dom_node(DOMNode* node);
int add_attribute(DOMNode* node, const char* name, const char* value);
const char* get_attribute(DOMNode* node, const char* name);
int add_child(DOMNode* parent, DOMNode* child);
int set_text_content(DOMNode* node, const char* text);
int register_event_handler(DOMNode* node, event_handler_func handler);
void execute_event_handlers(DOMNode* node, void* event_data);
DOMNode* find_node_by_id(DOMNode* root, const char* id);
int find_nodes_by_tag_name(DOMNode* root, const char* tag_name, DOMNode** results, int max_results, int* count);
int find_nodes_by_class_name(DOMNode* root, const char* class_name, DOMNode** results, int max_results, int* count);
int set_node_id(DOMNode* node, const char* id);
int set_node_class_name(DOMNode* node, const char* class_name);
DOMNode* init_dom_tree(const char* root_tag_name);
void init_global_dom_tree(const char* filename);
...
// Global DOM tree root
DOMNode* dom_tree_root = NULL;

// Array to store DOM nodes corresponding to UI elements during parsing
DOMNode* ui_element_dom_nodes[MAX_ELEMENTS];
...
void parse_chtml(const char* filename) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        // Create corresponding DOM node for this element and store in the mapping array
        ui_element_dom_nodes[num_elements] = create_dom_node(tag_name);
        if (ui_element_dom_nodes[num_elements]) {
            // Link the UI element to the DOM node
            ui_element_dom_nodes[num_elements]->ui_element_ref = &elements[num_elements];
            
            // Set ID if available
            if (strlen(elements[num_elements].id) > 0) {
                set_node_id(ui_element_dom_nodes[num_elements], elements[num_elements].id);
            }
            
            // Set class name (from label) if available
            if (strlen(elements[num_elements].label) > 0) {
                set_node_class_name(ui_element_dom_nodes[num_elements], elements[num_elements].label);
            }
        }
        ...
        // Add attributes to the corresponding DOM node
        if (ui_element_dom_nodes[num_elements]) {
            // Add coordinates as attributes
            if (elements[num_elements].x != 0) {
                char val[20];
                sprintf(val, "%d", elements[num_elements].x);
                add_attribute(ui_element_dom_nodes[num_elements], "x", val);
            }
            if (elements[num_elements].y != 0) {
                char val[20];
                sprintf(val, "%d", elements[num_elements].y);
                add_attribute(ui_element_dom_nodes[num_elements], "y", val);
            }
            if (elements[num_elements].width.unit == FLEX_UNIT_PERCENT) {
                char val[20];
                sprintf(val, "%.2f%%", elements[num_elements].width.value);
                add_attribute(ui_element_dom_nodes[num_elements], "width", val);
            } else if (elements[num_elements].width.unit == FLEX_UNIT_PIXEL && elements[num_elements].width.value != 0) {
                char val[20];
                sprintf(val, "%.0f", elements[num_elements].width.value);
                add_attribute(ui_element_dom_nodes[num_elements], "width", val);
            }
            if (elements[num_elements].height.unit == FLEX_UNIT_PERCENT) {
                char val[20];
                sprintf(val, "%.2f%%", elements[num_elements].height.value);
                add_attribute(ui_element_dom_nodes[num_elements], "height", val);
            } else if (elements[num_elements].height.unit == FLEX_UNIT_PIXEL && elements[num_elements].height.value != 0) {
                char val[20];
                sprintf(val, "%.0f", elements[num_elements].height.value);
                add_attribute(ui_element_dom_nodes[num_elements], "height", val);
            }
            
            // Add ID if present
            if (strlen(elements[num_elements].id) > 0) {
                add_attribute(ui_element_dom_nodes[num_elements], "id", elements[num_elements].id);
            }
            
            // Add label if present
            if (strlen(elements[num_elements].label) > 0) {
                add_attribute(ui_element_dom_nodes[num_elements], "label", elements[num_elements].label);
            }
            
            // Add onClick if present
            if (strlen(elements[num_elements].onClick) > 0) {
                add_attribute(ui_element_dom_nodes[num_elements], "onClick", elements[num_elements].onClick);
            }
            
            // Add href if present (for links)
            if (strlen(elements[num_elements].href) > 0) {
                add_attribute(ui_element_dom_nodes[num_elements], "href", elements[num_elements].href);
            }
            
            // Add min/max/step for slider
            if (strcmp(elements[num_elements].type, "slider") == 0) {
                char val[20];
                sprintf(val, "%d", elements[num_elements].slider_min);
                add_attribute(ui_element_dom_nodes[num_elements], "min", val);
                sprintf(val, "%d", elements[num_elements].slider_max);
                add_attribute(ui_element_dom_nodes[num_elements], "max", val);
                sprintf(val, "%d", elements[num_elements].slider_step);
                add_attribute(ui_element_dom_nodes[num_elements], "step", val);
                sprintf(val, "%d", elements[num_elements].slider_value);
                add_attribute(ui_element_dom_nodes[num_elements], "value", val);
            }
            
            // Add checked for checkbox
            if (strcmp(elements[num_elements].type, "checkbox") == 0) {
                add_attribute(ui_element_dom_nodes[num_elements], "checked", 
                              elements[num_elements].is_checked ? "true" : "false");
            }
        }
        
        // If this element is a container (panel, menu, etc.), it might have children
        // Check if the tag should be pushed onto the stack for potential children
        if (strcmp(tag_name, "panel") == 0 || strcmp(tag_name, "canvas") == 0 || 
            strcmp(tag_name, "menu") == 0 || strcmp(tag_name, "window") == 0 ||
            strcmp(tag_name, "header") == 0 || strcmp(tag_name, "container") == 0) {
            stack_top++;
            parent_stack[stack_top] = num_elements;
        }
    ...
    // Now build the proper DOM tree structure using the parent-child relationships
    for (int i = 0; i < num_elements; i++) {
        if (ui_element_dom_nodes[i] && elements[i].parent >= 0 && elements[i].parent < num_elements) {
            // Add this node as a child of its parent in the DOM tree
            if (ui_element_dom_nodes[elements[i].parent]) {
                add_child(ui_element_dom_nodes[elements[i].parent], ui_element_dom_nodes[i]);
            }
        }
    }
    
    // The root of the DOM tree is the first element (usually window)
    if (num_elements > 0 && ui_element_dom_nodes[0]) {
        dom_tree_root = ui_element_dom_nodes[0];
    } else {
        dom_tree_root = NULL;
    }
...
void init_view(const char* filename) {
    ...
    // Initialize the DOM tree - it's already built by the parser now
    // The parser now builds the DOM tree directly during parsing
}
...
// Structure to hold element index and z_level for sorting
typedef struct {
    int index;
    int z_level;
} ElementIndex;

// Comparison function for qsort to sort element indices by z_level
int compare_element_indices(const void* a, const void* b) {
    ElementIndex* idx_a = (ElementIndex*)a;
    ElementIndex* idx_b = (ElementIndex*)b;
    return idx_a->z_level - idx_b->z_level;
}
...
void display() {
    ...
    // Create an array of indices for sorting to maintain parent-child relationships
    ElementIndex* sorted_indices = NULL;
    if (num_elements > 1) {
        sorted_indices = (ElementIndex*)malloc(num_elements * sizeof(ElementIndex));
        if (sorted_indices) {
            // Initialize the indices array
            for (int i = 0; i < num_elements; i++) {
                sorted_indices[i].index = i;
                sorted_indices[i].z_level = elements[i].z_level;
            }
            
            // Sort the indices by z_level
            qsort(sorted_indices, num_elements, sizeof(ElementIndex), compare_element_indices);
        }
    }

    // Draw all regular elements first (now sorted by z_level)
    for (int i = 0; i < num_elements; i++) {
        // Get the actual element index after sorting
        int element_index = sorted_indices ? sorted_indices[i].index : i;
        
        // Draw the element but don't draw submenu items here
        // (they'll be drawn separately to ensure they appear on top)
        if (strcmp(elements[element_index].type, "menuitem") == 0 && 
            elements[element_index].parent != -1 && 
            strcmp(elements[elements[element_index].parent].type, "menu") == 0 &&
            elements[elements[element_index].parent].is_open) {
            // Skip drawing menuitems that are part of an open menu here
            // They will be drawn with the menu in the next step
        } else if (strcmp(elements[element_index].type, "menu") == 0 && elements[element_index].is_open) {
            // For open menus, skip drawing here since they are handled in the submenu section below
            // This prevents drawing the menu background in its original position alongside the correctly positioned submenu
        } else {
            draw_element(&elements[element_index]);
        }
    }
    ...
    for (int i = 0; i < num_elements; i++) {
        if (strcmp(elements[i].type, "menu") == 0 && elements[i].is_open) {
            ...
            // Calculate parent menu's absolute position
            ...
            int parent_menu_abs_y_coord = parent_menu_abs_y + elements[parent_menu].y;
            ...
            // Draw the main menu header/label part for context menus that have been repositioned
            // This ensures the menu header appears at the correct position (e.g. mouse click position for context menus)
            // For context menus, the header should appear at the contextual position (where mouse clicked)
            // Calculate header position based on the context menu's set position, not the submenu calculation
            int header_x = parent_menu_abs_x + elements[parent_menu].x;
            int header_y_opengl = window_height - (parent_menu_abs_y + elements[parent_menu].y + elements[parent_menu].calculated_height);
            
            // Draw menu bar background (the header part)
            glColor4f(elements[parent_menu].color[0], elements[parent_menu].color[1], elements[parent_menu].color[2], elements[parent_menu].alpha);
            glBegin(GL_QUADS);
            glVertex2i(header_x, header_y_opengl);
            glVertex2i(header_x + elements[parent_menu].calculated_width, header_y_opengl);
            glVertex2i(header_x + elements[parent_menu].calculated_width, header_y_opengl + elements[parent_menu].calculated_height);
            glVertex2i(header_x, header_y_opengl + elements[parent_menu].calculated_height);
            glEnd();
            ...
            int text_offset_x = 5;
            // Center the text vertically: start from the bottom of the menu and add centered offset
            int text_offset_y = (elements[parent_menu].calculated_height / 2) - 9;  // Use calculated_height
            glRasterPos2i(header_x + text_offset_x, header_y_opengl + text_offset_y);
            for (char* c = elements[parent_menu].label; *c != '\0'; c++) {
                glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, *c);
            }
        }
    }
    ...
    if (is_3d) {
        ...
        for (int i = 0; i < shape_count; i++) {
            ...
            if (strcmp(s->type, "SQUARE") == 0 || strcmp(s->type, "RECT") == 0) {
                ...
                glColor4f(s->color[0], s->color[1], s->color[2], s->alpha);
                // Use width and height for 3D scaling
                float radius_x = s->width / 20.0f;
                float radius_y = s->height / 20.0f;
                
                // Draw a cylinder/flat disc
                glBegin(GL_TRIANGLE_FAN);
                glVertex3f(0.0f, 0.0f, 0.0f);  // Center of circle
                for (int j = 0; j <= 20; j++) {
                    float angle = j * (2.0f * 3.14159f / 20);
                    float dx = radius_x * cos(angle);
                    float dy = 0.0f; // Flat in XZ plane
                    float dz = radius_y * sin(angle);
                    glVertex3f(dx, dy, dz);
                }
                glEnd();
                glPopMatrix();
...
// Function to create a new DOM node
DOMNode* create_dom_node(const char* tag_name) {
    ...
    node->parent = NULL;
    node->child_count = 0;
    node->event_handler_count = 0;
    node->ui_element_ref = NULL;
    
    // Set tag name
    if (tag_name) {
        node->tag_name = (char*)malloc(strlen(tag_name) + 1);
        if (node->tag_name) {
    ...
    for (int i = 0; i < EVENT_TYPE_MAX; i++) {
        node->event_handlers[i] = NULL;
    }
    
    return node;
}

// Function to free a DOM node and its children recursively
void free_dom_node(DOMNode* node) {
    if (!node) return;
...
// Function to add an attribute to a node
int add_attribute(DOMNode* node, const char* name, const char* value) {
    if (!node || !name || !value || node->attr_count >= MAX_ATTRIBUTES) {
        return 0; // Failure
    }
    
    strncpy(node->attributes[node->attr_count].name, name, MAX_ATTR_NAME_LEN - 1);
    node->attributes[node->attr_count].name[MAX_ATTR_NAME_LEN - 1] = '\0';
    
    strncpy(node->attributes[node->attr_count].value, value, MAX_ATTR_VALUE_LEN - 1);
    node->attributes[node->attr_count].value[MAX_ATTR_VALUE_LEN - 1] = '\0';
    
    node->attr_count++;
    return 1; // Success
}

// Function to get an attribute value by name
const char* get_attribute(DOMNode* node, const char* name) {
...
int set_text_content(DOMNode* node, const char* text) {
    ...
    // Free existing text content if any
    ...
    if (text) {
        node->text_content = (char*)malloc(strlen(text) + 1);
        if (node->text_content) {
...
int register_event_handler(DOMNode* node, event_handler_func handler) {
    if (!node || !handler || node->event_handler_count >= MAX_EVENT_HANDLERS) {
        return 0; // Failure
    }
    
    node->event_handlers[node->event_handler_count] = handler;
    node->event_handler_count++;
    return 1; // Success
}
...
void execute_event_handlers(DOMNode* node, void* event_data) {
    ...
    for (int i = 0; i < node->event_handler_count; i++) {
        if (node->event_handlers[i]) {
            node->event_handlers[i](node, event_data);
        }
    }
}

// Function to find a node by ID (depth-first search)
DOMNode* find_node_by_id(DOMNode* root, const char* id) {
    ...
    for (int i = 0; i < root->child_count; i++) {
        DOMNode* result = find_node_by_id(root->children[i], id);
...
// Function to find nodes by tag name (depth-first search)
int find_nodes_by_tag_name(DOMNode* root, const char* tag_name, DOMNode** results, int max_results, int* count) {
    if (!root || !tag_name || !results) return 0;
    
    // Check if this node matches the tag name
    ...
    // Recursively search children
    for (int i = 0; i < root->child_count; i++) {
        find_nodes_by_tag_name(root->children[i], tag_name, results, max_results, count);
    }
    
    return *count;
}

// Function to find nodes by class name (depth-first search)
int find_nodes_by_class_name(DOMNode* root, const char* class_name, DOMNode** results, int max_results, int* count) {
    if (!root || !class_name || !results) return 0;
    
    // Check if this node matches the class name
    if (root->class_name && strcmp(root->class_name, class_name) == 0) {
        if (*count < max_results) {
            results[*count] = root;
            (*count)++;
        }
    ...
    // Recursively search children
    for (int i = 0; i < root->child_count; i++) {
        find_nodes_by_class_name(root->children[i], class_name, results, max_results, count);
    }
    
    return *count;
}

// Function to set the ID of a node
int set_node_id(DOMNode* node, const char* id) {
    if (!node) return 0;
    
    // Free existing ID if any
    if (node->id) {
        free(node->id);
        node->id = NULL;
    }
...
// Function to set the class name of a node
int set_node_class_name(DOMNode* node, const char* class_name) {
    if (!node) return 0;
    
    // Free existing class name if any
    if (node->class_name) {
    ...
    if (class_name) {
        node->class_name = (char*)malloc(strlen(class_name) + 1);
        if (node->class_name) {
...
// Function to initialize the DOM tree with a root node
DOMNode* init_dom_tree(const char* root_tag_name) {
    return create_dom_node(root_tag_name);
}

// Function to build DOM tree from UI elements (during parsing)
DOMNode* build_dom_from_ui_elements(UIElement* elements, int num_elements) {
    if (num_elements <= 0) return NULL;
    
    // Create the root DOM node from the first UI element (usually the window element)
    DOMNode* root = create_dom_node(elements[0].type);
    if (!root) return NULL;
    
    // Set up ID if available
    if (strlen(elements[0].id) > 0) {
        set_node_id(root, elements[0].id);
    }
    
    // Set up attributes based on UI element properties
    // We'll use the first element as our root reference
    root->ui_element_ref = &elements[0];
    
    // Map UI element attributes to DOM attributes
    if (elements[0].x != 0) {
        char val[20];
        sprintf(val, "%d", elements[0].x);
        add_attribute(root, "x", val);
    }
    if (elements[0].y != 0) {
        char val[20];
        sprintf(val, "%d", elements[0].y);
        add_attribute(root, "y", val);
    }
    if (elements[0].calculated_width != 0) {
        char val[20];
        sprintf(val, "%d", elements[0].calculated_width);
        add_attribute(root, "width", val);
    }
    if (elements[0].calculated_height != 0) {
        char val[20];
        sprintf(val, "%d", elements[0].calculated_height);
        add_attribute(root, "height", val);
    }
    
    // Now add the remaining elements as children based on parent relationships
    for (int i = 1; i < num_elements; i++) {
        DOMNode* node = create_dom_node(elements[i].type);
        if (!node) continue;
        
        // Set up ID if available
        if (strlen(elements[i].id) > 0) {
            set_node_id(node, elements[i].id);
        }
        
        // Set up class if available
        if (strlen(elements[i].label) > 0) {
            set_node_class_name(node, elements[i].label);
        }
        
        // Add attributes based on UI element properties
        if (elements[i].x != 0) {
            char val[20];
            sprintf(val, "%d", elements[i].x);
            add_attribute(node, "x", val);
        }
        if (elements[i].y != 0) {
            char val[20];
            sprintf(val, "%d", elements[i].y);
            add_attribute(node, "y", val);
        }
        if (elements[i].calculated_width != 0) {
            char val[20];
            sprintf(val, "%d", elements[i].calculated_width);
            add_attribute(node, "width", val);
        }
        if (elements[i].calculated_height != 0) {
            char val[20];
            sprintf(val, "%d", elements[i].calculated_height);
            add_attribute(node, "height", val);
        }
        
        // Add href attribute if it's a link element
        if (strlen(elements[i].href) > 0) {
            add_attribute(node, "href", elements[i].href);
        }
        
        // Add onClick attribute if present
        if (strlen(elements[i].onClick) > 0) {
            add_attribute(node, "onClick", elements[i].onClick);
        }
        
        // Set UI element reference
        node->ui_element_ref = &elements[i];
        
        // Add to DOM tree based on parent relationship
        // The parent is indicated by the 'parent' field in UIElement
        if (elements[i].parent >= 0 && elements[i].parent < num_elements) {
            // Find the parent DOM node and add this as its child
            // This is simplified - in a full implementation we'd need a way to map UIElement indices to DOM nodes
            // For now, we'll just add it to the root node
            add_child(root, node);
        } else {
            // If no parent, add it directly to the root
            add_child(root, node);
        }
    }
...
// Function to get the DOM node at a specific UI element index
// This helps map between UI elements and DOM nodes
DOMNode* get_dom_node_for_ui_element(DOMNode* dom_root, int ui_element_index, UIElement* elements) {
    if (!dom_root || !elements) return NULL;
    
    // If the dom node references the requested UI element, return it
    if (dom_root->ui_element_ref == &elements[ui_element_index]) {
        return dom_root;
    }
    
    // Recursively search children
    for (int i = 0; i < dom_root->child_count; i++) {
        DOMNode* result = get_dom_node_for_ui_element(dom_root->children[i], ui_element_index, elements);
        if (result) return result;
    }
...
// Function to initialize the global DOM tree when the view is initialized
void init_global_dom_tree(const char* filename) {
    // Build the DOM tree from the UI elements that have been parsed
    // This will be called after parse_chtml in init_view
    if (dom_tree_root) {
        free_dom_node(dom_tree_root);  // Free any existing DOM tree
    }
    
    dom_tree_root = build_dom_from_ui_elements(elements, num_elements);
}
