...
#include <dirent.h>
#include <sys/stat.h>
#include <sys/select.h>  // for event-based I/O
#include <errno.h>
...
int update_model();
int update_model_nonblocking();  // New non-blocking function
...
int update_model() {
    // If no module is running (child_pid is -1), return 0 so UI updates continue normally
    if (child_pid == -1) {
...
// Function to be called continuously to update the model state from the game module using non-blocking I/O.
// Returns 1 if the model was updated, 0 otherwise.
int update_model_nonblocking() {
    ...
    // Use select() to check if data is available on the pipe without blocking
    fd_set readfds;
    ...
    if (activity < 0) {
        // Error in select
        if (errno != EINTR) {  // EINTR is ok, just means it was interrupted
            perror("select error");
        }
        return 0;
    }
    ...
    // Handle the case where read() returns -1 (error) or 0 (EOF)
    if (bytes_read < 0 && errno != EAGAIN && errno != EWOULDBLOCK) {
        // Real error occurred (not just "no data available")
        // This could mean the child process has terminated
        perror("read error from child process");
        return 0;
    }
