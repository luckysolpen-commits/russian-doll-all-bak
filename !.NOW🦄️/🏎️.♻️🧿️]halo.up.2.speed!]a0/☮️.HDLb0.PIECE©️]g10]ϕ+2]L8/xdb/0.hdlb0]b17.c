...
int max_cycles = -1; // -1 means run indefinitely (default behavior)
...
int resolve_input(unsigned short raw_input, unsigned char *ram, unsigned char switch_0, unsigned char switch_1, unsigned char clock, int *is_blank) {
    *is_blank = 0;
    ...
    else if (raw_input == 5) return switch_0;//101
    else if (raw_input == 6) return switch_1;//110
...
void run_cycle() {
    for (int i = 0; i < num_instructions; i++) {
        ...
        if (chip_location == 0) { // No logic
            int output_value = -1;
            int warn_input_a = (raw_input_a == 0 && is_blank_b); // Warn if input_a is 0 when input_b is blank
            int warn_input_b = (raw_input_b == 0 && is_blank_a); // Warn if input_b is 0 when input_a is blank

            if (is_blank_a && !is_blank_b && input_b != -1) {
                output_value = input_b; // Use input_b if input_a is blank
            } else if (is_blank_b && !is_blank_a && input_a != -1) {
                output_value = input_a; // Use input_a if input_b is blank
            } else if (!is_blank_a && !is_blank_b && (input_a != -1 || input_b != -1)) {
                // Both non-blank, prefer non-zero input
                if (input_a != 0 && input_a != -1) {
                    output_value = input_a;
                    if (raw_input_b != 2 && raw_input_b != 3) {
                        printf("Warning: Both inputs non-blank at instruction %d, using non-zero input_a (%d). input_b (%d) should be blank (2 or 3).\n", 
                               i, input_a, raw_input_b);
                    } else {
                        printf("Warning: Both inputs non-blank at instruction %d, using non-zero input_a (%d). input_b is blank but both inputs provided.\n", 
                               i, input_a);
                    }
                } else if (input_b != 0 && input_b != -1) {
                    output_value = input_b;
                    if (raw_input_a != 2 && raw_input_a != 3) {
                        printf("Warning: Both inputs non-blank at instruction %d, using non-zero input_b (%d). input_a (%d) should be blank (2 or 3).\n", 
                               i, input_b, raw_input_a);
                    } else {
                        printf("Warning: Both inputs non-blank at instruction %d, using non-zero input_b (%d). input_a is blank but both inputs provided.\n", 
                               i, input_b);
                    }
                } else {
                    // Both are zero or invalid, default to input_a
                    output_value = input_a;
                    printf("Warning: Both inputs non-blank and zero or invalid at instruction %d, defaulting to input_a (%d).\n", i, input_a);
                }
            } else {
            ...
            // Issue warning if 0 was used instead of blank
            if (warn_input_a) {
                printf("Warning: input_a at instruction %d is 0; use blank (2 or 3) for unused input.\n", i);
            } else if (warn_input_b) {
                printf("Warning: input_b at instruction %d is 0; use blank (2 or 3) for unused input.\n", i);
            }

            // Write output to CLI or RAM
            if (output_value != -1) {
                if (ram_output_address == 0) {
                    ...
                    fprintf(tape_fp, "%d%s", output_value, existing);
                    fclose(tape_fp);
                } else if (ram_output_address < 256) {
                    ram[ram_output_address] = output_value;
                    write_ram(ram);
                }
            }
...
void run_main_loop() {
    ...
    // Check if we've reached max cycles after initial run
    if (max_cycles != -1 && clock_iterations >= max_cycles) {
        printf("Reached maximum cycles (%d), exiting.\n", max_cycles);
        return;
    }

    // If max_cycles is specified, run automatically for that many cycles
    if (max_cycles != -1) {
        printf("Running automatically for %d cycles...\n", max_cycles);
        int initial_cycles = clock_iterations;
        while (clock_iterations < max_cycles) {
            run_cycle();
        }
        printf("Completed %d cycles automatically.\n", max_cycles - initial_cycles);
    }
...
int main(int argc, char *argv[]) {
    if (argc < 2 || argc > 3) {
        printf("Usage: %s program.txt [max_cycles]\n", argv[0]);
        printf("  program.txt: path to the program file to execute\n");
        printf("  max_cycles: optional maximum number of cycles to run (default: run indefinitely)\n");
        return 1;
    }
    ...
    if (argc == 3) {
        ...
        if (max_cycles <= 0) {
            printf("Error: max_cycles must be a positive integer\n");
            return 1;
        }
        printf("Running for maximum %d cycles\n", max_cycles);
    }
