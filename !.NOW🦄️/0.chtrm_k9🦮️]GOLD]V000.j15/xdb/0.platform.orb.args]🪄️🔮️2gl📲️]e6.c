...
int main(int argc, char* argv[]) {
    ...
    // Determine if the last argument is a CHTM file path
    char chtm_file_path[512] = "chtm/index.chtm"; // Default path
    
    // If the last argument contains ".chtm" (with any extension), treat it as the CHTM file path
    char* last_arg = argv[argc - 1];
    if (strstr(last_arg, ".chtm") != NULL) {
        strncpy(chtm_file_path, last_arg, sizeof(chtm_file_path) - 1);
        chtm_file_path[sizeof(chtm_file_path) - 1] = '\0';
    }

    // Calculate number of threads (limited to MAX_THREADS)
    // If last argument is a CHTM file, exclude it from thread commands
    int num_threads = (strstr(argv[argc - 1], ".chtm") != NULL) ? argc - 2 : argc - 1;
    if (num_threads <= 0) {
    ...
    // Print the CHTM file path being used
    printf("Using CHTM file: %s\n", chtm_file_path);
