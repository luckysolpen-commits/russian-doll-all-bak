...
void render_elements() {
    ...
    for (int i = 0; i < element_count; i++) {
        ...
        if (strcmp(el->type, "text") == 0) {
            ...
            if (el->width > 0 && el->height > 0) {
                ...
                if (ipc.active) {
                    // Ensure any previous commands are fully sent before sending display command
                    if (ipc.write_pipe) {
                    ...
                    // Ensure the display command is sent
                    if (ipc.write_pipe) {
                        fflush(ipc.write_pipe);
                    }
                    ...
                    // First, read any immediate responses without strict line count expectations
                    // This helps process responses that may have been sent after the previous command
                    int expected_lines = (strcmp(el->id, "emoji_palette") == 0) ? 1 : el->height;
                    int max_attempts = 20; // Maximum number of responses to read
                    int attempts = 0;
                    ...
                    while (attempts < max_attempts) {
                        if (read_ipc_response(response, sizeof(response)) == 0) {
                            ...
                            if (strncmp(response, "VAR:", 4) == 0) {
                                ...
                                // Special handling for emoji palette - store in model and print
                                ...
                                int space_count = 0;
                                int valid_char_count = 0;  // Count characters that are likely part of a canvas ('p', '.', numbers, letters, space)
                                
                                for (int j = 0; response[j] != '\0'; j++) {
                                    if (response[j] == ' ') {
                                        space_count++;
                                    } else if (response[j] == '.' || response[j] == 'p' || response[j] == 'X' || response[j] == 'O' || 
                                              (response[j] >= '0' && response[j] <= '9') || response[j] == ' ') {
                                        // Count characters that are typically used in canvas displays, including space
                                        valid_char_count++;
                                    }
                                }
                            ...
                            // If we have the expected canvas lines, we can stop reading for now
                            if (lines_read >= expected_lines) {
                                // But continue reading to ensure all VAR updates are processed
                                // We'll stop if we get a few more responses that aren't canvas lines
                                if (attempts > expected_lines + 3) break; // Allow a few more responses after canvas data
                            }
                        } else {
                    ...
                    // After reading, make sure canvas data is stored in the model
                    if (strcmp(el->id, "emoji_palette") == 0) {
                        ...
                        // For game canvas, ensure we have data for all lines
                        for (int r = lines_read; r < el->height && r < MAX_CANVAS_HEIGHT; r++) {
                            if (r == 0 && lines_read == 0) {
                                // If no data was received at all, initialize with default
                                char default_line[MAX_CANVAS_WIDTH * 2];
                                for (int c = 0; c < el->width && c < MAX_CANVAS_WIDTH; c++) {
                                    default_line[c * 2] = '.';      // Placeholder char
                                    default_line[c * 2 + 1] = ' ';  // Space separator
                                ...
                                default_line[el->width * 2] = '\0';
                                model_update_canvas_data(canvas_element_idx, r, default_line);
                            }
                        }
                    }
                } else {
    ...
    // CLI input is now handled as an element in the CHTM file, so we don't need hardcoded print
    fflush(stdout);
}
...
bool process_input(int ch) {
    ...
    // Handle Backspace (only for active CLI element)
    if ((ch == 127 || ch == 8) && active_interaction_element != -1 && strcmp(elements[active_interaction_element].type, "cli_io") == 0) {
        if (input_buffer_pos > 0) {
            printf("\b \b");
    ...
    // Handle Spacebar - send to active canvas element (but we'll remove this special handling)
    // For now, comment out the spacebar handling to see if that stops the corruption
    // if (ch == 32 && active_interaction_element != -1 && strcmp(elements[active_interaction_element].type, "canvas") == 0) {
    //     send_ipc_command("32");
    //     // Ensure the command is flushed to the module
    ...
    // Handle Enter Key (Universal Two-Stage Interaction Model)
    if (ch == '\r' || ch == '\n') {
        // Stage 2: An element is ALREADY active ([^]). This is the second Enter press, which executes the action.
        if (active_interaction_element != -1) {
            ...
            active_interaction_element = -1; // Deactivate after action
            return true;
        }
        // Stage 1: No element is active. This is the first Enter press, which activates the focused element ([>]).
    ...
    // Handle ESC, Ctrl+D, Tab for deactivation
    if (ch == ESC_KEY || ch == 4 || ch == 9) {
        if (active_interaction_element != -1) {
    ...
    // Handle Arrow Keys (Interaction vs. Navigation)
    if (ch >= ARROW_LEFT && ch <= ARROW_DOWN) {
        // If an element is active, send commands to it (if it's a canvas).
        if (active_interaction_element != -1 && strcmp(elements[active_interaction_element].type, "canvas") == 0) {
            switch (ch) {
                case ARROW_UP:    send_ipc_command("MOVE:U"); break;
                case ARROW_DOWN:  send_ipc_command("MOVE:D"); break;
                case ARROW_LEFT:
                    if (strcmp(elements[active_interaction_element].id, "emoji_palette") == 0) send_ipc_command("PALETTE_MOVE:L");
                    else send_ipc_command("MOVE:L");
                    break;
                case ARROW_RIGHT:
                    if (strcmp(elements[active_interaction_element].id, "emoji_palette") == 0) send_ipc_command("PALETTE_MOVE:R");
                    else send_ipc_command("MOVE:R");
                    break;
            }
        }
        // Otherwise, navigate focus.
        else {
    ...
    // Handle Printable Characters (only for active CLI or palette)
    // Spacebar (ch == 32) is explicitly excluded to maintain sanitary environment
    if (isprint(ch) && ch != 32) {
        if (active_interaction_element != -1) {
            UIElement* el = &elements[active_interaction_element];
            if (strcmp(el->type, "cli_io") == 0) {
                ...
                fflush(stdout);
                return false;
            } else if (strcmp(el->type, "canvas") == 0 && strcmp(el->id, "emoji_palette") == 0 && isdigit(ch)) {
                char cmd[32];
                snprintf(cmd, sizeof(cmd), "PALETTE_MOVE:%d", ch - '0');
...
int find_next_interactive_element(int start_index) {
    for (int i = start_index + 1; i < element_count; i++) {
        if (strcmp(elements[i].type, "link") == 0 || 
            ...
            strcmp(elements[i].type, "cli_io") == 0) {
            return i;
        }
    }
