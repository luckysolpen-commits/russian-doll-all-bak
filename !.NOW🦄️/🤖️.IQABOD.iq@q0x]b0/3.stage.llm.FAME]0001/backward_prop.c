...
int main(int argc, char *argv[]) {
    ...
    // Save attention gradients in the same format as optimizer expects (same as save_attention format)
    sprintf(pth,"%s/grad_attn.txt",od);
    FILE *attn_f = fopen(pth, "w");
    if(attn_f) {
        // Save W_q gradients
        for(int i=0; i<EMBEDDING_DIM; i++) for(int j=0; j<EMBEDDING_DIM; j++) fprintf(attn_f, "%f ", g_a.W_q[i][j]);
        fprintf(attn_f, "\n");
        // Save W_k gradients
        for(int i=0; i<EMBEDDING_DIM; i++) for(int j=0; j<EMBEDDING_DIM; j++) fprintf(attn_f, "%f ", g_a.W_k[i][j]);
        fprintf(attn_f, "\n");
        // Save W_v gradients
        for(int i=0; i<EMBEDDING_DIM; i++) for(int j=0; j<EMBEDDING_DIM; j++) fprintf(attn_f, "%f ", g_a.W_v[i][j]);
        fprintf(attn_f, "\n");
        fclose(attn_f);
        fprintf(stderr, "Saved attention gradients to %s\n", pth);
    }
    
    // Save MLP gradients in the same format as optimizer expects (same as save_mlp format)
    sprintf(pth,"%s/grad_mlp.txt",od);
    FILE *mlp_f = fopen(pth, "w");
    if(mlp_f) {
        // Save weights
        for(int i=0; i<EMBEDDING_DIM; i++) for(int j=0; j<HIDDEN_DIM; j++) fprintf(mlp_f, "%f ", g_m.weights[i][j]);
        fprintf(mlp_f, "\n");
        // Save biases
        for(int i=0; i<HIDDEN_DIM; i++) fprintf(mlp_f, "%f ", g_m.biases[i]);
        fprintf(mlp_f, "\n");
        fclose(mlp_f);
        fprintf(stderr, "Saved MLP gradients to %s\n", pth);
    }
    
    // Save output gradients in the same format as optimizer expects (same as save_output format)
    sprintf(pth,"%s/grad_output.txt",od);
    FILE *out_f = fopen(pth, "w");
    if(out_f) {
        // Save output weights
        for(int i=0; i<HIDDEN_DIM; i++) {
            for(int j=0; j<vs; j++) {
                fprintf(out_f, "%f ", g_o.weights[i][j]);
            }
            fprintf(out_f, "\n");
        }
        // Save output biases
        for(int i=0; i<vs; i++) {
            fprintf(out_f, "%f ", g_o.biases[i]);
        }
        fprintf(out_f, "\n");
        fclose(out_f);
        fprintf(stderr, "Saved output gradients to %s\n", pth);
    }
