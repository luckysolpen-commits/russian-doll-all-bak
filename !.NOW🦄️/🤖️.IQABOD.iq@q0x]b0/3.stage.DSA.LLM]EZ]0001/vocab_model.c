...
// Function to generate hash-based deterministic embedding from word
float generate_embedding_from_word(const char *word) {
    ...
    return (float)(hash % 1000000) / 1000000.0f;
}
...
int add_word(struct VocabEntry *vocab, int vocab_size, const char *word) {
    ...
    for (int i = 0; i < vocab_size; i++) {
        if (strcmp(vocab[i].word, word) == 0) {
            vocab[i].count++;
            return vocab_size; // Return same size since we're just updating count
        }
    }
    
    vocab[vocab_size].count = 1;
    // Generate deterministic embedding based on the word content
    vocab[vocab_size].embedding = generate_embedding_from_word(word);
    vocab[vocab_size].pe = 0.0f;
    // Generate deterministic weight based on word content
    vocab[vocab_size].weight = generate_embedding_from_word(word) * 1.23f; // Use different multiplier
...
int main(int argc, char *argv[]) {
    ...
    // Calculate positional encoding after sorting to maintain consistent ordering
    // First, sort the vocabulary by word to ensure deterministic order
    for (int i = 0; i < vocab_size - 1; i++) {
        for (int j = i + 1; j < vocab_size; j++) {
            if (strcmp(vocab[i].word, vocab[j].word) > 0) {
                // Swap entries
                struct VocabEntry temp = vocab[i];
                vocab[i] = vocab[j];
                vocab[j] = temp;
            }
        }
    ...
    // Recalculate positional encoding based on sorted order
    for (int i = 0; i < vocab_size; i++) {
        vocab[i].pe = (float)i / (float)vocab_size;
        // Also update the embedding based on the sorted position for consistency
        if (i > 0) { // Keep first embedding based on word content
            vocab[i].embedding = generate_embedding_from_word(vocab[i].word) + (float)i * 0.001f; // Add position factor
        }
    }
