...
// --- RoPE Implementation for backward pass ---
// Function to compute rotary positional embedding (same as in forward_prop.c)
...
// Function to compute inverse rotary positional embedding (to reverse the RoPE transformation for gradients)
void apply_inv_rope(float *x, int dim, int pos, float base) {
    if (dim % 2 != 0) {
        fprintf(stderr, "RoPE requires even dimension size, got %d\n", dim);
        return;
    }
...
int main(int argc, char *argv[]) {
    ...
    for(int i=0;i<vs;i++) {
        for(int j=0;j<EMBEDDING_DIM;j++) {
            // Use vocabulary values for gradient computation - using RoPE-enhanced pe values
            float vocab_val = (j == 1) ? v[i].pe : ((j == 0) ? v[i].embedding : 
                           (j == 2) ? v[i].weight : 
                           (j == 3) ? v[i].bias1 : 
                           (j == 4) ? v[i].bias2 : 
                           (j == 5) ? v[i].bias3 : v[i].bias4);
                           
            if (vocab_val != vocab_val || g_as[i] != g_as[i] || 
                vocab_val > 1e10f || vocab_val < -1e10f || 
                g_as[i] > 1e10f || g_as[i] < -1e10f) {
                continue;  // Skip invalid values
            }
            g_q[j] += vocab_val * g_as[i];
        }
    }
    
    for(int i=0;i<EMBEDDING_DIM;i++) {
        for(int j=0;j<EMBEDDING_DIM;j++) {
            // Compute gradients for W_q weights using RoPE-enhanced vocabulary values
            float input_val = (j == 1) ? v[wi].pe : ((j == 0) ? v[wi].embedding : 
                            (j == 2) ? v[wi].weight : 
                            (j == 3) ? v[wi].bias1 : 
                            (j == 4) ? v[wi].bias2 : 
                            (j == 5) ? v[wi].bias3 : v[wi].bias4);
                            
            if (g_q[j] != g_q[j] || input_val != input_val || 
                g_q[j] > 1e10f || g_q[j] < -1e10f || 
                input_val > 1e10f || input_val < -1e10f) {
                g_a.W_q[i][j] = 0;
            } else {
                g_a.W_q[i][j] = g_q[j] * input_val;
            }
        }
    }
