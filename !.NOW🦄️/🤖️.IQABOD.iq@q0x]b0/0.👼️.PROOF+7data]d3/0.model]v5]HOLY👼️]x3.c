...
// Define structures to hold previous epoch values for comparison
typedef struct {
    float pe_prev;
    float weight1_prev;
    float weight2_prev;
...
void export_readable_model(char** vocab, int vocab_size, float* word_emb, float* attn_weights, float* ff_weights1, float* ff_weights2, float* biases, const char* subject_name) {
    // Create the directory path for the subject
    char folder[100];
    snprintf(folder, 100, "curricula/%s", subject_name);
    
    // Create the file path for the readable model export
    char filepath[200];
    snprintf(filepath, 200, "%s/%s.txt", folder, subject_name);
    ...
    // Print header with more descriptive column names
    fprintf(f, "word_index word embedding1 pe_weight1 weight1 weight2 bias1 bias2 bias3 bias4\n");
    
    // For each word in the vocabulary, export its embedding and learned parameters with sequential context
    for (int i = 0; i < vocab_size; i++) {
        // Apply ROPE to this word's embedding to capture the actual ROPE transformation
        // For the first dimension of this word's embedding at position 0
        float theta = 10000.0f / powf(10000.0f, 0.0f / EMBED_DIM); // For dimension 0
        float angle = 0 * theta; // Position 0
        float word_emb_0 = word_emb[i * EMBED_DIM]; // First dimension of word's embedding
        float word_emb_1 = (EMBED_DIM > 1) ? word_emb[i * EMBED_DIM + 1] : 0.0f; // Second dimension if available
        
        // Apply ROPE transformation: for dimensions (j, j+1), apply rotation
        // First dimension after ROPE: emb[j]*cos - emb[j+1]*sin
        float rope_emb_0 = word_emb_0 * cosf(angle) - word_emb_1 * sinf(angle);
        
        // Calculate attention-based context by simulating how this word would interact 
        // with other words in the vocabulary (using attention weights)
        float attn_context = 0.0f;
        // Use the attention weights to see how this word would attend to others
        // This captures learned sequential relationships
        for (int j = 0; j < EMBED_DIM; j++) {
            // Use Q weights from attention (first EMBED_DIM * EMBED_DIM values)
            attn_context += attn_weights[j * EMBED_DIM + 0]; // Using first column as example
        }
        
        // Apply attention context to the ROPE-transformed value 
        float pe_weight1 = rope_emb_0 * (1.0f + attn_context); // Context-modified PE value
        
        // Extract the first embedding value (learned through backpropagation)
        float embedding1 = word_emb[i * EMBED_DIM];
        
        // Calculate weight1: concatenate all backprop weights into 1 number, multiplied by PE and embedding
        float weight1 = 0.0f;
        // Sum all FF1 weights (learned through backpropagation)
        for (int j = 0; j < EMBED_DIM; j++) {
            for (int k = 0; k < FF_DIM; k++) {
                weight1 += ff_weights1[j * FF_DIM + k];
            }
        }
        // Incorporate attention context in the weight calculation
        weight1 = weight1 * embedding1 * pe_weight1 * (1.0f + attn_context);
        
        // Calculate weight2: concatenate all computer-used biases into 1 number, multiplied by PE and embedding
        float weight2 = 0.0f;
        // Sum all bias values (the computer-learned biases from training)
        for (int j = 0; j < EMBED_DIM; j++) {
            weight2 += biases[j];
        }
        // Incorporate attention context in the bias calculation
        weight2 = weight2 * embedding1 * pe_weight1 * (1.0f + attn_context);
        
        // Bias columns should be 0.0 for human tweaking
        float bias1 = 0.0, bias2 = 0.0, bias3 = 0.0, bias4 = 0.0;
        
        // Write the row for this word
        fprintf(f, "%d %s %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f\n", 
                i + 1,           // word_index (starting from 1)
                vocab[i],        // word
                embedding1,      // embedding1 - learned through backpropagation during training
                pe_weight1,      // pe_weight1 - ROPE-transformed value with attention context
                weight1,         // weight1 - concatenated backprop weights * embedding * ROPE * attention context
                weight2,         // weight2 - concatenated computer biases * embedding * ROPE * attention context
                bias1,           // bias1 - empty for human editing
                bias2,           // bias2 - empty for human editing
                bias3,           // bias3 - empty for human editing
                bias4            // bias4 - empty for human editing
        );
    }
    
    fclose(f);
    printf("Exported readable model to %s\n", filepath);
}

// Add debug tracking function for epoch-by-epoch changes
void debug_epoch_changes(char** vocab, int vocab_size, float* word_emb, float* attn_weights, float* ff_weights1, float* ff_weights2, float* biases, const char* subject_name, int epoch, DebugValues* prev_values) {
    // Create the directory path for the subject
    char folder[100];
    snprintf(folder, 100, "curricula/%s", subject_name);
    
    // Create the file path for the debug output
    char filepath[200];
    snprintf(filepath, 200, "%s/%s_debug.txt", folder, subject_name);
    
    FILE* f;
    int is_header_written = 0;
    ...
    f = fopen(filepath, "a");
    if (!f) {
        printf("Error opening %s for writing debug data\n", filepath);
        return;
    }
    
    // Write header if this is the first entry
    if (!is_header_written) {
        fprintf(f, "epoch word_index word pe_old pe_current pe_difference weight1_old weight1_current weight1_difference weight2_old weight2_current weight2_difference final_pe_change final_weight1_change final_weight2_change\n");
    }
    
    // For each word in the vocabulary, write debug information
    for (int i = 0; i < vocab_size; i++) {
        // Apply ROPE to this word's embedding to capture the actual ROPE transformation
        float theta = 10000.0f / powf(10000.0f, 0.0f / EMBED_DIM); // For dimension 0
        float angle = 0 * theta; // Position 0
        ...
        // Calculate attention-based context by simulating how this word would interact with other words
        float attn_context = 0.0f;
        for (int j = 0; j < EMBED_DIM; j++) {
            attn_context += attn_weights[j * EMBED_DIM + 0]; // Using first column as example
        }
        ...
        // Apply attention context to the ROPE-transformed value 
        float pe_current = rope_emb_0 * (1.0f + attn_context);
        
        // Calculate current weight1
        float weight1_current = 0.0f;
        for (int j = 0; j < EMBED_DIM; j++) {
            for (int k = 0; k < FF_DIM; k++) {
                weight1_current += ff_weights1[j * FF_DIM + k];
            }
        }
        weight1_current = weight1_current * word_emb[i * EMBED_DIM] * pe_current * (1.0f + attn_context);
        
        // Calculate current weight2
        float weight2_current = 0.0f;
        for (int j = 0; j < EMBED_DIM; j++) {
            weight2_current += biases[j];
        }
        weight2_current = weight2_current * word_emb[i * EMBED_DIM] * pe_current * (1.0f + attn_context);
        
        // Get previous values from stored data
        float pe_old = (epoch == 1) ? 0.0f : prev_values[i].pe_prev;
        float weight1_old = (epoch == 1) ? 0.0f : prev_values[i].weight1_prev;
        float weight2_old = (epoch == 1) ? 0.0f : prev_values[i].weight2_prev;
        
        float pe_difference = pe_current - pe_old;
        float weight1_difference = weight1_current - weight1_old;
        float weight2_difference = weight2_current - weight2_old;
        
        char final_pe_change = pe_difference >= 0 ? '+' : '-';
        char final_weight1_change = weight1_difference >= 0 ? '+' : '-';
        char final_weight2_change = weight2_difference >= 0 ? '+' : '-';
        
        fprintf(f, "%d %d %s %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %c %c %c\n",
                epoch,                    // epoch
                i + 1,                    // word_index
                vocab[i],                 // word
                pe_old,                   // pe_old 
                ...
                weight1_old,              // weight1_old
                weight1_current,          // weight1_current
                weight1_difference,       // weight1_difference
                weight2_old,              // weight2_old
                weight2_current,          // weight2_current
                weight2_difference,       // weight2_difference
                final_pe_change,          // final_pe_change
                final_weight1_change,     // final_weight1_change
                final_weight2_change      // final_weight2_change
        );
        
        // Store current values for next epoch comparison
        prev_values[i].pe_prev = pe_current;
        prev_values[i].weight1_prev = weight1_current;
        prev_values[i].weight2_prev = weight2_current;
    }
...
int main(int argc, char** argv) {
    ...
    for (int s = 0; s < NUM_SUBJECTS; s++) {
        ...
        // Allocate storage for debug values
        DebugValues* prev_values = malloc(vocab_size * sizeof(DebugValues));
        if (!prev_values) { printf("Malloc failed\n"); exit(1); }
        // Initialize previous values to zero
        for (int i = 0; i < vocab_size; i++) {
            prev_values[i].pe_prev = 0.0f;
            prev_values[i].weight1_prev = 0.0f;
            prev_values[i].weight2_prev = 0.0f;
        }
        ...
        for (int epoch = 0; epoch < EPOCHS; epoch++) {
            ...
            // Call debug function every 5 epochs
            if ((epoch + 1) % 5 == 0) {
                debug_epoch_changes(vocab, vocab_size, word_emb, attn_weights, ff_weights1, ff_weights2, biases, subjects[s], epoch + 1, prev_values);
            }
        }
        fclose(loss_f);
        ...
        // Export the model in human-readable format
        export_readable_model(vocab, vocab_size, word_emb, attn_weights, ff_weights1, ff_weights2, biases, subjects[s]);
