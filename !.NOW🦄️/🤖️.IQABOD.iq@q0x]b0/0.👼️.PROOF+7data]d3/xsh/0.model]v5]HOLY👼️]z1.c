...
void debug_epoch_changes(char** vocab, int vocab_size, float* word_emb, float* attn_weights, float* ff_weights1, float* ff_weights2, float* biases, const char* subject_name, int epoch) {
    // Create the directory path for the subject
    char folder[100];
    snprintf(folder, 100, "curricula/%s", subject_name);
    ...
    for (int i = 0; i < vocab_size; i++) {
        ...
        // For debugging purposes, assume old values are 0 for first epoch
        // In a real implementation, you'd store previous epoch values
        float pe_old = (epoch == 1) ? 0.0f : 0.0f; // Placeholder - in real implementation would track previous epoch
        float weight1_old = (epoch == 1) ? 0.0f : 0.0f; // Placeholder - in real implementation would track previous epoch
        float weight2_old = (epoch == 1) ? 0.0f : 0.0f; // Placeholder - in real implementation would track previous epoch
