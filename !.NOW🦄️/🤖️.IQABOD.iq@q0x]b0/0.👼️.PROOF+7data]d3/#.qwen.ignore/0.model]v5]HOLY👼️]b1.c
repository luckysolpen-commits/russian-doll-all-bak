...
// Function to export model data in the required format
void export_model_data(char** vocab, int vocab_size, float* word_emb, float* ff_weights1, float* ff_weights2, float* biases, const char* subject) {
    // Create the filename for the exported data
    char filepath[200];
    snprintf(filepath, 200, "curricula/%s/%s.txt", subject, subject);
    
    FILE* f = fopen(filepath, "w");
    if (!f) { 
        printf("Error opening %s for exporting model data\n", filepath); 
        return; 
    }
    
    // Calculate total number of weights and biases
    int total_ff_weights = (EMBED_DIM * FF_DIM) + (FF_DIM * EMBED_DIM); // ff_weights1 + ff_weights2
    int total_biases = EMBED_DIM;
    
    // Header indicating the structure
    fprintf(f, "# Format: number word embedding1 embedding2 ... embedding%d weight1 weight2 ... weight%d bias1 bias2 ... bias%d\n", 
            EMBED_DIM, total_ff_weights, total_biases);
    fprintf(f, "# For each word in vocabulary, export its hash embedding, feed-forward weights, and biases\n");
    
    // Export each word with its associated parameters
    for (int i = 0; i < vocab_size; i++) {
        fprintf(f, "%d %s ", i, vocab[i]);
        
        // Export hash embedding values
        for (int j = 0; j < EMBED_DIM; j++) {
            fprintf(f, "%.6f ", word_emb[i * EMBED_DIM + j]);
        }
        
        // Export feed-forward weights (ff_weights1 flattened) - all values
        for (int j = 0; j < EMBED_DIM * FF_DIM; j++) {
            fprintf(f, "%.6f ", ff_weights1[j]);
        }
        
        // Export feed-forward weights (ff_weights2 flattened) - all values  
        for (int j = 0; j < FF_DIM * EMBED_DIM; j++) {
            fprintf(f, "%.6f ", ff_weights2[j]);
        }
        
        // Export biases - all bias values
        for (int j = 0; j < EMBED_DIM; j++) {
            fprintf(f, "%.6f ", biases[j]);
        }
    ...
    fclose(f);
    printf("Exported model data to %s\n", filepath);
}
...
int main(int argc, char** argv) {
    ...
    for (int s = 0; s < NUM_SUBJECTS; s++) {
        ...
        // Export model data in the requested format
        export_model_data(vocab, vocab_size, word_emb, ff_weights1, ff_weights2, biases, subjects[s]);
