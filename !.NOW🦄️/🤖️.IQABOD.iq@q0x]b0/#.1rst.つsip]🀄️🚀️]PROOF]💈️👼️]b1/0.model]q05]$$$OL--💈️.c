...
typedef struct {
    float* amplitudes; // Now plain floats
    int n_qubits;
} QuantumState;
...
void qft(QuantumState* state) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = i + 1; j < n; j++) {
            ...
            for (int k = 0; k < (1 << n); k++) {
                if ((k & (1 << i)) && (k & (1 << j))) {
                    // Approximate complex rotation with amplitude scaling
                    state->amplitudes[k] *= cosf(angle); // Real part only
                }
            }
        }
    }
...
float train_hhl(float* word_emb, float* attn_weights, float* ff_weights1, float* ff_weights2, float* biases,
    ...
    QuantumState state = {malloc(state_size * sizeof(float)), n_qubits};
    ...
    for (int i = 0; i < state_size; i++) {
        float g = (i < vocab_size * EMBED_DIM) ? fabsf(gradients[i]) : 0.01f;
        eigenvalues[i] = 1.0f + g * 10.0f;
    }
    for (int i = 0; i < state_size; i++) {
    ...
    */
    ...
    float norm = 0;
    for (int i = 0; i < state_size; i++) norm += state.amplitudes[i] * state.amplitudes[i];
    norm = sqrtf(norm);
    if (norm == 0) norm = 1e-10;

    float old_weight = word_emb[0];
    for (int i = 0; i < vocab_size * EMBED_DIM; i++) {
     //   float update = LEARNING_RATE * state.amplitudes[i] * 100.0f / norm;
     float update = LEARNING_RATE * crealf(state.amplitudes[i]) * 32.0f / norm; //🪂️32
        word_emb[i] += update; // Flipped to += for descent
    }
    printf("Update[0]: %.6f, Old Weight[0]: %.6f, New Weight[0]: %.6f\n", 
           LEARNING_RATE * state.amplitudes[0] * 32.0f / norm, old_weight, word_emb[0]);

    free(attn_out); free(ff_out); free(logits); free(q); free(k); free(v); free(scores); free(hidden);
    free(d_logits); free(gradients); free(state.amplitudes); free(eigenvalues);
    return loss;
}
