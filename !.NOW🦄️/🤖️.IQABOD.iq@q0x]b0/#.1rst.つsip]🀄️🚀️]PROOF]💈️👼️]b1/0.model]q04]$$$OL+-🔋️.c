...
typedef struct {
    float complex* amplitudes;
    int n_qubits;
} QuantumState;

void hadamard(QuantumState* state, int qubit) {
    int n = 1 << state->n_qubits;
    for (int i = 0; i < n; i++) {
        if (!(i & (1 << qubit))) {
            float complex temp = state->amplitudes[i];
            state->amplitudes[i] += state->amplitudes[i | (1 << qubit)];
            state->amplitudes[i | (1 << qubit)] = temp - state->amplitudes[i | (1 << qubit)];
            state->amplitudes[i] /= sqrtf(2);
            state->amplitudes[i | (1 << qubit)] /= sqrtf(2);
        }
    }
}
...
void qft(QuantumState* state) {
    ...
    for (int i = 0; i < n; i++) {
        hadamard(state, i);
        for (int j = i + 1; j < n; j++) {
            float angle = 2 * M_PI / (1 << (j - i + 1));
            for (int k = 0; k < (1 << n); k++) {
                if ((k & (1 << i)) && (k & (1 << j))) {
                    state->amplitudes[k] *= cexpf(I * angle);
                }
            }
        }
    }
    for (int i = 0; i < n / 2; i++) {
        for (int j = 0; j < (1 << n); j++) {
            if ((j & (1 << i)) != (j & (1 << (n - 1 - i)))) {
                float complex temp = state->amplitudes[j];
                state->amplitudes[j] = state->amplitudes[j ^ (1 << i) ^ (1 << (n - 1 - i))];
                state->amplitudes[j ^ (1 << i) ^ (1 << (n - 1 - i))] = temp;
            }
        }
    }
}
...
//
float train_hhl(float* word_emb, float* attn_weights, float* ff_weights1, float* ff_weights2, float* biases,
    ...
    for (int i = 0; i < vocab_size; i++) {
        for (int d = 0; d < EMBED_DIM; d++) {
            gradients[i * EMBED_DIM + d] = d_logits[i] * ff_out[(seq_len - 1) * EMBED_DIM + d];
            if (gradients[i * EMBED_DIM + d] > CLIP_VALUE) gradients[i * EMBED_DIM + d] = CLIP_VALUE;
            if (gradients[i * EMBED_DIM + d] < -CLIP_VALUE) gradients[i * EMBED_DIM + d] = -CLIP_VALUE;
        }
    }
    ...
    // Option 1: Fallback with amplified updates
    /*
    float old_weight = word_emb[0];
    for (int i = 0; i < vocab_size * EMBED_DIM; i++) {
        word_emb[i] -= LEARNING_RATE * gradients[i] * 1000.0f; // Amplify by 1000x
    }
    printf("Update[0]: %.6f, Old Weight[0]: %.6f, New Weight[0]: %.6f\n", 
           LEARNING_RATE * gradients[0] * 1000.0f, old_weight, word_emb[0]);
*/
    // Uncomment for Option 2: Simplified HHL
    int n_qubits = ceil(log2(vocab_size * EMBED_DIM));
    int state_size = 1 << n_qubits;
    QuantumState state = {malloc(state_size * sizeof(float complex)), n_qubits};
    if (!state.amplitudes) { printf("Malloc failed\n"); exit(1); }

    for (int i = 0; i < state_size; i++) {
        state.amplitudes[i] = (i < vocab_size * EMBED_DIM) ? gradients[i] : 0;
    }

    for (int i = 0; i < n_qubits; i++) hadamard(&state, i);
    qft(&state);
    float* eigenvalues = malloc(state_size * sizeof(float));
    if (!eigenvalues) { printf("Malloc failed\n"); exit(1); }
    
    
    for (int i = 0; i < state_size; i++) eigenvalues[i] = 1.0 + 0.1 * (i + 1);
    for (int i = 0; i < state_size; i++) {
        if (eigenvalues[i] != 0) state.amplitudes[i] /= eigenvalues[i];
    }
    qft(&state);

    float norm = 0;
    for (int i = 0; i < state_size; i++) norm += cabsf(state.amplitudes[i]) * cabsf(state.amplitudes[i]);
    norm = sqrtf(norm);
    if (norm == 0) norm = 1e-10;

    float old_weight = word_emb[0];
    for (int i = 0; i < vocab_size * EMBED_DIM; i++) {
       // float update = LEARNING_RATE * crealf(state.amplitudes[i]) * 100.0f / norm; // Amplify HHL updates (too much tho?)
        float update = LEARNING_RATE * crealf(state.amplitudes[i]) * 32.0f / norm; //🪂️
      //  word_emb[i] -= update;
      word_emb[i] += update; //+- hac? This reverses the step, aligning it with minimizing loss instead of maximizing it. Takes 2 seconds to test.


    }
    printf("Update[0]: %.6f, Old Weight[0]: %.6f, New Weight[0]: %.6f\n", 
           LEARNING_RATE * crealf(state.amplitudes[0]) * 100.0f / norm, old_weight, word_emb[0]);
    free(state.amplitudes); free(eigenvalues);
    

    free(attn_out); free(ff_out); free(logits); free(q); free(k); free(v); free(scores); free(hidden);
    free(d_logits); free(gradients);
    return loss;
}
    ...
    for (int s = 0; s < NUM_SUBJECTS; s++) {
        ...
        for (int epoch = 0; epoch < EPOCHS; epoch++) {
            ...
            for (int i = 0; i < word_count - 1; i++) {
                ...
                total_loss += train_hhl(word_emb, attn_weights, ff_weights1, ff_weights2, biases, rope_emb, i + 1, targets, vocab_size);
                free(rope_emb);
                steps++;
            }
