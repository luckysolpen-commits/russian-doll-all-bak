...
#define LEARNING_RATE 0.1f  // Increased from 0.01
#define EPOCHS 200          // Increased from 50
...
int main() {
    ...
    for (int epoch = 0; epoch < EPOCHS; epoch++) {
        ...
        for (int s = 0; s < NUM_SUBJECTS; s++) {
            ...
            float norm = 0;
            for (int d = 0; d < EMBED_DIM; d++) norm += input[d] * input[d];
            norm = sqrtf(norm);
            if (norm == 0) norm = 1e-10;
            for (int d = 0; d < EMBED_DIM; d++) input[d] /= norm;  // Normalize input
            ...
            if (epoch % 10 == 0) {
                printf("Epoch %d, Subject %s weights: ", epoch, subjects[s]);
                for (int j = 0; j < NUM_SUBJECTS; j++) printf("%.4f ", weights[j]);
                printf("\n");
            }
        }
