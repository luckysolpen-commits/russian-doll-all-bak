...
// Forward declarations for image functionality
void preload_image_textures();
void cleanup_textures();
GLuint load_image_as_texture(const char* image_path, float* aspect_ratio);
...
void init_view(const char* filename) {
    ...
    // Preload all image textures after parsing is complete
    preload_image_textures();
...
int load_page(const char* filename) {
    ...
    // Preload image textures for the new page
    preload_image_textures();
...
int activate_page(const char* filename) {
    ...
    for (int i = 0; i < content_manager.num_pages_loaded; i++) {
        if (strcmp(content_manager.pages[i].filename, filename) == 0) {
            ...
            // Preload image textures for the activated page
            preload_image_textures();
...
void draw_element(UIElement* el) {
    ...
    if (strcmp(el->type, "canvas") == 0) {
        ...
        // Render image element with preloaded texture
        if (el->image_src[0] != '\0' && el->texture_loaded && el->texture_id != 0) {
            ...
            // Draw textured quad for the image using proper texture coordinates
            glColor4f(1.0f, 1.0f, 1.0f, el->alpha); // Use alpha from element
            glBegin(GL_QUADS);
            // Bottom-left of texture to bottom-left of quad
            glTexCoord2f(0.0f, 0.0f); glVertex2i(abs_x, abs_y);
            // Bottom-right of texture to bottom-right of quad  
            glTexCoord2f(1.0f, 0.0f); glVertex2i(abs_x + el->calculated_width, abs_y);
            // Top-right of texture to top-right of quad
            glTexCoord2f(1.0f, 1.0f); glVertex2i(abs_x + el->calculated_width, abs_y + el->calculated_height);
            // Top-left of texture to top-left of quad
            glTexCoord2f(0.0f, 1.0f); glVertex2i(abs_x, abs_y + el->calculated_height);
            glEnd();
            
            // Restore the previous OpenGL state
            glPopAttrib();
        } else {
            // If no image source provided or texture failed to load, draw a placeholder rectangle
...
void cleanup_view() {
    // Clean up texture resources first to prevent memory leaks
    cleanup_textures();
...
int parse_inline_chtml(const char* chtml_content, int parent_element_index) {
    ...
    // Preload image textures for the new elements
    // Only preload for the newly added elements
    for (int i = original_num_elements; i < num_elements; i++) {
        if ((strcmp(elements[i].type, "img") == 0 || strcmp(elements[i].type, "image") == 0) && 
            ...
            float aspect_ratio = 1.0f; // Initialize aspect ratio variable
            GLuint texture_id = load_image_as_texture(elements[i].image_src, &aspect_ratio);
            elements[i].texture_id = texture_id;
            elements[i].texture_loaded = (texture_id != 0);
            elements[i].image_aspect_ratio = aspect_ratio;
        }
    }
...
GLuint load_image_as_texture(const char* image_path, float* aspect_ratio) {
    ...
    if (data) {
        ...
        if (height > 0) {
            ...
            *aspect_ratio = 1.0f; // Default to 1:1 if height is 0
        }
        
        // Use proper function pointer approach as shown in the example
        // But since we're using GLUT, we need to load GL function pointers manually if needed
        // For now, using standard OpenGL functions which should be available
        ...
        // Note: glGenerateMipmap may not be available without extensions, so comment out if needed
        // glGenerateMipmap(GL_TEXTURE_2D);
        
        stbi_image_free(data);
        printf("Texture loaded successfully from: %s (ID: %u, %dx%d, aspect ratio: %.2f, channels: %d)\n", 
               image_path, textureID, width, height, *aspect_ratio, nrChannels);
    } else {
        printf("Failed to load texture: %s - Error: %s\n", image_path, stbi_failure_reason());
        glBindTexture(GL_TEXTURE_2D, textureID);
        // Create a simple 1x1 red texture as fallback
        unsigned char fallback_data[] = {255, 0, 0}; // Red pixel
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 1, 1, 0, GL_RGB, GL_UNSIGNED_BYTE, fallback_data);
        *aspect_ratio = 1.0f; // Default aspect ratio for fallback
...
// Function to preload all image textures after parsing is complete
void preload_image_textures() {
    printf("Preloading %d image textures...\n", num_elements);
    ...
    for (int i = 0; i < num_elements; i++) {
        if ((strcmp(elements[i].type, "img") == 0 || strcmp(elements[i].type, "image") == 0) && 
            ...
            // Store the texture ID in the element
            elements[i].texture_id = texture_id;
            elements[i].texture_loaded = (texture_id != 0);
            elements[i].image_aspect_ratio = aspect_ratio;
            
            printf("Image '%s' loaded with texture ID: %u, loaded: %s, aspect ratio: %.2f\n", 
                   elements[i].image_src, texture_id, elements[i].texture_loaded ? "true" : "false", aspect_ratio);
        }
    }
    
    printf("Completed preloading image textures.\n");
}

// Function to cleanup texture resources
void cleanup_textures() {
