...
int interaction_mode_element = -1; // Index of canvas in interaction mode (-1 if none)
        ...
        if (strcmp(el->type, "text") == 0) {
            ...
            if (el->width > 0 && el->height > 0) {
                ...
                if (el->label[0] != '\0') {
                    ...
                    if (el == &elements[interaction_mode_element]) {
                        // Element is in interaction mode - show with "^"
                        if (el->key > 0) {
                            printf("[^] %d. %s\n", el->key, processed_label);  // Show interaction mode indicator
                        } else {
                            printf("[^] %s\n", processed_label);  // Show interaction mode indicator without number
                        }
                    } else if (el->focused) {
                        // Element is just selected (navigation) - show with ">"
                        if (el->key > 0) {
                            printf("[>] %d. %s\n", el->key, processed_label);  // Show selected indicator
                        } else {
                            printf("[>] %s\n", processed_label);  // Show selected indicator without number
                        }
                    } else {
                        // Element is not focused - show with regular brackets
                        if (el->key > 0) {
                            printf("[ ] %d. %s\n", el->key, processed_label);  // Show unselected
                        } else {
                            printf("[ ] %s\n", processed_label);  // Show unselected without number
                        }
                    }
                }
                ...
                if (ipc.active) {
                    // Send commands based on the canvas type
                    if (strcmp(el->id, "emoji_palette") == 0) {
                        send_ipc_command("DISPLAY_PALETTE");
                    } else {
                    ...
                    while (total_lines < (expected_lines + 5)) { // canvas height (1 for palette, height for game canvas) + some extra for potential position data
                        if (read_ipc_response(response, sizeof(response)) == 0) {
                            ...
                            } else if (strcmp(el->id, "emoji_palette") == 0) {
                                // Special handling for emoji palette - just print the line directly
                                printf("%s\n", response);
                                lines_read++;
...
bool process_input(int ch) {
    ...
    // Handle Enter key - if input buffer has content, check if it's a single digit for menu selection, otherwise evaluate
        ...
        if (input_buffer_pos > 0) {
            ...
            if (input_buffer_pos == 1 && isdigit(input_buffer[0])) {
                ...
                for (int i = 0; i < element_count; i++) {
                    if (elements[i].key == selected_key) {
                        // Set focus to the element being activated
                        set_focus(i);
                ...
                // This could be a file path if we're in error state with no interactive elements
                if (element_count == 2 && // Only has error messages (no interactive elements)
                    strcmp(elements[0].type, "text") == 0 && 
                    strstr(elements[0].label, "Error: Could not find file") != NULL) {
                    // User may be entering a file path to load
                    printf("\nTrying to load file: %s\n", input_buffer);
                    switch_to_file(input_buffer);
                    clear_input_buffer();
                    return true; // Redraw with new file
                } else {
                    printf("\nProcessed input: %s\n", input_buffer); // Process the input buffer
                    clear_input_buffer();
                    return true; // Redraw needed to show status message
            ...
            // BUT if there's a focused canvas element, enter should "activate" it for detailed interaction
            if (focused_element_index >= 0 && 
                strcmp(elements[focused_element_index].type, "canvas") == 0) {
                // Activate interaction mode for the focused canvas
                interaction_mode_element = focused_element_index;
                clear_input_buffer(); // Clear the buffer to ensure clean state
                printf("\rCanvas activated for detailed control. Use arrows to control canvas content, ESC to deactivate.\n");
                printf("Enter key (1-9) or 'q' to quit: ");
                fflush(stdout);
                return true; // Redraw to update display with focus indicator
            } else {
                clear_input_buffer(); // Clear the buffer after processing
                printf("\n");
                fflush(stdout);
    ...
    // Manually echo printable characters (letters, numbers, symbols) except special keys
        ...
        if (isdigit(ch)) {
            ...
            if (!found) {
                // Check if the focused element is the palette canvas in interaction mode and we're pressing 0-7
                if (focused_element_index >= 0 && 
                    interaction_mode_element == focused_element_index && // Check if in interaction mode
                    strcmp(elements[focused_element_index].type, "canvas") == 0 &&
                    strcmp(elements[focused_element_index].id, "emoji_palette") == 0 &&
                    ipc.active &&
                    selected_key >= 0 && selected_key <= 7) {
                    // Send command to select palette item
                    char cmd[32];
                    snprintf(cmd, sizeof(cmd), "PALETTE_MOVE:%d", selected_key);
                    send_ipc_command(cmd);
                    printf(" Palette selection %d chosen", selected_key);
                    fflush(stdout);
        ...
        switch (ch) {
            case ARROW_UP: // Up arrow
                // Check if the currently focused canvas is in interaction mode
                if (interaction_mode_element >= 0 && 
                    interaction_mode_element == focused_element_index &&
                    strcmp(elements[interaction_mode_element].type, "canvas") == 0 &&
                    ipc.active) {
                    // Canvas is in interaction mode - handle canvas-specific controls
                    if (strcmp(elements[interaction_mode_element].id, "emoji_palette") == 0) {
                        // For palette in interaction mode, up arrow might do something else or be ignored
                        // For now, navigate to previous element
                        int new_focus = find_prev_interactive_element(focused_element_index);
                        if (new_focus != -1) {
                            set_focus(new_focus);
                            printf("\rFocus moved to previous element\n");
                            printf("Enter key (1-9) or 'q' to quit: ");
                            fflush(stdout);
                            return true; // Redraw to show new focus
                        } else {
                            printf("\rNo previous interactive element\n");
                            printf("Enter key (1-9) or 'q' to quit: ");
                            fflush(stdout);
                            return true;
                        }
                    } else {
                        // It's the game canvas in interaction mode, move the player
                    ...
                    // Interaction mode is off - navigate between elements
                    int new_focus = find_prev_interactive_element(focused_element_index);
                    if (new_focus != -1) {
                        set_focus(new_focus);
                        printf("\rFocus moved to previous element\n");
            ...
            case ARROW_DOWN: // Down arrow
                ...
                if (interaction_mode_element >= 0 && 
                    ...
                    // Canvas is in interaction mode - handle canvas-specific controls
                    if (strcmp(elements[interaction_mode_element].id, "emoji_palette") == 0) {
                        // For palette in interaction mode, down arrow might do something else or be ignored
                        // For now, navigate to next element
                        int new_focus = find_next_interactive_element(focused_element_index);
                        if (new_focus != -1) {
                            set_focus(new_focus);
                            printf("\rFocus moved to next element\n");
                            printf("Enter key (1-9) or 'q' to quit: ");
                            fflush(stdout);
                            return true; // Redraw to show new focus
                        } else {
                            printf("\rNo next interactive element\n");
                            printf("Enter key (1-9) or 'q' to quit: ");
                            fflush(stdout);
                            return true;
                        }
            ...
            case ARROW_RIGHT: // Right arrow
                ...
                if (interaction_mode_element >= 0 && 
                    ...
                    // Canvas is in interaction mode - handle canvas-specific controls
                    if (strcmp(elements[interaction_mode_element].id, "emoji_palette") == 0) {
                        send_ipc_command("PALETTE_MOVE:R");
                        printf("\rRight arrow pressed (palette selection moved right)\n");
                        printf("Enter key (1-9) or 'q' to quit: ");
                        fflush(stdout);
                        return true; // Redraw to update canvas
                    } else {
                    ...
                    // Interaction mode is off, or different element focused - navigate between elements
                    int new_focus = find_next_interactive_element(focused_element_index);
                    if (new_focus != -1) {
                        set_focus(new_focus);
                        printf("\rFocus moved to next element\n");
            ...
            case ARROW_LEFT: // Left arrow
                ...
                if (interaction_mode_element >= 0 && 
                    ...
                    // Canvas is in interaction mode - handle canvas-specific controls
                    if (strcmp(elements[interaction_mode_element].id, "emoji_palette") == 0) {
                        send_ipc_command("PALETTE_MOVE:L");
                        printf("\rLeft arrow pressed (palette selection moved left)\n");
                        printf("Enter key (1-9) or 'q' to quit: ");
                        fflush(stdout);
                        return true; // Redraw to update canvas
                    } else {
            ...
            case ESC_KEY: // Escape key
                // If canvas is in interaction mode, exit interaction mode
                if (interaction_mode_element >= 0) {
                    // Exit interaction mode but keep the element visually focused
                    interaction_mode_element = -1;
                    clear_input_buffer(); // Clear the buffer to ensure clean state
                    const char* key_name = (ch == 4) ? "Ctrl+D" : (ch == 9) ? "Tab" : (ch == ESC_KEY) ? "ESC" : "Unknown";
                    printf("\rCanvas deactivated (%s), navigation mode active\n", key_name);
                    printf("Enter key (1-9) or 'q' to quit: ");
                    fflush(stdout);
                    return true; // Redraw to update display
                } else {
                    // If not in interaction mode, just show message
                    clear_input_buffer(); // Clear the buffer
                    const char* key_name2 = (ch == 4) ? "Ctrl+D" : (ch == 9) ? "Tab" : (ch == ESC_KEY) ? "ESC" : "Unknown";
                    printf("\r%s pressed (not in interaction mode)\n", key_name2);
                    printf("Enter key (1-9) or 'q' to quit: ");
                    fflush(stdout);
                }
                return true; // Redraw to reflect any changes
                break;
            default:
                if (!isprint(ch)) {
...
int find_next_interactive_element(int start_index) {
    for (int i = start_index + 1; i < element_count; i++) {
        if (strcmp(elements[i].type, "link") == 0 || 
            strcmp(elements[i].type, "button") == 0 ||
            strcmp(elements[i].type, "canvas") == 0) {
            return i;
        }
    }
