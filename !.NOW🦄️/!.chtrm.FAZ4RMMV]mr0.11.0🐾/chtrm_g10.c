...
// UI Element types
typedef struct {
    char type[MAX_ATTR_LEN];    // "text", "link", "button", "panel", "canvas", "cli_io", "menu"
    ...
    // For parent-child relationships (especially for menu elements)
    int parent_index;           // Index of parent element (-1 if no parent)
    int children[MAX_CHILDREN]; // Indices of child elements
    int num_children;           // Number of child elements
    int current_child_focus;    // Index of currently focused child (for active menus)
    bool visibility;            // Whether children are always visible (1) or only when menu is active (0)
} UIElement;
...
int execute_on_next_enter = -1; // Index of element that should execute on next Enter (-1 if none)
char current_file[MAX_PATH_LEN];
FILE* log_file = NULL;
FILE* debug_file = NULL;  // Debug log file
IPCState ipc = {0};  // Initialize all fields to 0

// Stack for tracking element hierarchy during parsing (for nested elements like menus)
int element_stack[ELEMENT_STACK_SIZE];
int stack_top = -1;  // Points to top element in stack, -1 if empty
...
// Token types for the hierarchical parser
typedef enum {
    TOKEN_TEXT,
    TOKEN_OPEN_TAG,  
    TOKEN_CLOSE_TAG,
...
typedef struct {
    TokenType type;
    char content[512];  // For text content
    char tag_name[64];  // For tag names
    char attributes[256]; // For tag attributes
} Token;
...
Token* tokenize_chtm(const char* content, int* token_count);
...
// Tokenizer function for CHTM hierarchical parsing
Token* tokenize_chtm(const char* content, int* token_count) {
    if (!content) return NULL;
    
    Token* tokens = malloc(MAX_ELEMENTS * sizeof(Token));
    ...
    *token_count = 0;
    ...
    while (cursor < content_end) {
        ...
        if (tag_start == NULL) {
            // No more tags, add remaining text content if any
            if (*cursor != '\0') {
                // Add text token
                if (*token_count < MAX_ELEMENTS) {
                    Token* token = &tokens[*token_count];
                    token->type = TOKEN_TEXT;
                    
                    // Copy the remaining text content
                    int text_len = content_end - cursor;
                    if (text_len > 511) text_len = 511; // Limit to buffer size
                    
                    strncpy(token->content, cursor, text_len);
                    token->content[text_len] = '\0';
                    
                    // Trim leading/trailing whitespace
                    char* start = token->content;
        ...
        if (tag_end == NULL) break; // Malformed content
        
        // Extract tag content (without < and >)
        int tag_len = tag_end - tag_start - 1;
        char tag_content[512];
        if (tag_len >= 512) tag_len = 511; // Limit to buffer size
        strncpy(tag_content, tag_start + 1, tag_len);
        ...
        if (tag_content[0] == '/') {
            ...
            if (*token_count < MAX_ELEMENTS) {
                ...
                char* name_start = tag_content + 1; // Skip '/'
                char* name_end = strchr(name_start, ' ');
                if (name_end != NULL) {
                    int name_len = name_end - name_start;
                    if (name_len > 63) name_len = 63;
                    strncpy(token->tag_name, name_start, name_len);
                    token->tag_name[name_len] = '\0';
                } else {
                    strncpy(token->tag_name, name_start, 63);
                    token->tag_name[63] = '\0';
                }
                
                // Attributes are not used in closing tags, but we set to empty
                token->attributes[0] = '\0';
                
                (*token_count)++;
            }
        } else if (tag_content[strlen(tag_content)-1] == '/') {
            // Self-closing tag
            if (*token_count < MAX_ELEMENTS) {
                Token* token = &tokens[*token_count];
                token->type = TOKEN_SELFCLOSE_TAG;
                
                // Extract tag name and attributes
                char* space_pos = strchr(tag_content, ' ');
                if (space_pos) {
                    int name_len = space_pos - tag_content;
                    if (name_len > 63) name_len = 63;
                    strncpy(token->tag_name, tag_content, name_len);
                    token->tag_name[name_len] = '\0';
                    
                    // Copy attributes
                    space_pos++; // Skip space
                    strncpy(token->attributes, space_pos, 255);
                    token->attributes[255] = '\0';
                } else {
                    // No attributes
                    strncpy(token->tag_name, tag_content, 63);
                    token->tag_name[63] = '\0';
                    token->attributes[0] = '\0';
                }
                
                // Remove the trailing '/' from tag name if present
                int tag_name_len = strlen(token->tag_name);
                if (tag_name_len > 0 && token->tag_name[tag_name_len-1] == '/') {
                    token->tag_name[tag_name_len-1] = '\0';
                }
    ...
    // Resize the tokens array to the exact size needed
    if (*token_count > 0) {
        Token* resized_tokens = realloc(tokens, (*token_count) * sizeof(Token));
        if (resized_tokens) {
...
void parse_attributes(UIElement* el, const char* attr_str) {
    ...
    while (*pos) {
        ...
        *pos = '\0';
        ...
        } else if (strcmp(name_start, "visibility") == 0) {
            el->visibility = (atoi(value_start) != 0);  // 1 if true/non-zero, 0 if false/zero
        }
...
// Parse CHTM content with stack-based tracking for nested elements
void parse_chtm_file(const char* content) {
    ...
    // Stack to track nested elements (for parent-child relationships)
    ...
    for (int i = 0; i < MAX_ELEMENTS; i++) {
        ...
        elements[i].parent_index = -1;  // Initialize parent to -1 (no parent)
        elements[i].num_children = 0;   // Initialize children count to 0
        elements[i].current_child_focus = -1; // Initialize current child focus to -1 (no focus)
        // Children array is already zeroed by memset
    }
    ...
    while (*cursor) {
        ...
        if (tag_start > cursor) {
            ...
            if (len < 512) {
                ...
                if (*trimmed) {
                    // If we're inside a button/link/text element that was just created, add this as its label
                    if (stack_top >= 0) {
                        int parent_idx = element_stack[stack_top];
                        UIElement* parent = &elements[parent_idx];
                        
                        // For button, link, and text elements, assign text as label
                        if (strcmp(parent->type, "button") == 0 || 
                            strcmp(parent->type, "link") == 0 ||
                            strcmp(parent->type, "text") == 0) {
                            // Only set label if it's currently empty (first text content encountered)
                            if (parent->label[0] == '\0') {
                                strncpy(parent->label, trimmed, MAX_LABEL_LEN - 1);
                            } else {
                                // If label already exists, append to it with space separator
                                int current_len = strlen(parent->label);
                                int remaining_space = MAX_LABEL_LEN - 1 - current_len;
                                if (remaining_space > 1) {
                                    strncat(parent->label, " ", remaining_space - 1);
                                    strncat(parent->label, trimmed, remaining_space - 2);
                                }
                            }
                        } else {
                            // For other element types, create a new text element as children
                            if (element_count < MAX_ELEMENTS) {
                                ...
                                elements[element_count].parent_index = parent_idx;  // Set parent
                                
                                // Add to parent's children list
                                if (elements[parent_idx].num_children < MAX_CHILDREN) {
                                    elements[parent_idx].children[elements[parent_idx].num_children] = element_count;
                                    elements[parent_idx].num_children++;
                                }
                        ...
                        // If no parent element (top level), create as standalone text element
                        if (element_count < MAX_ELEMENTS) {
                            elements[element_count].key = -1;
                            strcpy(elements[element_count].type, "text");
                            strncpy(elements[element_count].label, trimmed, MAX_LABEL_LEN - 1);
                            elements[element_count].parent_index = -1;  // No parent
                            element_count++;
                        }
                    }
                }
        ...
        // Handle closing tags first (tags that start with /)
        if (tag[0] == '/') {
            char* closing_tag_name = tag + 1;
            
            // Pop from stack for closing tags that had elements pushed
            if (strcmp(closing_tag_name, "menu") == 0) {
                if (stack_top >= 0) {
                    stack_top--; // Pop menu from stack
                }
            }
            else if (strcmp(closing_tag_name, "button") == 0 || 
                     strcmp(closing_tag_name, "link") == 0 ||
                     strcmp(closing_tag_name, "text") == 0) {
                if (stack_top >= 0) {
                    stack_top--; // Pop from stack
                }
            }
            else if (strcmp(closing_tag_name, "panel") == 0) {
                if (stack_top >= 0 && element_stack[stack_top] == -1) {
                    stack_top--; // Pop panel marker from stack
                }
            }
        ...
        // Check if self-closing (only if not a closing tag)
        ...
        // Process tag based on stack-based element hierarchy
        if (strcmp(tag_name, "menu") == 0) {
            if (element_count < MAX_ELEMENTS) {
                elements[element_count].key = -1;
                strcpy(elements[element_count].type, "menu");
                parse_attributes(&elements[element_count], space_pos);
                
                // Set parent based on current stack top
                elements[element_count].parent_index = (stack_top >= 0) ? element_stack[stack_top] : -1;
                
                // Establish parent-child relationship if there's a parent
                if (elements[element_count].parent_index != -1 && elements[element_count].parent_index < MAX_ELEMENTS) {
                    UIElement* parent = &elements[elements[element_count].parent_index];
                ...
                // Push this menu onto the stack for containment tracking
            ...
            if (element_count < MAX_ELEMENTS) {
                ...
                // Set parent based on current stack top - this is the CRITICAL part
                elements[element_count].parent_index = (stack_top >= 0) ? element_stack[stack_top] : -1;
                ...
                // Push this button element onto the stack for content processing if not self-closing
                if (!self_closing && stack_top < STACK_SIZE - 1) {
                    stack_top++;
                    element_stack[stack_top] = element_count;
                }
            ...
            if (element_count < MAX_ELEMENTS) {
                ...
                // Push this link element onto the stack for content processing if not self-closing
                if (!self_closing && stack_top < STACK_SIZE - 1) {
                    stack_top++;
                    element_stack[stack_top] = element_count;
                }
            ...
            if (element_count < MAX_ELEMENTS) {
                ...
                // Push this text element onto the stack for content processing if not self-closing
                if (!self_closing && stack_top < STACK_SIZE - 1) {
                    stack_top++;
                    element_stack[stack_top] = element_count;
                }
            ...
            if (element_count < MAX_ELEMENTS) {
                ...
                elements[element_count].parent_index = -1;  // br tags don't belong to parent containers
                element_count++;
            }
        }
            ...
            if (element_count < MAX_ELEMENTS) {
                elements[element_count].key = -1;
                strcpy(elements[element_count].type, "text");  // Treat as text container for now
                parse_attributes(&elements[element_count], space_pos);
            ...
            // Panel is a container marker, use -1 to mark it on stack
            if (stack_top < STACK_SIZE - 1) {
                stack_top++;
                element_stack[stack_top] = -1; // Use -1 to indicate panel container
            }
            cursor = tag_end + 1;
            continue;
        }
...
// Child elements are only visible when their parent is active
bool is_element_visible(UIElement* el) {
    if (el->parent_index != -1) {
        // This is a child element - check if parent is active
        UIElement* parent = &elements[el->parent_index];
        if (strcmp(parent->type, "menu") == 0) {
            // For menu items, check if parent menu has visibility attribute set
            // If visibility is true (1), children are always visible; otherwise only when menu is active
            if (parent->visibility) {
                return true;  // Children always visible
            } else {
                // For menu items, they're only visible when the menu is active
                return (active_interaction_element == el->parent_index);
            }
        } else {
            // For other types of parent elements, especially those that might serve as containers
            // (like text elements used with innerHTML), make them visible if the parent would normally be visible
            // Since non-menu parents aren't typically active, we need a different approach
            
            // Check if this child element was added dynamically through innerHTML
            // For such elements, we want them to be visible if they're in the main flow
            // This would be the case for child elements of containers that serve as dynamic content areas
            if (parent->num_children > 0) {
                // If the parent has children (like from innerHTML), make them visible 
                // unless there's a specific reason not to
                return true; // For container-like parent elements, show children
            } else {
                // For other non-menu parent elements, keep previous behavior
                return (active_interaction_element == el->parent_index);
            }
        }
    } else {
        // Top-level elements are always visible
        return true;
    }
}
...
bool is_interactive_element(UIElement* el) {
    // If this element has a parent, it's a child element
    if (el->parent_index != -1) {
        UIElement* parent = &elements[el->parent_index];
        // Child elements are only interactive when their parent menu is active
        if (strcmp(parent->type, "menu") == 0 && active_interaction_element == el->parent_index) {
            return true;  // Child elements are interactive when parent menu is active
        } else {
            return false;  // Child elements are not interactive otherwise
        }
    }
    
    // Top-level elements (those without parents) are interactive if they're one of these types
    return (strcmp(el->type, "link") == 0 || 
            strcmp(el->type, "button") == 0 ||
            strcmp(el->type, "canvas") == 0 ||
            strcmp(el->type, "cli_io") == 0 ||
            strcmp(el->type, "menu") == 0);
}
...
void render_elements() {
    ...
    for (int i = 0; i < element_count; i++) {
        ...
        // Only render elements that are visible in the current context
        if (!is_element_visible(el)) {
            continue;
        }
        
        // Don't render child elements individually if their parent menu is active
        // because they will be rendered as part of the parent menu
        if (el->parent_index != -1) {
            UIElement* parent = &elements[el->parent_index];
            if (strcmp(parent->type, "menu") == 0 && active_interaction_element == el->parent_index) {
                continue; // Skip this child element as it will be rendered as part of the parent menu
            }
        }
        ...
        if (strcmp(el->type, "br") == 0) {
            ...
            if (el->key > 0 && el->label[0] != '\0') {
                ...
                if (is_interactive_element(el)) {
                    ...
                    if (i == active_interaction_element) {
                        ...
                        // Check if this element is a child of a menu with visibility="1"
                        if (el->parent_index != -1) {
                            UIElement* parent = &elements[el->parent_index];
                            if (strcmp(parent->type, "menu") == 0 && parent->visibility) {
                                // This is a child of a menu with visibility="1", format it like menu items
                                // Find what position this child has in its parent's children array
                                for (int j = 0; j < parent->num_children; j++) {
                                    if (parent->children[j] == i) {
                                        printf("    [ ] %d. %s", j + 1, processed_label);  // Indented like menu items with number
                                        break;
                                    }
                                }
                            } else {
                                printf("    [ ] %s", processed_label);  // Indented for child items
                            }
                        } else {
                            printf("    [ ] %s", processed_label);  // Indented for child items
                        }
                    ...
                    if (i == active_interaction_element) {
                        ...
                        if (el->parent_index != -1) {
                            ...
                            if (strcmp(parent->type, "menu") == 0 && parent->visibility) {
                                ...
                                printf("    [ ] Link: %s", processed_label);  // Indented for child items
                            }
                        } else {
                            printf("    [ ] Link: %s", processed_label);  // Indented for child items
                        }
                    ...
                    if (i == active_interaction_element) {
                        ...
                        if (el->parent_index != -1) {
                            ...
                            if (strcmp(parent->type, "menu") == 0 && parent->visibility) {
                                ...
                                for (int j = 0; j < parent->num_children; j++) {
                                    if (parent->children[j] == i) {
                                        printf("    [ ] %d. Link", j + 1);  // Indented like menu items with number
                                        break;
                                    }
                                }
                            } else {
                                printf("    [ ] Link");  // Indented for child items
                            }
                        } else {
                            printf("    [ ] Link");  // Indented for child items
                        }
            ...
            if (el->key > 0 && el->label[0] != '\0') {
                ...
                if (is_interactive_element(el)) {
                    ...
                    if (i == active_interaction_element) {
                        ...
                        // Check if this element is a child of a menu with visibility="1"
                        // If so, format it more like how it appears in menu context
                        if (el->parent_index != -1) {
                            UIElement* parent = &elements[el->parent_index];
                            if (strcmp(parent->type, "menu") == 0 && parent->visibility) {
                                // This is a child of a menu with visibility="1", format it like menu items
                ...
                if (is_interactive_element(el)) {
                    ...
                    if (i == active_interaction_element) {
                        printf("[^] [Button: %s]", processed_label);
                    } else {
                        // Check if this element is a child of a menu with visibility="1"
                        // If so, format it more like how it appears in menu context
                        if (el->parent_index != -1) {
                            ...
                            if (strcmp(parent->type, "menu") == 0 && parent->visibility) {
                                ...
                                printf("    [ ] Button: %s", processed_label);  // Indented for child items
                            }
                        } else {
                            printf("    [ ] Button: %s", processed_label);  // Indented for child items
                        }
                    ...
                    if (i == active_interaction_element) {
                        ...
                        if (el->parent_index != -1) {
                            ...
                            if (strcmp(parent->type, "menu") == 0 && parent->visibility) {
                                ...
                                for (int j = 0; j < parent->num_children; j++) {
                                    if (parent->children[j] == i) {
                                        printf("    [ ] %d. Button", j + 1);  // Indented like menu items with number
                                        break;
                                    }
                                }
                            } else {
                                printf("    [ ] Button");  // Indented for child items
                            }
                        } else {
                            printf("    [ ] Button");  // Indented for child items
                        }
            ...
            if (el->width > 0 && el->height > 0) {
                ...
                if (is_interactive_element(el)) {
                    ...
                    if (el->label[0] != '\0') {
                        ...
                        substitute_variables_in_string(el->label, processed_label, sizeof(processed_label));
                        if (i == active_interaction_element) {
                            printf("[^] %s\n", processed_label);  // Show interaction mode indicator
                        } else {
                            printf("    [ ] %s\n", processed_label);  // Indented for child items
                        }
                    } else {
                        // If canvas has no label, still show if it's in interaction mode
                        if (i == active_interaction_element) {
                            printf("[^] Canvas\n");  // Show interaction mode indicator
                        } else {
                            printf("    [ ] Canvas\n");  // Indented for child items
                        }
                    }
                }
            ...
            // Menu element - show with its label and focus indicator
            if (is_interactive_element(el)) {
                interactive_index++;
                if (el->label[0] != '\0') {
                    char processed_label[MAX_LABEL_LEN * 2];
                    ...
                    if (i == active_interaction_element) {
                        // Element is in interaction mode - show with "^"
                        printf("[^] %d. %s\n", interactive_index, processed_label);  // Show interaction mode indicator
                        // When menu is active, also show its child items (without affecting main navigation numbering)
                        if (el->num_children > 0) {
                            printf("  Menu Items:\n");
                            for (int child_idx = 0; child_idx < el->num_children; child_idx++) {
                                int child_element_idx = el->children[child_idx];
                                if (child_element_idx >= 0 && child_element_idx < element_count) {
                                    UIElement* child_el = &elements[child_element_idx];
                                    if (strcmp(child_el->type, "button") == 0) {
                                        // Highlight the child - if it's marked for execution show [^], if it's focused show [>], otherwise show [ ]
                                        if (child_element_idx == execute_on_next_enter) {
                                            printf("    [^] %d. %s\n", child_idx + 1, child_el->label[0] ? child_el->label : "Unlabeled Button");
                                        } else if (child_idx == el->current_child_focus) {
                                            printf("    [>] %d. %s\n", child_idx + 1, child_el->label[0] ? child_el->label : "Unlabeled Button");
                                        } else {
                                            printf("    [ ] %d. %s\n", child_idx + 1, child_el->label[0] ? child_el->label : "Unlabeled Button");
                                        ...
                                        // For non-button children, just display them with number
                                        if (child_element_idx == execute_on_next_enter) {
                                            printf("    [^] %d. %s\n", child_idx + 1, child_el->label[0] ? child_el->label : "Item");
                                        } else if (child_idx == el->current_child_focus) {
                                            printf("    [>] %d. %s\n", child_idx + 1, child_el->label[0] ? child_el->label : "Item");
                                        } else {
                                            printf("    [ ] %d. %s\n", child_idx + 1, child_el->label[0] ? child_el->label : "Item");
                    ...
                    // If menu has no label, still show with index if it's interactive
                    if (i == active_interaction_element) {
                        printf("[^] %d. Menu\n", interactive_index);  // Show interaction mode indicator
                        // When menu is active, also show its child items (without affecting main navigation numbering)
                        if (el->num_children > 0) {
...
bool process_input(int ch) {
    ...
    if (ch == 32 && active_interaction_element != -1 && 
        ...
        if (strcmp(el->type, "cli_io") == 0) {
            ...
            // Immediately check for responses from the module (especially for commands that 
            // should update the display immediately)
            bool needs_refresh = process_module_responses();
            
            // If processing responses indicated a refresh is needed, render immediately
            if (needs_refresh) {
                render_elements();
    ...
    if (ch == '\r' || ch == '\n') {
        // Log enter key processing to debug file for precise timeline tracking
        FILE *debug_file = fopen("#.debug/dom_refresh_debug.log", "a");
        if (debug_file) {
            time_t now = time(0);
            char* time_str = ctime(&now);
            fprintf(debug_file, "INNERHTML: Enter key processed at: %s", time_str);
            fclose(debug_file);
        }
        ...
        if (nav_input_active) {
            if (nav_input_pos > 0) {
                ...
                if (target_number >= 1 && found_element_index != -1) {
                    ...
                    // Log element navigation/set focus for precise timeline
                    FILE *debug_file = fopen("#.debug/dom_refresh_debug.log", "a");
                    if (debug_file) {
                        time_t now = time(0);
                        char* time_str = ctime(&now);
                        fprintf(debug_file, "INNERHTML: Element #%d focused via navigation at: %s", target_number, time_str);
                        fclose(debug_file);
                    }
                } else {
                ...
                if (focused_element_index != -1) {
                    ...
                    // If the activated element is a menu, initialize child focus to the first child
                    if (strcmp(elements[active_interaction_element].type, "menu") == 0) {
                        UIElement* menu_el = &elements[active_interaction_element];
                        if (menu_el->num_children > 0) {
                            menu_el->current_child_focus = 0; // Focus on first child
                        }
                    }
                    clear_input_buffer();
                    
                    // Log element activation for precise timeline
                    FILE *debug_file = fopen("#.debug/dom_refresh_debug.log", "a");
                    if (debug_file) {
                        time_t now = time(0);
                        char* time_str = ctime(&now);
                        fprintf(debug_file, "INNERHTML: Element activated (focus->active) at: %s", time_str);
                        fclose(debug_file);
                    }
        ...
        // If any element is active [^]: Enter is passed directly to the appropriate module and element stays active
            ...
            if (strcmp(el->type, "canvas") == 0) {
                ...
                if (ipc.active && input_buffer_pos > 0) {
                    ...
                    // Immediately check for responses from the module (especially for commands like INNER_HTML 
                    // that should update the display immediately)
                    bool needs_refresh = process_module_responses();
                ...
                // For buttons, execute the onClick command on Enter press
                if (strlen(el->onClick) > 0) {
                    // Log when INNER_HTML command is triggered and sent to module
                    if (strncmp(el->onClick, "INNER_H", 7) == 0) {  // Covers both INNER_HTML and INNERHTML
                        FILE *dom_debug_file = fopen("#.debug/dom_refresh_debug.log", "a");
                        if (dom_debug_file) {
                            time_t now = time(0);
                            char* time_str = ctime(&now);
                            fprintf(dom_debug_file, "INNERHTML: Request sent from framework to module at: %s", time_str);
                            fclose(dom_debug_file);
                        }
                    }
                    
                    // Execute the onClick command immediately
                ...
                // For menus, check if a child item is currently focused
                if (el->num_children > 0 && el->current_child_focus >= 0) {
                    // Execute the currently focused child immediately (unified behavior with links/buttons)
                    int child_idx = el->current_child_focus;
                    int child_element_idx = el->children[child_idx];
                    if (child_element_idx >= 0 && child_element_idx < element_count) {
                        UIElement* child_el = &elements[child_element_idx];
                        // Execute the child's action (onClick or href) immediately, keeping parent menu active
                        if (strcmp(child_el->type, "button") == 0 && strlen(child_el->onClick) > 0) {
                            // Log when INNER_HTML command is triggered and sent to module
                            if (strncmp(child_el->onClick, "INNER_H", 7) == 0) {  // Covers both INNER_HTML and INNERHTML
                                FILE *dom_debug_file = fopen("#.debug/dom_refresh_debug.log", "a");
                                if (dom_debug_file) {
                                    time_t now = time(0);
                                    char* time_str = ctime(&now);
                            ...
                            send_ipc_command(child_el->onClick);
                            ...
                            return true; // Stay in menu active state and trigger refresh
                        }
                        // If the child is a link, navigate to its href
                        else if (strcmp(child_el->type, "link") == 0 && strlen(child_el->href) > 0) {
                            switch_to_file(child_el->href);
                            return true; // Keep menu active but trigger navigation
                        }
                        // If the child is another menu, make it the active element
                        else if (strcmp(child_el->type, "menu") == 0) {
                            active_interaction_element = child_element_idx;
                            // If the child menu has children of its own, initialize child focus
                            if (child_el->num_children > 0) {
                                child_el->current_child_focus = 0; // Focus on first child
                            }
                            return true; // Redraw to show new active element
                        }
                    }
                }
                // If no child element is focused or no children exist, handle menu's own onClick (if any)
        ...
        // If no element is active but one is focused [>]: Enter activates it
            ...
            // Log element activation (from focused to active) for precise timeline
            FILE *debug_file = fopen("#.debug/dom_refresh_debug.log", "a");
            if (debug_file) {
                time_t now = time(0);
                char* time_str = ctime(&now);
                fprintf(debug_file, "INNERHTML: Element activated (focus->active) via Enter at: %s", time_str);
                fclose(debug_file);
            }
    ...
    if (ch == ESC_KEY) {
        ...
        // Handle active elements normally
        if (active_interaction_element != -1) {
            UIElement* current_el = &elements[active_interaction_element];
            // If the current active element has a parent (like a menu item), go back to the parent
            if (current_el->parent_index != -1) {
                // Check if the parent is a menu and make it active instead
                UIElement* parent_el = &elements[current_el->parent_index];
                if (strcmp(parent_el->type, "menu") == 0) {
                    active_interaction_element = current_el->parent_index;
                    // Set the parent's child focus back to the element that was active (current_el)
                    for (int i = 0; i < parent_el->num_children; i++) {
                        if (parent_el->children[i] == (active_interaction_element == current_el->parent_index ? active_interaction_element : (current_el - elements))) {
                            // Actually, the active element is now the parent, so we need to find which child of the parent the current element was
                            for (int j = 0; j < parent_el->num_children; j++) {
                                if (parent_el->children[j] == (current_el - elements)) {  // current_el index is (current_el - elements)
                                    parent_el->current_child_focus = j;
                                    break;
                                }
                            }
                            break;
                    ...
                    // Simpler approach: find which child of parent was the previous active element
                    for (int j = 0; j < parent_el->num_children; j++) {
                        if (parent_el->children[j] == (current_el - elements)) {  
                            parent_el->current_child_focus = j;
                            break;
                        }
                    }
            ...
            // If no parent menu, or parent is not a menu, just deactivate
    ...
    if (ch >= ARROW_LEFT && ch <= ARROW_DOWN) {
        ...
        // If an element is active, handle arrow keys appropriately
        if (active_interaction_element != -1) {
            UIElement* el = &elements[active_interaction_element];
            
            // Handle menu-specific navigation when menu is active
            if (strcmp(el->type, "menu") == 0) {
                if (el->num_children > 0) {
                    // When menu is active, arrow keys navigate between its children
                    if (ch == ARROW_UP || ch == ARROW_LEFT) {
                        // Move to previous child
                        if (el->current_child_focus > 0) {
                            el->current_child_focus--;
                        } else if (el->num_children > 0) {
                            // Wrap around to last child
                            el->current_child_focus = el->num_children - 1;
                        }
                    } else if (ch == ARROW_DOWN || ch == ARROW_RIGHT) {
                        // Move to next child
                        if (el->current_child_focus < el->num_children - 1) {
                            el->current_child_focus++;
                        } else if (el->num_children > 0) {
                ...
                return true; // Redraw to show updated child focus
            }
            // For canvas and cli_io, send raw ASCII codes to modules with element ID
            else if (strcmp(el->type, "canvas") == 0 || strcmp(el->type, "cli_io") == 0) {
                ...
                if (strcmp(el->type, "canvas") == 0 && strlen(el->id) > 0) {
                    // Include element ID in the command for targeted input
                    snprintf(key_cmd, sizeof(key_cmd), "KEY:%d:ELEMENT_ID:%s", ch, el->id);
                } else {
                    // For non-canvas elements or canvas without ID, use basic format
                    snprintf(key_cmd, sizeof(key_cmd), "KEY:%d", ch);
    ...
    if (isprint(ch)) {
        ...
        if (active_interaction_element != -1) {
            ...
            // Handle special case when a menu is active - allow digit input to select child elements
            if (strcmp(el->type, "menu") == 0 && isdigit(ch)) {
                int child_num = ch - '0';  // Convert char digit to int
                
                // Check if the digit corresponds to a child element
                if (child_num >= 1 && child_num <= el->num_children) {
                    int child_idx = child_num - 1;  // Convert to 0-based index
                    el->current_child_focus = child_idx;  // Set focus to the selected child
                    return true;  // Redraw to show updated child focus
                }
            }
...
void debug_log(const char* format, ...) {
    if (!debug_file) {
        debug_file = fopen("#.debug/debug.log", "w");
        if (!debug_file) {
...
void capture_rendered_frame() {
    FILE* file = fopen("#.debug/frames.txt", "a");
    if (file != NULL) {
        time_t now = time(0);
        char* time_str = ctime(&now);
...
// Process a single response and return whether refresh is needed
bool process_single_response(char* response) {
    // Remove newline from response
    response[strcspn(response, "\n")] = 0;
    ...
    if (strncmp(response, "VAR:", 4) == 0) {
        ...
        if (equals) {
            *equals = '\0';
            ...
            // FIX: Any variable update should trigger a refresh to update text with variable values
            return true;  // Requires refresh for any variable change
        } else {
            debug_log("ERROR: Invalid VAR format: %s", response);
        }
    }
    // Check if this is a batched variable update
        ...
        if (semi_pos) {
            *semi_pos = '\0'; // Replace semicolon with null terminator
            ...
            debug_log("ERROR: Invalid VARS format - missing semicolon: %s", response);
            return false; // No refresh needed for invalid format
        }
        ...
        // FIX: Any variable update should trigger a refresh to update text with variable values
        return true;  // Requires refresh after any variable changes
    }
    ...
    // Handle INNER_HTML command to dynamically update element content
        ...
        // Log when module sends INNER_HTML response back to framework
        FILE *dom_debug_file = fopen("#.debug/dom_refresh_debug.log", "a");
        if (dom_debug_file) {
            time_t now = time(0);
            char* time_str = ctime(&now);
            fprintf(dom_debug_file, "INNERHTML: Response received from module and being processed at: %s", time_str);
            fclose(dom_debug_file);
        }
            ...
            *colon_pos = '\0'; // Split at the colon
            ...
            if (target_element_idx != -1) {
                ...
                if (num_elements >= 0) {
                    ...
                    // Log timestamp of when DOM change is actually made (elements added to virtual DOM)
                    FILE *dom_debug_file = fopen("#.debug/dom_refresh_debug.log", "a");
                    if (dom_debug_file) {
                        time_t now = time(0);
                        char* time_str = ctime(&now);
                        fprintf(dom_debug_file, "INNERHTML: DOM elements added to virtual DOM at: %s", time_str);
                        fclose(dom_debug_file);
                    }
                } else {
                ...
                // Debug: log that content replacement is completed
                printf("DEBUG: Content replacement completed for innerHTML\n"); // Console debug
                debug_log("Content replacement completed for innerHTML");
                
                // Also log to a separate file for easier tracking
                FILE *debug_file = fopen("#.debug/render_debug.log", "a");
                if (debug_file) {
                    time_t now = time(0);
                    char* time_str = ctime(&now);
                    fprintf(debug_file, "INNERHTML: Content updated, waiting for refresh signal at %s", time_str);
                    fclose(debug_file);
                }
                
                // INNER_HTML updates change the DOM structure, so we need to refresh UI state systems
                // This ensures navigation, focus, and visibility systems recognize new elements
                debug_log("INNER_HTML: About to refresh UI state systems after DOM modification");
                
                // Log timestamp of DOM refresh to debug file
                FILE *dom_debug_file = fopen("#.debug/dom_refresh_debug.log", "a");
                if (dom_debug_file) {
                    time_t now = time(0);
                    char* time_str = ctime(&now);
                    fprintf(dom_debug_file, "INNERHTML: DOM structure updated and UI state systems refreshed at: %s", time_str);
                    fclose(dom_debug_file);
                }
                
                initialize_focus(); // Rescan for interactive elements after DOM changes
                debug_log("INNER_HTML: UI state systems refreshed after DOM modification");
                
                // INNER_HTML updates should trigger a refresh, similar to VAR updates with status_message
                return true;  // Requires refresh
            } else {
    ...
    return false; // No refresh needed for other responses or invalid responses
}

// Function to process any pending responses from modules
bool process_module_responses() {
    if (!ipc.active) {
        return false; // No active module to read from
    }
    
    bool needs_refresh = false;
    
    // Process all available responses in a loop
    while (1) {
        ...
        if (read_ipc_response(response, sizeof(response)) != 0) {
            break; // Error reading response, no more data
        }
        
        // Process this single response and check if refresh is needed
        if (process_single_response(response)) {
            needs_refresh = true;
        }
    }
