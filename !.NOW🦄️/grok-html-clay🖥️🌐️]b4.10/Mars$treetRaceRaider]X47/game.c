...
void save_current_state(Player players[], int num_players, int game_length, Market *market, Company companies[], int num_companies);
void load_game_state(Player players[], int *num_players, int *game_length, Market *market, Company companies[], int num_companies);
...
void reset_terminal_mode() {
    ...
    write(STDOUT_FILENO, "[?25h", 6); // Show cursor
...
void load_game_state(Player players[], int *num_players, int *game_length, Market *market, Company companies[], int num_companies) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        char *value = strtok(NULL, "\n");
        if (key && value) {
            if (strcmp(key, "game_length") == 0) *game_length = atoi(value);
            else if (strcmp(key, "num_players") == 0) *num_players = atoi(value);
            else if (strcmp(key, "market_interest_rate") == 0) market->interest_rate = atof(value);
            else if (strncmp(key, "player_", 7) == 0) {
                int player_index; char player_key[50];
                sscanf(key, "player_%d_%s", &player_index, player_key);
                if (strcmp(player_key, "name") == 0) strcpy(players[player_index].name, value);
                else if (strcmp(player_key, "cash") == 0) players[player_index].cash = atof(value);
                else if (strcmp(player_key, "type") == 0) players[player_index].type = (PlayerType)atoi(value);
                else if (strcmp(player_key, "ticker_on") == 0) players[player_index].ticker_on = atoi(value);
            } else if (strncmp(key, "company_", 8) == 0) {
                int company_index; char company_key[50];
                sscanf(key, "company_%d_%s", &company_index, company_key);
                if (strcmp(company_key, "name") == 0) strcpy(companies[company_index].name, value);
                else if (strcmp(company_key, "price") == 0) companies[company_index].price = atof(value);
                else if (strcmp(company_key, "shares") == 0) companies[company_index].shares = atoi(value);
            }
        }
    }
    fclose(fp);
...
void save_current_state(Player players[], int num_players, int game_length, Market *market, Company companies[], int num_companies) {
    ...
    if (fp == NULL) { printf("Error: Could not open gamestate.txt for writing.\n"); return; }
    fprintf(fp, "game_length:%d\nnum_players:%d\nmarket_interest_rate:%.2f\n", game_length, num_players, market->interest_rate);
    for (int i = 0; i < num_players; i++) {
        fprintf(fp, "player_%d_name:%s\nplayer_%d_cash:%.2f\nplayer_%d_type:%d\nplayer_%d_ticker_on:%d\n",
                i, players[i].name, i, players[i].cash, i, players[i].type, i, players[i].ticker_on);
    }
    for (int i = 0; i < num_companies; i++) {
        fprintf(fp, "company_%d_name:%s\ncompany_%d_price:%.2f\ncompany_%d_shares:%d\n",
                i, companies[i].name, i, companies[i].price, i, companies[i].shares);
    }
    fclose(fp);
}
...
void display_main_menu(Player *player, Market *market, char* choice_buffer) {
    ...
    buffer_print(&ab, "----------------------------------\n");
    buffer_print(&ab, "My Balance Sheet: (Millions)\n");
    buffer_print(&ab, "Cash (DD's)..\t\t%.2f\n", player->cash);
...
int main(int argc, char *argv[]) {
    ...
    load_game_state(players, &num_players, &game_length, &market, companies, 5);
