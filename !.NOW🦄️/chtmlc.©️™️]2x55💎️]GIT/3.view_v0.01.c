...
void parse_chtml(const char* filename) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        elements[num_elements].width_str[0] = '\0';
        elements[num_elements].height_str[0] = '\0';
        ...
        while (attr_start && (attr_start = strstr(attr_start, " ")) != NULL) {
            ...
            else if (strcmp(attr_name, "width") == 0) strncpy(elements[num_elements].width_str, attr_value, 19);
            else if (strcmp(attr_name, "height") == 0) strncpy(elements[num_elements].height_str, attr_value, 19);
...
void calculate_layout() {
    for (int i = 0; i < num_elements; i++) {
        ...
        // Calculate width
        if (strchr(el->width_str, '%')) {
            float percentage = atof(el->width_str) / 100.0f;
            if (el->parent != -1) {
                el->width = (int)(elements[el->parent].width * percentage);
            } else {
                el->width = (int)(window_width * percentage);
            }
        } else {
            el->width = atoi(el->width_str);
        }

        // Calculate height
        if (strchr(el->height_str, '%')) {
            float percentage = atof(el->height_str) / 100.0f;
            if (el->parent != -1) {
                el->height = (int)(elements[el->parent].height * percentage);
            } else {
                el->height = (int)(window_height * percentage);
            }
        } else {
            el->height = atoi(el->height_str);
        }
    }
}
...
void draw_element(UIElement* el) {
    ...
    if (strcmp(el->type, "canvas") == 0) {
        ...
        if (el->canvas_render_func != NULL) {
            ...
            glScissor(abs_x, abs_y, el->width, el->height);
        ...
        if (should_draw_menu_bar) {
            ...
            int text_offset_y = (el->height / 2) - 9;  // Adjusted to position text properly in menu and prevent cutting off
    ...
    *height = 600;
    ...
    for (int i = 0; i < num_elements; i++) {
        if (strcmp(elements[i].type, "window") == 0) {
            ...
            if (elements[i].width != 0) *width = elements[i].width;
            if (elements[i].height != 0) *height = elements[i].height;
