...
int main() {
    ...
    while (1) {
        ...
        if (fgets(command, sizeof(command), stdin) != NULL) {
            ...
            // 1. Send player shape (blue moving square with variable alpha)
            printf("SHAPE;SQUARE;player;%.2f;%.2f;%.2f;20.0;20.0;5.0;0.0;0.0;1.0;%.2f\n", 
                   player_x, player_y, player_z, player_alpha);
            
            // 2. Send enemy shape (red square with variable alpha)
            printf("SHAPE;RECT;enemy;%.2f;%.2f;%.2f;15.0;15.0;5.0;1.0;0.0;0.0;%.2f\n", 
                   enemy_x, enemy_y, enemy_z, enemy_alpha);
            
            // 3. Send second enemy shape (orange square - static alpha)
            printf("SHAPE;SQUARE;enemy2;%.2f;%.2f;%.2f;12.0;12.0;5.0;1.0;0.5;0.0;1.0\n", 
                   enemy2_x, enemy2_y, enemy2_z);
            
            // 4. Send powerup shape (green circle with pulsing alpha)
            printf("SHAPE;CIRCLE;powerup;%.2f;%.2f;%.2f;8.0;8.0;5.0;0.0;1.0;0.0;%.2f\n", 
                   powerup_x, powerup_y, powerup_z, pulse_alpha);
            
            // 5. Send background element (purple pulsing square with static alpha)
            ...
            // 6. Send animated shapes (rotating triangle with wave alpha)
            float time = (float)score / 10.0f;
            float rot_x = 400.0f + 100.0f * cos(time);
            float rot_y = 300.0f + 100.0f * sin(time);
            printf("SHAPE;TRIANGLE;rotating_shape;%.2f;%.2f;%.2f;15.0;15.0;5.0;1.0;1.0;0.0;%.2f\n", 
