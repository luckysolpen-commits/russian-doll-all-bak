...
// Include PDLO parser for executable routing (optional - define USE_PDLO_ROUTING to enable)
#ifdef USE_PDLO_ROUTING
#include "../../locations/pdlo_parser.c"
#endif
...
int score = 0;
char last_command_output[1000] = "";  // Command output message for display
int initial_render_done = 0;  // Flag to prevent double initial render
...
void execute_clock_method(const char* method_name);
...
void render_print() {
    ...
    // Get pet name
    FILE *fp = popen("./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet get-state name", "r");
    if (fp) {
    ...
    // Get hunger
    fp = popen("./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet get-state hunger", "r");
    if (fp) {
    ...
    // Get happiness
    fp = popen("./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet get-state happiness", "r");
    if (fp) {
    ...
    // Get energy
    fp = popen("./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet get-state energy", "r");
    if (fp) {
    ...
    // Get level
    fp = popen("./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet get-state level", "r");
    if (fp) {
    ...
    // Get turn count from clock piece state file
    char turn_str[10] = "0";
    FILE *turn_fp = fopen("pieces/clock/state.txt", "r");
    if (turn_fp) {
        char line[256];
        while (fgets(line, sizeof(line), turn_fp)) {
            line[strcspn(line, "\n")] = 0; // Remove newline
            if (strncmp(line, "turn ", 5) == 0) {
                sscanf(line, "turn %s", turn_str);
                break;
            }
        }
        fclose(turn_fp);
    ...
    // Get time from clock piece state file
    char time_str[20] = "08:00:00";
    FILE *time_fp = fopen("pieces/clock/state.txt", "r");
    if (time_fp) {
        char line[256];
        while (fgets(line, sizeof(line), time_fp)) {
            line[strcspn(line, "\n")] = 0; // Remove newline
            if (strncmp(line, "time ", 5) == 0) {
                sscanf(line, "time %s", time_str);
                break;
            }
        }
        fclose(time_fp);
    ...
    fprintf(fp2, "Controls: w=up, s=down, a=left, d=right | 1=feed, 2=play, 3=sleep, 4=status, 5=evolve, 6=end_turn\n");
    
    // Display command output message if available
    if (last_command_output[0] != '\0') {
        fprintf(fp2, "\n%s\n", last_command_output);
        // Clear the message after displaying
        last_command_output[0] = '\0';
    }
    ...
    // Append marker to master_ledger frame_changed (for both renderers)
...
// Save map to file
void save_map_to_file(const char *map_name) {
    char filename[MAX_FILENAME];
    snprintf(filename, MAX_FILENAME, "pieces/world/%s", map_name);
    ...
    if (file) {
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols_rows; j++) {
                fputc(map_matrix[i][j], file);
            }
            fputc('\n', file); // Add newline at end of each row
        }
        fclose(file);
    }
}
...
void update_map_0(int x_coords, int y_coords) {
    ...
    if (target_tile == WALL_SYMBOL) {
        ...
        FILE *fp = popen("./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet get-state happiness", "r");
        ...
        // Increase happiness
        sprintf(cmd, "./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet set-state happiness %d", happiness + 5);
        system(cmd);
        ...
        char cmd[200];
        sprintf(cmd, "./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet set-state pos_x %d", player_x);
        system(cmd);
        sprintf(cmd, "./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet set-state pos_y %d", player_y);
        system(cmd);
        
        // Save the updated map to file so it persists across restarts
        save_map_to_file("map.txt");
    }
...
void update_player_stats() {
    ...
    char cmd[200];
    // Get current energy
    FILE *fp = popen("./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet get-state energy", "r");
    ...
    if (energy > 0) {
        sprintf(cmd, "./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet set-state energy %d", energy - 1);
        system(cmd);
    }
    ...
    if (hunger < 100) {
        sprintf(cmd, "./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet set-state hunger %d", hunger + 1);
        system(cmd);
    }
}
...
void process_key_command(int key) {
    ...
    switch (key) {
        ...
        case '1': 
            // Feed command
            {
                char cmd[200];
                // Get current hunger
                FILE *fp = popen("./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet get-state hunger", "r");
                int hunger = 50; // default
                if (fp) {
                    char val[10];
                    if (fgets(val, sizeof(val), fp)) {
                ...
                sprintf(cmd, "./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet set-state hunger %d", new_hunger);
                system(cmd);
                ...
                sprintf(cmd, "./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet set-state happiness %d", new_happiness);
                system(cmd);
                ...
                // Get response from fuzzpet piece and set for display
                char response_cmd[300];
                char response[500];
                FILE *rsp_fp;
                sprintf(response_cmd, "./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet get-response fed");
                rsp_fp = popen(response_cmd, "r");
                if (rsp_fp) {
                    if (fgets(response, sizeof(response), rsp_fp)) {
                        response[strcspn(response, "\n")] = 0;
                        snprintf(last_command_output, sizeof(last_command_output), "[FUZZPET] %s", response);
                    }
                    pclose(rsp_fp);
                }
            }
        ...
        case '2':
            // Play command
            {
                ...
                sprintf(cmd, "./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet set-state energy %d", new_energy);
                system(cmd);
                ...
                char response[500];
                FILE *rsp_fp;
                sprintf(response_cmd, "./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet get-response played");
                rsp_fp = popen(response_cmd, "r");
                if (rsp_fp) {
                    if (fgets(response, sizeof(response), rsp_fp)) {
                        response[strcspn(response, "\n")] = 0;
                        sprintf(last_command_output,  "[FUZZPET] %s", response);
                    }
                    pclose(rsp_fp);
                }
            }
        ...
        case '3':
            // Sleep command
            {
                ...
                char response[500];
                FILE *rsp_fp;
                sprintf(response_cmd, "./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet get-response slept");
                rsp_fp = popen(response_cmd, "r");
                if (rsp_fp) {
                    if (fgets(response, sizeof(response), rsp_fp)) {
                        response[strcspn(response, "\n")] = 0;
        ...
        case '5':
            // Evolve command
            {
                char cmd[200];
                // Get current level
                FILE *fp = popen("./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet get-state level", "r");
                int level = 1; // default
                if (fp) {
                    char val[10];
                    if (fgets(val, sizeof(val), fp)) {
                ...
                sprintf(cmd, "./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet set-state level %d", new_level);
                system(cmd);
                ...
                char response[500];
                FILE *rsp_fp;
                sprintf(response_cmd, "./pieces/master_ledger/plugins/+x/piece_manager.+x fuzzpet get-response evolved");
                rsp_fp = popen(response_cmd, "r");
                if (rsp_fp) {
                    if (fgets(response, sizeof(response), rsp_fp)) {
                        response[strcspn(response, "\n")] = 0;
            ...
            break;
        case '6':
            // End turn command - advance time and increment turn counter
            {
                // Execute the combined turn and time incrementer (using PDLO routing)
                execute_clock_method("end_turn");
                
                // Set command output message to display in frame
                strcpy(last_command_output, "[CMD] Turn ended. Time and turn advanced.");
                
                // Log turn end event to master ledger
                time_t rawtime;
                struct tm *timeinfo;
                char timestamp[100];
                time(&rawtime);
                ...
                FILE *ledger = fopen("pieces/master_ledger/master_ledger.txt", "a");
                if (ledger) {
                    fprintf(ledger, "[%s] EventFire: turn_end on clock | Trigger: end_turn_action\n", timestamp);
                    fclose(ledger);
                }
                
                // Log turn end event to master ledger  
                FILE *resp_ledger = fopen("pieces/master_ledger/master_ledger.txt", "a");
                if (resp_ledger) {
                    fprintf(resp_ledger, "[%s] EventFire: turn_end on clock | Source: game_logic\n", timestamp);
                    fclose(resp_ledger);
                }
            }
            break;
        default: 
            if (key >= 'a' && key <= 'z') {
                ...
                char cmd[200];
                sprintf(cmd, "./pieces/master_ledger/plugins/+x/piece_manager.+x keyboard set-state last_key_logged '%c'", key);
                system(cmd);
            }
    ...
    if (x != 0 || y != 0) {
        update_map_0(x, y);
        // Execute the time and turn incrementers when moving (using PDLO routing)
        execute_clock_method("increment_time");
        execute_clock_method("increment_turn");
    }
...
// Execute clock methods using PDLO routing
// Uses PDLO parser if USE_PDLO_ROUTING is defined, otherwise uses hardcoded fallback
void execute_clock_method(const char* method_name) {
    char exe_path[MAX_FILENAME];
    
#ifdef USE_PDLO_ROUTING
    // Full TPM-compliant PDLO routing
    PDLOData clock_pdl;
    if (parse_pdl_file("pieces/clock/clock.pdl", &clock_pdl)) {
        if (get_method_executable(&clock_pdl, method_name, exe_path, sizeof(exe_path))) {
            system(exe_path);
            return;
        }
    }
    // Fall through to hardcoded paths if PDLO parsing fails
#endif
    
    // Hardcoded fallback paths (ensures system works even without PDLO parser)
    if (strcmp(method_name, "increment_time") == 0) {
        snprintf(exe_path, sizeof(exe_path), "./pieces/clock/plugins/+x/increment_time.+x");
    } else if (strcmp(method_name, "increment_turn") == 0) {
        snprintf(exe_path, sizeof(exe_path), "./pieces/clock/plugins/+x/increment_turn.+x");
    } else if (strcmp(method_name, "end_turn") == 0) {
        snprintf(exe_path, sizeof(exe_path), "./pieces/clock/plugins/+x/end_turn.+x");
    } else {
        return;
    }
...
int main(int argc, char **argv) {
    ...
    // Clear the keyboard history file to prevent processing old commands from previous sessions
    FILE *history = fopen("pieces/keyboard/history.txt", "w");
    if (history) {
