...
class PieceManager:
    ...
    def get_path_for_piece(self, piece_name, filename):
        ...
        return os.path.join(self.script_dir, f'../../{piece_name}/{filename}')
    ...
    def get_piece_responses(self, piece_name):
        """Get responses dictionary from a piece's piece.txt file."""
        state_path = self.get_path_for_piece(piece_name, 'piece.txt')
        
        if not os.path.exists(state_path):
            print(f"Warning: Piece file not found for {piece_name}", file=sys.stderr)
            return {}
            ...
            with open(state_path, 'r') as f:
                ...
                for line in f:
                    ...
                    if line_stripped.startswith('responses:'):
                        ...
                        continue
                    if in_responses:
                        # Check if we've hit another top-level section (no leading whitespace, has colon)
                        if line_stripped and ':' in line_stripped and not line[0].isspace():
                        ...
                        if ':' in line_stripped and not line_stripped.startswith('#'):
                            ...
                            key = key.strip()
                            value = value.strip()
                            if key and value and key not in ['state', 'methods', 'event_listeners', 'responses']:
                                responses[key] = value
        except Exception as e:
            print(f"Error reading responses for {piece_name}: {e}", file=sys.stderr)
            
        return responses
    
    def write_piece_state(self, piece_name, state_data):
        """Write state to a piece's piece.txt file, preserving structure."""
        state_path = self.get_path_for_piece(piece_name, 'piece.txt')
                ...
                for line in existing_lines:
                    ...
                    if line_stripped.startswith('state:'):
                        ...
                        for key, value in state_data.items():
                            f.write(f'  {key}={value}\n')
                            state_keys_written.add(key)
                        continue
                    if in_state_section:
                        # Skip old state lines
                        if line_stripped and not line[0].isspace() and ':' in line_stripped:
                            in_state_section = False
                        else:
                            continue
                    f.write(line)
                
                # If no state section existed, add it at the beginning
                if not any(l.strip().startswith('state:') for l in existing_lines):
                    # Write state at top, then rest of content
                    pass  # For now, just create simple state file if structure doesn't match
