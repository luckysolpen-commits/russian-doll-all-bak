...
def render_frame():
    ...
    marker_path = os.path.join(script_dir, '../pieces/master_ledger/frame_changed.txt')
...
def process_key(key_code):
    ...
    if char == 'w':
        ...
        subprocess.run(['python3', 'plugins/increment_time.py'], check=False)
        subprocess.run(['python3', 'plugins/increment_turn.py'], check=False)
    elif char == 's':
    ...
    # Command processor logic - delegate to fuzzpet
        ...
        subprocess.run(['python3', 'plugins/end_turn.py'], check=False)
    else:
