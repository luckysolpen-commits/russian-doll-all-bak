...
int main(int argc, char **argv) {
    ...
    // Get the current file modification time BEFORE checking for changes
    last_current_frame_time = get_file_mod_time("pieces/display/current_frame.txt");
    
    // Also get initial marker file size
    struct stat st;
    ...
    time_t current_time;
    while (1) {
        // Check marker file SIZE for reliable change detection
        if (stat("pieces/master_ledger/frame_changed.txt", &st) == 0) {
