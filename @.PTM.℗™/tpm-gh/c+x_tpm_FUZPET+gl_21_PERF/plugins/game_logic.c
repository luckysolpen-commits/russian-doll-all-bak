...
int score = 0;
char last_command_output[200] = "";  // Command output message for display
...
// Render and save the complete frame to the GL display piece
void gl_render_print() {
    FILE *fp2 = fopen("pieces/gl_display/current_frame.txt", "w");
    if (fp2 == NULL) {
        fprintf(stderr, "Error opening pieces/gl_display/current_frame.txt\n");
        return;
    }

    // Write the complete frame including map and pet stats (same as render_print)
    fprintf(fp2, "==================================\n");
    fprintf(fp2, "       FUZZPET DASHBOARD        \n");
    fprintf(fp2, "==================================\n");
...
void process_key_command(int key) {
    ...
    switch (key) {
        ...
        case '1': 
            // Feed command
            {
                ...
                // Request response for the event as per TPM standards
                FILE *resp_ledger = fopen("pieces/master_ledger/master_ledger.txt", "a");
                if (resp_ledger) {
                    fprintf(resp_ledger, "[%s] ResponseRequest: fed on fuzzpet | Source: game_logic\n", timestamp);
                    fclose(resp_ledger);
                }
            }
            break;
        case '2':
            // Play command
            {
                ...
                FILE *resp_ledger = fopen("pieces/master_ledger/master_ledger.txt", "a");
                if (resp_ledger) {
                    fprintf(resp_ledger, "[%s] ResponseRequest: played on fuzzpet | Source: game_logic\n", timestamp);
                    fclose(resp_ledger);
                }
            }
            break;
        case '3':
            // Sleep command
            {
                ...
                FILE *resp_ledger = fopen("pieces/master_ledger/master_ledger.txt", "a");
                if (resp_ledger) {
                    fprintf(resp_ledger, "[%s] ResponseRequest: slept on fuzzpet | Source: game_logic\n", timestamp);
                    fclose(resp_ledger);
                }
            }
            break;
        ...
        case '5':
            // Evolve command
            {
                ...
                FILE *resp_ledger = fopen("pieces/master_ledger/master_ledger.txt", "a");
                if (resp_ledger) {
                    fprintf(resp_ledger, "[%s] ResponseRequest: evolved on fuzzpet | Source: game_logic\n", timestamp);
                    fclose(resp_ledger);
                }
            }
            break;
        case '6':
            ...
            {
                // Execute the combined turn and time incrementer
                system("./+x/end_turn.+x");
    ...
    if (x != 0 || y != 0) {
        update_map_0(x, y);
        // Execute the time and turn incrementers when moving
        system("./+x/increment_time.+x");
        system("./+x/increment_turn.+x");
    }
...
int main(int argc, char **argv) {
    ...
    system("mkdir -p pieces/keyboard pieces/display pieces/gl_display");
    ...
    gl_render_print();  // Render the initial GL map
