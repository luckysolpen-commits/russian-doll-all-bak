...
## 🧊 3D TPMOS (fuzz-op-gl)
The `fuzz-op-gl` project represents a major milestone: **Full TPMOS Compliance in 3D**. It follows the **Sovereign Piece** pattern to decouple logic from rendering.

### 1. Host vs. Manager Split
*   **The Host (`gl_desktop.c`):** A "dumb" renderer. It reads `session/state.txt` to draw the world. It only owns the **Free Mode Camera**.
*   **The Manager (`fuzz-op-gl_manager.c`):** The "State Master." It owns entity positions, map logic, and **Locked Camera Modes**.

### 2. Command Injection Sync
To keep the 3D view and the Manager in sync, the Host injects commands back into the Manager's history buffer:
*   `COMMAND: CAMERA_SET:x,y,z,p,y,r` - Updates the Manager on camera movement.
*   `COMMAND: INTERACT` - Signals the Manager to enter map control mode.

### 3. Genetic Reconstruction
Entities in 3D TPMOS are not hardcoded. They are reconstructed from **Artifacts**.
*   **Artifacts (`artifact.txt`):** 8x8x8 voxel bitmasks defining the physical shape.
*   **Styles (`state.txt`):** Color and metadata defining the appearance.

### Code Example: 3D Sovereign Manager
The Manager (`projects/fuzz-op-gl/manager/fuzz-op-gl_manager.c`) parses 3D-specific commands:

```c
// gltpm_input_thread - Listen for Host commands
while (fgets(line, sizeof(line), history_file)) {
    if (strstr(line, "COMMAND: CAMERA_SET:")) {
        sscanf(line + 20, "%f,%f,%f", &cam_pos[0], &cam_pos[1], &cam_pos[2]);
        save_manager_state(); // Persistent 3D state
    }
    if (strstr(line, "COMMAND: INTERACT")) {
        is_map_control = 1;
