...
/* MAX_PATH may be defined on Windows, so we use a custom name */
#ifndef MAX_PATH_LEN
...
typedef struct {
    ...
    char content_source[MAX_PATH_LEN];
    int active;
} DesktopWindow;
...
void resolve_root(void) {
    ...
    while (fgets(line, sizeof(line), kvp)) {
        if (strncmp(line, "project_root=", 13) == 0) {
            char *v = line + 13;
            v[strcspn(v, "\n\r")] = 0;
            if (strlen(v) > 0) strncpy(project_root, v, MAX_PATH_LEN-1);
            break;
        }
    }
    fclose(kvp);
...
void create_terminal_window(void) {
    ...
    char title[32], content[MAX_PATH_LEN];
...
void create_folder_window(const char* name) {
    char title[64], content[MAX_PATH_LEN];
...
void init_paths(void) {
    ...
    if (asprintf(&desktop_state_path, "%s/projects/gl-os/pieces/desktop/state.txt", project_root) == -1) desktop_state_path = NULL;
}
