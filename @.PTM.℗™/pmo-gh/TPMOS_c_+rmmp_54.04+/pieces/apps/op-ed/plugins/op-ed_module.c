...
int is_active_layout() {
    ...
    if (fgets(line, sizeof(line), f)) { 
        ...
        if (strstr(cur, my_layout_path) != NULL) {
            ...
            char suite_dir[MAX_PATH];
            size_t n = strnlen(my_layout_path, sizeof(suite_dir) - 1);
            memcpy(suite_dir, my_layout_path, n);
            suite_dir[n] = '\0';
            char *slash = strrchr(suite_dir, '/');
            if (slash) *slash = '\0';

            if ((suite_dir[0] != '\0' && strstr(cur, suite_dir) != NULL) ||
                strstr(cur, "pieces/apps/op-ed/layouts/") != NULL) {
                res = 1;
            }
        }
        free(path); 
        ...
        if (key == 'w' || key == 'W' || key == 1002 || key == 's' || key == 'S' || key == 1003 || key == 'a' || key == 'A' || key == 1000 || key == 'd' || key == 'D' || key == 1001) {
            ...
            if (strcmp(active_target_id, "selector") == 0) {
                ...
                if (!resolve_piece_dir(active_target_id, piece_dir, sizeof(piece_dir))) {
                    ...
                    set_response("Entity not found, returned to selector");
                } else {
                    char *cmd = NULL; 
                ...
                if (!found) {
                    ...
                    if (asprintf(&pieces_p, "%s/projects/%s/pieces", project_root, current_project) != -1) {
                        ...
                        if (dir) {
                            struct dirent *entry;
                            while ((entry = readdir(dir)) != NULL) {
                                if (entry->d_name[0] == '.' || strcmp(entry->d_name, "selector") == 0 || strncmp(entry->d_name, "world_", 6) == 0) continue;
                                int tx = get_state_int_fast(entry->d_name, "pos_x");
                                int ty = get_state_int_fast(entry->d_name, "pos_y");
                                if (tx == cursor_x && ty == cursor_y) {
                                    strncpy(active_target_id, entry->d_name, 63);
