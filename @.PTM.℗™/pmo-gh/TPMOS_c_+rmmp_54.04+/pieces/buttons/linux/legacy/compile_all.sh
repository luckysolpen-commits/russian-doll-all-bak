...
# macOS Compatible: Uses framework flags for OpenGL/GLUT

echo "=== Compiling CHTPM v0.01 (File-Based Communication) ==="
echo "Using macro folder structure: pieces/apps/fuzzpet_app/"
echo ""

# Detect macOS and set appropriate compiler flags
if [ "$(uname)" = "Darwin" ]; then
    IS_MACOS=1
    # macOS uses frameworks for OpenGL and GLUT
    GL_FLAGS="-framework OpenGL -framework GLUT"
    # Silence OpenGL deprecation warnings on macOS
    CFLAGS_MACOS="-DGL_SILENCE_DEPRECATION"
    # macOS GLUT is typically in /System/Library/Frameworks
    # FreeType from Homebrew is in /usr/local/include or /opt/homebrew/include
    if [ -d "/opt/homebrew/include/freetype2" ]; then
        FREETYPE_FLAGS="-I/opt/homebrew/include/freetype2 -L/opt/homebrew/lib -lfreetype"
    elif [ -d "/usr/local/include/freetype2" ]; then
        FREETYPE_FLAGS="-I/usr/local/include/freetype2 -L/usr/local/lib -lfreetype"
    else
        FREETYPE_FLAGS="-I/usr/include/freetype2 -lfreetype"
    fi
    echo "  [macOS] Using framework flags: $GL_FLAGS"
    echo "  [macOS] Using FreeType flags: $FREETYPE_FLAGS"
    echo "  [macOS] Silencing OpenGL deprecation warnings"
else
    IS_MACOS=0
    GL_FLAGS="-lGL -lglut -lX11"
    FREETYPE_FLAGS="-lfreetype"
    CFLAGS_MACOS=""
fi
...
gcc -o pieces/apps/editor/plugins/+x/db_editor_module.+x pieces/apps/editor/plugins/db_editor_module.c
gcc -o pieces/apps/editor/plugins/+x/pal_editor_module.+x pieces/apps/editor/plugins/pal_editor_module.c
...
echo "Compiling keyboard/keyboard_input (Linux Muscle)..."
mkdir -p pieces/keyboard/plugins/+x
gcc -D_GNU_SOURCE -o pieces/keyboard/plugins/+x/keyboard_input.+x pieces/keyboard/src/keyboard_input_linux.c
...
mkdir -p pieces/display/plugins/+x
gcc -c $CFLAGS_MACOS -I/usr/include/freetype2 $FREETYPE_FLAGS pieces/display/gl_renderer.c -o /tmp/gl_renderer.o 2>&1 && \
    gcc $CFLAGS_MACOS /tmp/gl_renderer.o -o pieces/display/plugins/+x/gl_renderer.+x $GL_FLAGS $FREETYPE_FLAGS 2>&1 && \
    rm -f /tmp/gl_renderer.o || echo "  (GL renderer skipped - dependencies may be missing)"
...
gcc -o pieces/apps/gl_os/plugins/+x/gl_desktop.+x pieces/apps/gl_os/plugins/gl_desktop.c $GL_FLAGS -lGLU

# --- Fuzz-Op Project Manager (Legacy FuzzPet) ---
echo "Compiling projects/fuzz-op/manager..."
mkdir -p projects/fuzz-op/manager/+x
gcc -o projects/fuzz-op/manager/+x/fuzz-op_manager.+x projects/fuzz-op/manager/fuzz-op_manager.c -pthread

# --- GL-OS Project Manager ---
echo "Compiling projects/gl-os/manager..."
mkdir -p projects/gl-os/manager/+x
gcc -o projects/gl-os/manager/+x/gl_os_manager.+x projects/gl-os/manager/gl_os_manager.c -pthread
gcc -o pieces/apps/gl_os/plugins/+x/gl_os_session.+x pieces/apps/gl_os/plugins/gl_os_session.c
gcc -o pieces/apps/gl_os/plugins/+x/gl_os_loader.+x pieces/apps/gl_os/plugins/gl_os_loader.c
gcc $CFLAGS_MACOS -o pieces/apps/gl_os/plugins/+x/gl_os_renderer.+x pieces/apps/gl_os/plugins/gl_os_renderer.c $GL_FLAGS 2>/dev/null || true
...
# --- Prisc VM ---
echo "Compiling Prisc VM..."
mkdir -p pieces/system/prisc
gcc -o pieces/system/prisc/prisc+x pieces/system/prisc/prisc+x.c

# --- PDL Reader ---
echo "Compiling PDL reader..."
mkdir -p pieces/system/pdl/+x
gcc -o pieces/system/pdl/+x/pdl_reader.+x pieces/system/pdl/pdl_reader.c

# --- Special Purpose Modules ---
echo "Compiling man-pal_module..."
mkdir -p pieces/apps/man-pal/plugins/+x
gcc -o pieces/apps/man-pal/plugins/+x/man-pal_module.+x pieces/apps/man-pal/plugins/man-pal_module.c

echo "Compiling man-ops_module..."
mkdir -p pieces/apps/man-ops/plugins/+x
gcc -o pieces/apps/man-ops/plugins/+x/man-ops_module.+x pieces/apps/man-ops/plugins/man-ops_module.c

echo "Compiling op-ed_module..."
mkdir -p pieces/apps/op-ed/plugins/+x
gcc -o pieces/apps/op-ed/plugins/+x/op-ed_module.+x pieces/apps/op-ed/plugins/op-ed_module.c

# --- User Authentication App ---
echo "Compiling user_module..."
mkdir -p pieces/apps/user/plugins/+x
gcc -o pieces/apps/user/plugins/+x/user_module.+x pieces/apps/user/plugins/user_module.c -pthread -lssl -lcrypto
