...
void process_key(int key) {
    if (active_index == -1) {
        if (key == ARROW_UP || key == 'w' || key == 'W') {
            int prev = focus_index; do { focus_index--; } while (focus_index >= 0 && !is_navigable(focus_index));
            if (focus_index < 0) focus_index = prev; strcpy(nav_buffer, (key == ARROW_UP || key == 'w') ? "w" : "W"); clear_nav_on_next = true;
        } else if (key == ARROW_DOWN || key == 's' || key == 'S') {
            int prev = focus_index; do { focus_index++; } while (focus_index < element_count && !is_navigable(focus_index));
            if (focus_index >= element_count) focus_index = prev; strcpy(nav_buffer, (key == ARROW_DOWN || key == 's') ? "s" : "S"); clear_nav_on_next = true;
        } else if (key == ARROW_LEFT || key == 'a' || key == 'A') { strcpy(nav_buffer, (key == ARROW_LEFT || key == 'a') ? "a" : "A"); clear_nav_on_next = true; }
        else if (key == ARROW_RIGHT || key == 'd' || key == 'D') { strcpy(nav_buffer, (key == ARROW_RIGHT || key == 'd') ? "d" : "D"); clear_nav_on_next = true; }
        else if (isdigit(key)) {
            ...
            nav_buffer[0] = '\0'; if (element_count > 0 && focus_index < element_count) {
                ...
                else if (strcmp(el->type, "menu") == 0) { active_index = focus_index; if (el->num_children > 0) el->current_child_focus = 0; }
                else { 
                    active_index = focus_index; 
                    if (strcmp(el->type, "cli_io") != 0 && strcmp(el->onClick, "INTERACT") != 0) { 
                        if (strlen(el->onClick) > 0) send_command(el->onClick);
                        active_index = -1; 
                    }
                }
            }
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        if (stat("pieces/apps/fuzzpet_app/fuzzpet/view_changed.txt", &st) == 0) { 
            if (st.st_size > last_view_file_size) { last_view_file_size = st.st_size; dirty = 1; } 
        }
