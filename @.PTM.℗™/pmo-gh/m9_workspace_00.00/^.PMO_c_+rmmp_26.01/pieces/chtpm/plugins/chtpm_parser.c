...
int module_launched = 0;  // Track if module has been launched

char* scratch_substituted = NULL;
char project_root_path[MAX_PATH] = ".";

/* Resolve project root dynamically (same as orchestrator.c) */
void resolve_root() {
...
/* Build absolute path using project_root (same as orchestrator.c) */
...
void load_vars() {
    ...
    if (strstr(current_layout, "playrm") != NULL || strstr(current_layout, "player") != NULL) {
        resp_path = build_path_malloc("pieces/apps/fuzzpet_app/fuzzpet/last_response.txt");
    } else {
        resp_path = build_path_malloc("pieces/apps/fuzzpet_app/fuzzpet/last_response.txt");
    }
    ...
    if (rf) {
        ...
        if (fgets(resp_buf, sizeof(resp_buf), rf)) {
            set_var("last_response", trim_pmo(resp_buf));
        }
        fclose(rf);
    }
    free(resp_path);
    ...
    /* Load editor state for editor layout */
    ...
    if (vf) {
        ...
        /* Fallback if preferred view not found */
...
void inject_raw_key(int code) {
    /* Standard TPM injection path - shared by all modules for now */
...
void launch_module(const char* launch_str) { 
    if (module_launched && current_module_pid > 0) return;  // Already running
    cleanup_module();
    ...
    // NOTE: Parser continues running (NOT suspended) - like HTML <script> tag

    current_module_pid = fork(); 
    if (current_module_pid == 0) { 
        /* Child: change to project root before exec (same as orchestrator.c) */
        ...
        module_launched = 1;  // Mark as launched
...
void handle_launch_command(const char* cmd) {
    ...
    fprintf(stderr, "DEBUG: handle_launch_command('%s')\n", cmd);
    fprintf(stderr, "DEBUG: app_name='%s'\n", app_name);
    
    /* Build absolute path to app_list.txt */
    ...
    *p_dst = '\0'; free(content);
    ...
    for (int i = 0; i < tc && element_count < MAX_ELEMENTS; i++) {
        ...
        if (t->type == TOKEN_TEXT) {
            // Check if previous tag was <module> - if so, this text is the path
            if (i > 0 && strcmp(tokens[i-1].tag_name, "module") == 0 && 
                tokens[i-1].type == TOKEN_OPEN_TAG) {
                char module_path[512];
                strncpy(module_path, trim_pmo(t->content), 511);
                module_path[511] = '\0';
                launch_module(module_path);
                continue;
            }
            ...
            // NEW: Parse <module> tag and launch immediately
            if (strcmp(t->tag_name, "module") == 0 && t->type == TOKEN_SELFCLOSE_TAG) {
                // Extract module path from attributes or content
...
void process_key(int key) {
    if (active_index == -1) {
        if (key == ARROW_UP || key == 'w' || key == 'W' || key == JOY_UP) { int prev = focus_index; do { focus_index--; if (focus_index < 0) focus_index = element_count-1; } while (focus_index != prev && !is_navigable(focus_index)); clear_nav_on_next = true; }
            ...
            /* Write debug to file since stderr might be redirected */
            FILE *dbg = fopen("pieces/chtpm/debug.log", "a");
            if (dbg) {
                fprintf(dbg, "DEBUG: ENTER pressed, focus_index=%d, element_count=%d\n", focus_index, element_count);
                if (focus_index >= 0 && focus_index < element_count) {
                    UIElement *el = &elements[focus_index];
                    fprintf(dbg, "DEBUG: Element type='%s', href='%s', onClick='%s'\n", el->type, el->href, el->onClick);
                    if (strlen(el->href) > 0) { strncpy(current_layout, el->href, MAX_PATH-1); parse_chtm(); focus_index = 0; initialize_focus(); }
                        ...
                        fprintf(dbg, "DEBUG: Calling send_command('%s')\n", el->onClick);
        ...
        if (key == ESC_KEY || key == JOY_BUTTON_8) { active_index = -1; }
            ...
            // Pass x/z for Z-level control, arrows for movement
            inject_raw_key(eff);
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        // Check if module exited (optional - for clean shutdown)
        if (current_module_pid > 0) {
            int status;
            if (waitpid(current_module_pid, &status, WNOHANG) != 0) {
                fprintf(stderr, "OS: Module exited.\n");
                current_module_pid = -1;
                dirty = 1;
            }
        }
        ...
        // Parser always processes frame updates (not suspended)
