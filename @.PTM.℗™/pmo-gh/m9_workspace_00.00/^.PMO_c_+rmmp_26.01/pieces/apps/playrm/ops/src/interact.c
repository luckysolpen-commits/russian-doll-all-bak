    ...
    if (dir) {
        ...
        while ((entry = readdir(dir)) != NULL) {
            ...
            if (px == ax && py == ay && pz == az) {
                ...
                char details[512];
                snprintf(details, sizeof(details), "interacted with %s at (%d,%d) script=%s", entry->d_name, px, py, script_rel);
                log_event("Interact", active_piece, details);

                if (strcmp(script_rel, "unknown") != 0) {
                    /* Execute via Prisc */
                    char cmd[MAX_PATH * 3];
                    snprintf(cmd, sizeof(cmd), "%s/prisc-xpanse+4]SHIP/prisc+x %s/projects/%s/%s '' '' %s/pieces/apps/player_app/manager/player_ops.txt > /dev/null 2>&1",
                             project_root, project_root, current_project, script_rel, project_root);
                    system(cmd);
                }
                break;
            }
