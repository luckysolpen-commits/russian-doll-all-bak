...
int main(int argc, char *argv[]) {
    ...
    snprintf(map_path, sizeof(map_path), "%s/projects/%s/maps/%s_z%d.txt",
             project_root, current_project, map_id, current_z);
    ...
    if (y < 0 || y >= line_count) {
        fprintf(stderr, "Error: Y position %d out of bounds (map has %d lines)\n", y, line_count);
        return 1;
    }
    
    int line_len = strlen(lines[y]);
    if (x < 0 || x >= line_len) {
        fprintf(stderr, "Error: X position %d out of bounds (line has %d chars)\n", x, line_len);
        return 1;
    }
