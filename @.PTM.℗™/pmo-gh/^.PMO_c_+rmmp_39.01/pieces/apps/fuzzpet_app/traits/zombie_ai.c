...
int get_state_int(const char* piece_id, const char* key) {
    ...
    char *path = NULL;
    if (asprintf(&path, "%s/projects/fuzz-op/pieces/%s/state.txt", project_root, piece_id) == -1) return -1;
...
int main(int argc, char *argv[]) {
    ...
    snprintf(zstate_path, sizeof(zstate_path), "%s/projects/fuzz-op/pieces/%s/state.txt", project_root, zombie_id);
    char lines[100][256];
    int lc = 0, fx = 0, fy = 0;
