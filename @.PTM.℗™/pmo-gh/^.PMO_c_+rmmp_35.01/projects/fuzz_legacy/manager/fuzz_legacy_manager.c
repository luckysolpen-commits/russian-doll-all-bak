...
void route_input(int key) {
    ...
    if (key == 'x' || key == 'X') {
        ...
        /* Also update selector pos_z to match */
        if (strcmp(active_target_id, "selector") != 0) {
    ...
    /* Handle method hotkeys (2-8) - uses SELECTED ENTITY's methods, not selector's */
    int method_key = -1;
    if (key >= '2' && key <= '8') method_key = key - '0';
    else if (key >= 2002 && key <= 2008) method_key = key - 2000;  /* Joystick buttons */
    
    if (method_key > 0 && strcmp(active_target_id, "selector") != 0) {
...
int is_active_layout() {
    ...
    if (fgets(line, sizeof(line), f)) {
        fclose(f);
        int res = (strstr(line, "game.chtpm") != NULL || strstr(line, "fuzz_legacy.chtpm") != NULL);
        free(path);
        return res;
    }
    fclose(f); free(path);
