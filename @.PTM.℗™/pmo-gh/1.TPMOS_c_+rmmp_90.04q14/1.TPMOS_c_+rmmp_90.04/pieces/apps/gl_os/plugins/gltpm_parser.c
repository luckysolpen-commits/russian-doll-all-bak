...
static int gltpm_process_canvas_block(GLTPMScene *scene, const char *project_root, const char *project_id,
    ...
    while (line) {
        if (strstr(line, "+====") != NULL || strstr(line, "[Z-LEVEL]") != NULL) { 
            line = strtok(NULL, "\n"); 
            continue; 
        }
        ...
        for (int col = 0; p[col] != '\0'; col++) {
            if (p[col] == ' ' || p[col] == '\r' || p[col] == '|') continue;
...
static int gltpm_load_scene(GLTPMScene *scene, const char *project_root,
    ...
    scene->xelector_pos[0] = atoi(gltpm_get_var(vars, var_count, "selector_pos_x"));
    scene->xelector_pos[1] = atoi(gltpm_get_var(vars, var_count, "selector_pos_y"));
    ...
    while (*p) {
        ...
        if (strncmp(trimmed, "<scene", 6) == 0) {
            ...
            if (gltpm_extract_attr(trimmed, "title", title, sizeof(title))) strncpy(scene->title, title, 127);
