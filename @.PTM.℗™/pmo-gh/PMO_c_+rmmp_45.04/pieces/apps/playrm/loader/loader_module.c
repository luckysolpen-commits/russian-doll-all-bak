...
void get_pdl_value(const char* project, const char* section, const char* key, char* out_val, size_t out_sz) {
    ...
    if (f) {
        ...
        while (fgets(line, sizeof(line), f)) {
            char *s_ptr = strstr(line, section);
            if (s_ptr && s_ptr == line) {
                in_section = 1;
                continue;
            }
            if (in_section && line[0] != ' ' && line[0] != '\t' && line[0] != '\n' && line[0] != '-' && !strstr(line, "SECTION")) {
                if (!strstr(line, "|")) {
                   // Possible new section or empty line
                   // But let's check for the key regardless in this line if in_section is true
                }
            }
...
void load_project(const char* name) {
    ...
    if (entry_layout[0] == '\0') {
        if (strcmp(name, "op-ed") == 0 || strcmp(name, "template") == 0) {
            strcpy(entry_layout, "pieces/apps/op-ed/layouts/op-ed.chtpm");
    ...
    if (dbg) { fprintf(dbg, "LOADER: Project=%s, Layout=%s, Map=%s\n", name, entry_layout, starting_map); fclose(dbg); }
