...
// NEW: Application Manager Thread
void* fuzzpet_manager_thread_func(void* arg) {
    launch_and_register("fuzzpet_manager", "pieces/apps/fuzzpet_app/manager/plugins/+x/fuzzpet_manager.+x", NULL, true);
    return NULL;
}
...
int main() {
    ...
    pthread_t kb_t, res_t, ren_t, gl_t, clock_t, parser_t, manager_t;
    
    pthread_create(&kb_t, NULL, keyboard_thread_func, NULL);
    pthread_create(&res_t, NULL, response_thread_func, NULL);
    pthread_create(&manager_t, NULL, fuzzpet_manager_thread_func, NULL);
