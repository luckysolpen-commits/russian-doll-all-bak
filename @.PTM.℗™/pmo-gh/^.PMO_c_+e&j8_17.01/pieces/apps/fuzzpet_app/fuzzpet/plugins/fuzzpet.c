...
// TPM FuzzPet Core (v1.3 - STABILITY RESTORED)
// Responsibility: High-level dashboard, local sync.
...
int selector_active = 1;           // 1 = selector mode at startup (user must select pet first)
...
void process_key(int key) {
    if (selector_active) {
        ...
        if (key == '9' || key == J_B8) toggle_selector();
            ...
            int h = get_piece_state_int("pet_01", "hunger");
            set_piece_state_int("pet_01", "hunger", (h > 20) ? h - 20 : 0);
            strcpy(current_response_key, "fed");
        }
        else if (key == '3' || key == J_B2) { // Play
            int h = get_piece_state_int("pet_01", "happiness");
            set_piece_state_int("pet_01", "happiness", (h < 90) ? h + 10 : 100);
            strcpy(current_response_key, "played");
        }
        else if (key == '4' || key == J_B3) { // Sleep
            int e = get_piece_state_int("pet_01", "energy");
            set_piece_state_int("pet_01", "energy", (e < 70) ? e + 30 : 100);
    ...
    // write_sovereign_view() disabled if map_render wins
}

void* input_thread(void* arg) {
    long last_pos = 0;
    while (1) {
        // Sync selector_active with manager state before reading input
        FILE *msf = fopen("pieces/apps/fuzzpet_app/manager/state.txt", "r");
        if (msf) {
            char line[256];
            while (fgets(line, sizeof(line), msf)) {
                if (strncmp(line, "active_target_id=", 17) == 0) {
                    char *id = line + 17;
                    id[strcspn(id, "\n\r")] = 0;
                    selector_active = (strcmp(id, "selector") == 0) ? 1 : 0;
                    break;
                }
            }
            fclose(msf);
...
int main() {
    resolve_root();
    // Sync with manager state - check if selector or pet is active
    FILE *msf = fopen("pieces/apps/fuzzpet_app/manager/state.txt", "r");
    if (msf) {
        char line[256];
        while (fgets(line, sizeof(line), msf)) {
