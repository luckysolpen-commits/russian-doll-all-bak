...
# 2. Reset all entities to known good state (FUZZPET V2 PROJECT STRUCTURE)
echo "Resetting entities..."
# Create project-based piece structure for fuzzpet_v2
mkdir -p projects/fuzzpet_v2/pieces/fuzzpet
mkdir -p projects/fuzzpet_v2/pieces/selector
mkdir -p projects/fuzzpet_v2/pieces/zombie_01

# Fuzzpet (player character)
cat > projects/fuzzpet_v2/pieces/fuzzpet/state.txt << EOF
name=Fuzzball
type=player
pos_x=5
pos_y=2
...
# Selector
cat > projects/fuzzpet_v2/pieces/selector/state.txt << EOF
name=Selector
type=selector
pos_x=5
pos_y=5
...
# Zombie (enemy)
cat > projects/fuzzpet_v2/pieces/zombie_01/state.txt << EOF
name=Zombie
type=zombie
pos_x=10
pos_y=2
...
mkdir -p projects/fuzzpet_v2/manager
echo "project_id=fuzzpet_v2" > projects/fuzzpet_v2/manager/state.txt
echo "active_target_id=selector" >> projects/fuzzpet_v2/manager/state.txt
echo "current_z=0" >> projects/fuzzpet_v2/manager/state.txt
echo "last_key=None" >> projects/fuzzpet_v2/manager/state.txt
> projects/fuzzpet_v2/history.txt
