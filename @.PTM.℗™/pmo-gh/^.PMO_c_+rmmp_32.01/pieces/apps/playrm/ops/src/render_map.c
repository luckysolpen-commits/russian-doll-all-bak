...
void resolve_paths() {
    ...
    /* FIX: Read project_id from project-local manager state FIRST */
    ...
    /* Fallback to global manager state if project not set */
    ...
    /* FIX: Default to fuzzpet_v2 if project not set */
    if (strlen(current_project) == 0 || strcmp(current_project, "template") == 0) {
        strcpy(current_project, "fuzzpet_v2");
    }
...
int main() {
            ...
            while ((entry = readdir(dir)) != NULL && entity_count < MAX_ENTITIES) {
                ...
                if (on_map_val == 0) {
                    if (strcmp(entry->d_name, "selector") == 0 || 
                        ...
                        strcmp(entry->d_name, "hero") == 0) on_map_val = 1;
                }
    ...
    if (asprintf(&state_path, "%s/pieces/apps/player_app/state.txt", project_root) != -1) {
        char *tmp_state = NULL; asprintf(&tmp_state, "%s.tmp", state_path);
        FILE *sf = fopen(tmp_state, "w");
        if (sf) {
            fprintf(sf, "project_id=%s\napp_title=%s\npet_status=%-57s\npet_name=%s\npet_hunger=%s\npet_happiness=%s\npet_energy=%s\npet_level=%s\nlast_response=%-45s\ncurrent_z=%d\nactive_target_id=%s\nlast_key=%s\n", 
                    current_project, app_title, pet_status, p_name, p_h, p_hp, p_e, p_l, last_resp, current_z, active_target_id, last_key);
            fclose(sf); rename(tmp_state, state_path);
        }
        free(tmp_state); free(state_path);
    }
