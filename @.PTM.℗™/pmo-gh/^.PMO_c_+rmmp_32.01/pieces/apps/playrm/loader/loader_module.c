...
void load_project(const char* name) {
    ...
    if (asprintf(&path, "%s/pieces/apps/player_app/manager/state.txt", project_root) != -1) {
        ...
        if (f) {
            ...
            // DEFAULT FOCUS:
            if (strcmp(name, "fuzzpet_v2") == 0) fprintf(f, "active_target_id=selector\n");
            else fprintf(f, "active_target_id=hero\n");
    ...
    if (strcmp(name, "fuzzpet_v2") == 0) {
        char *cmd = NULL;
        asprintf(&cmd, "mkdir -p %s/projects/fuzzpet_v2/pieces/fuzzpet", project_root);
        system(cmd); free(cmd);
        
        asprintf(&cmd, "mkdir -p %s/projects/fuzzpet_v2/pieces/selector", project_root);
        system(cmd); free(cmd);

        char *fp_state = NULL;
        asprintf(&fp_state, "%s/projects/fuzzpet_v2/pieces/fuzzpet/state.txt", project_root);
        if (access(fp_state, F_OK) != 0) {
            FILE *f = fopen(fp_state, "w");
            if (f) {
                /* FIX: Add type field for entity rendering */
        ...
        /* Initialize selector piece */
        char *sel_state = NULL;
        asprintf(&sel_state, "%s/projects/fuzzpet_v2/pieces/selector/state.txt", project_root);
        if (access(sel_state, F_OK) != 0) {
            FILE *f = fopen(sel_state, "w");
            if (f) {
