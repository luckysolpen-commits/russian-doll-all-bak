...
void write_gl_os_state(void) {
    ...
    /* Get selected index from active terminal window */
    int selected_idx = 0;
    for (int i = 0; i < window_count; i++) {
    ...
    if (selected_idx >= 2) selected_idx = 0;
    ...
    for (int i = 0; i < 2; i++) {
        ...
        snprintf(line, sizeof(line), "║    %s %d. [%-20s]           ║\n", 
                 indicator, i + 1, menu_items[i]);
        strcat(menu_options, line);
    }
    fprintf(f, "menu_options=%s", menu_options);
...
void init_terminal_menu(DesktopWindow* win) {
    ...
    win->option_count = 2;  /* 2 menu items for navigation testing */
...
void create_terminal_window(void) {
    ...
    add_history(&windows[idx], "========================================");
    add_history(&windows[idx], "  1. Desktop Apps");
    add_history(&windows[idx], "  2. Terminal Apps");
    add_history(&windows[idx], "  3. Cube Demo");
    add_history(&windows[idx], "  4. File Browser");
    add_history(&windows[idx], "  5. Process Monitor");
    add_history(&windows[idx], "  6. Settings");
    add_history(&windows[idx], "  7. Help");
    add_history(&windows[idx], "  8. Exit");
    add_history(&windows[idx], "========================================");
    ...
    if (active_window_id != -1) {
        for (int i = 0; i < window_count; i++) {
            if (windows[i].id == active_window_id && strcmp(windows[i].type, "terminal") == 0) {
                ...
                /* For menu navigation, X-axis (left/right) is unused in this layout,
                   but we keep it for potential horizontal scrolls/tabs */
                ...
                /* Write new frame if selection changed */
                if (selection_changed) {
                    printf("[JOY NAV] Selection changed, writing new frame...\n");
                    printf("[JOY NAV] selected_idx: %d\n", windows[i].selected_index);
                    write_gl_os_frame();
                    printf("[JOY NAV] Frame written\n");
                }
...
int main(int argc, char** argv) {
    ...
    /* PROJECT MODE: Check if projects/gl-os/history.txt exists */
    char* project_history_path = NULL;
    asprintf(&project_history_path, "%s/projects/gl-os/history.txt", project_root);
    if (access(project_history_path, F_OK) == 0) {
        /* Use project-specific history for input (manager writes here) */
        free(session_history_path);
        session_history_path = project_history_path;
        printf("GL-OS: Project mode detected. Using %s\n", session_history_path);
    } else {
        free(project_history_path);
    }
