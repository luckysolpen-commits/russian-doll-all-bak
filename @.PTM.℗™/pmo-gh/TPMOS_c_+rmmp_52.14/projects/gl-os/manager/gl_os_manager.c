...
#ifdef _WIN32
    mkdir("pieces/apps/gl_os");
#else
    mkdir("pieces/apps/gl_os", 0777);
#endif

    // Launch dedicated desktop as a child process using absolute path
    char renderer_path[MAX_PATH];
    build_p_path(renderer_path, sizeof(renderer_path), "pieces/apps/gl_os/plugins/+x/gl_desktop.+x");
    char cmd[2048];
#ifndef _WIN32
    snprintf(cmd, sizeof(cmd), "%s &", renderer_path);
#else
    snprintf(cmd, sizeof(cmd), "start %s", renderer_path);
#endif
    system(cmd);
