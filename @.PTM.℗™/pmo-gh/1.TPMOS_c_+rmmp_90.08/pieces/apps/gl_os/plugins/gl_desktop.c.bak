...
void write_gl_os_state(void) {
    ...
    /* Get menu options from active terminal window */
    for (int i = 0; i < window_count; i++) {
        if (windows[i].id == active_window_id && strcmp(windows[i].type, "terminal") == 0) {
...
void init_terminal_menu(DesktopWindow* win) {
    ...
    strcpy(win->menu_options[1], "Terminal Apps");
...
void execute_option(DesktopWindow* win, int index) {
    ...
    /* Standard Terminal Contexts */
        ...
        if (index == 0) {
            ...
            strcpy(win->menu_options[2], "3D Cube Demo");
            strcpy(win->menu_options[3], "back");
            add_history(win, "Entering Desktop Apps...");
        } else if (index == 1) {
            /* Terminal Apps */
...
void keyboard(unsigned char key, int x, int y) {
    ...
    if (active_idx != -1 && strcmp(windows[active_idx].type, "project_mirror") == 0) {
        ...
        if (win->is_map_control) {
            ...
            else if (key == 'w' || key == 'W') win->cam_pos[2] += speed; /* Forward */
            else if (key == 's' || key == 'S') win->cam_pos[2] -= speed; /* Backward */
            else if (key == 'a' || key == 'A') win->cam_pos[0] -= speed; /* Left */
            else if (key == 'd' || key == 'D') win->cam_pos[0] += speed; /* Right */
            else if (key == 'z' || key == 'Z') win->cam_pos[1] += speed; /* Up */
            else if (key == 'x' || key == 'X') win->cam_pos[1] -= speed; /* Down */
            else if (key == 'q' || key == 'Q') win->cam_rot[1] -= 5.0f;  /* Rotate Left */
            else if (key == 'e' || key == 'E') win->cam_rot[1] += 5.0f;  /* Rotate Right */
...
void special_keyboard(int key, int x, int y) {
    ...
    if (active_window_id != -1) {
        ...
        if (idx != -1) {
            ...
            if (strcmp(win->type, "project_mirror") == 0 && win->is_map_control) {
