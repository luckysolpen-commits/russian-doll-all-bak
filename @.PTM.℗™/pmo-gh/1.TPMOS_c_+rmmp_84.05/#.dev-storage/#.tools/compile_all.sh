...
compile_gl() {
    local src="$1"
    local dst="$2"
    local libs="${3:--lglut -lGLU -lGL}"

    if [ -f "$src" ]; then
        mkdir -p "$(dirname "$dst")"
        if gcc -o "$dst" "$src" $libs 2>/dev/null; then
            echo "  ✓ $dst"
            ((COMPILED++))
        else
            echo "  ✗ FAILED: $src"
...
for proj_dir in projects/*/; do
    ...
    if [ -f "${proj_dir}manager/${proj_name}_manager.c" ]; then
        if [ "$proj_name" = "user" ]; then
            ...
            if gcc -o "${proj_dir}manager/+x/${proj_name}_manager.+x" "${proj_dir}manager/${proj_name}_manager.c" -pthread -lcrypto 2>/dev/null; then
