...
int is_active_layout() {
    ...
    if (fgets(line, sizeof(line), f)) {
        fclose(f);
        int res = (strstr(line, "editor.chtpm") != NULL);
        free(path);
        return res;
    }
    fclose(f); free(path);
...
int main() {
    ...
    char path[MAX_CMD];
    snprintf(path, sizeof(path), "%s/pieces/apps/editor/manager/state.txt", project_root);
    ...
    snprintf(history_path, sizeof(history_path), "%s/pieces/apps/editor/history.txt", project_root);
                ...
                if (hf) {
                    ...
                    while (fscanf(hf, "%d", &key) == 1) {
                        ...
                        if (gui_focus_index == 1) {
                            // Use SHARED move_selector op instead of hardcoded cursor movement
                            if (key == 'w' || key == 'W' || key == 1002) {
                                char cmd[MAX_CMD];
                                snprintf(cmd, sizeof(cmd), "%s/pieces/apps/playrm/ops/+x/move_selector.+x up > /dev/null 2>&1", project_root);
                                system(cmd);
                            }
                            else if (key == 's' || key == 'S' || key == 1003) {
                                char cmd[MAX_CMD];
                                snprintf(cmd, sizeof(cmd), "%s/pieces/apps/playrm/ops/+x/move_selector.+x down > /dev/null 2>&1", project_root);
                                system(cmd);
                            }
                            else if (key == 'a' || key == 'A' || key == 1000) {
                                char cmd[MAX_CMD];
                                snprintf(cmd, sizeof(cmd), "%s/pieces/apps/playrm/ops/+x/move_selector.+x left > /dev/null 2>&1", project_root);
                                system(cmd);
                            }
                            else if (key == 'd' || key == 'D' || key == 1001) {
                                char cmd[MAX_CMD];
                                snprintf(cmd, sizeof(cmd), "%s/pieces/apps/playrm/ops/+x/move_selector.+x right > /dev/null 2>&1", project_root);
                                system(cmd);
                            }
                        ...
                        if (key == ' ' && gui_focus_index == 1 && map_count > 0) {
                            ...
                            if (is_entity) {
                                ...
                                // Terrain glyph - place tile using SHARED place_tile op
                                snprintf(cmd, sizeof(cmd), "%s/pieces/apps/editor/ops/+x/place_tile.+x %s %d %d %s > /dev/null 2>&1",
