...
void scan_projects() {
    char path[MAX_PATH];
    snprintf(path, sizeof(path), "%s/projects", project_root);
    ...
    while ((entry = readdir(dir)) != NULL && project_count < MAX_ITEMS) {
        ...
        char abs_p[MAX_PATH];
        snprintf(abs_p, sizeof(abs_p), "%s/%s", path, entry->d_name);
        struct stat st;
...
void scan_maps() {
    char path[MAX_PATH];
    snprintf(path, sizeof(path), "%s/projects/%s/maps", project_root, current_project);
    DIR *dir = opendir(path);
    if (!dir) { map_count = 0; return; }
    map_count = 0;
    struct dirent *entry;
                ...
                if (hf) {
                    ...
                    while (fscanf(hf, "%d", &key) == 1) {
                        ...
                        if (gui_focus_index == 1) {
                            if (key == 'w' || key == 'W' || key == 1002) cursor_y--;
                            else if (key == 's' || key == 'S' || key == 1003) cursor_y++;
                            else if (key == 'a' || key == 'A' || key == 1000) cursor_x--;
                            else if (key == 'd' || key == 'D' || key == 1001) cursor_x++;
                        }
                        ...
                        if (key == ' ' && gui_focus_index == 1 && map_count > 0) {
                            ...
                            snprintf(cmd, sizeof(cmd), "%s/pieces/apps/editor/ops/+x/place_tile.+x %s %d %d %s > /dev/null 2>&1",
                                     project_root, project_maps[map_idx], cursor_x, cursor_y, active_set[glyph_idx]);
                            system(cmd);
                        }
