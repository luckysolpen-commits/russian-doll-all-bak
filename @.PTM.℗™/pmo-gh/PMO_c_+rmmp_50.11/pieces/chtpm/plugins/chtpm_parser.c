...
void launch_module(const char* launch_str) { 
    ...
    // Check if module binary exists before launching
    char *full_module_path = build_path_malloc(launch_str);
    if (access(full_module_path, X_OK) != 0) {
        // Module binary missing - set error state and switch to error layout
        set_var("module_error", "MISSING");
        set_var("module_error_path", launch_str);
        
        // Switch to module_missing.chtpm layout
        strncpy(current_layout, "pieces/apps/playrm/layouts/module_missing.chtpm", MAX_PATH - 1);
        ...
        if (dbg) {
            time_t t; struct tm *tm;
            time(&t); tm = localtime(&t);
            fprintf(dbg, "[%02d:%02d:%02d] MODULE ERROR: Binary not found: %s\n",
                    tm->tm_hour, tm->tm_min, tm->tm_sec, launch_str);
            fclose(dbg);
        }
        return;  /* Don't try to launch missing module */
    }
    free(full_module_path);
    
    // Clear any previous error state
    set_var("module_error", "");
    set_var("module_error_path", "");
