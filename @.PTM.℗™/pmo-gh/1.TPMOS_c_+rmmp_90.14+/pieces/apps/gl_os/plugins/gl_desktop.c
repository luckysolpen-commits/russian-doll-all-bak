...
typedef struct {
    ...
    char project_id[64]; /* Canonical ID for projects */
    int active;
    int minimized;
...
void execute_option(DesktopWindow* win, int index) {
    ...
    /* Standard Terminal Contexts */
        ...
        if (index == 0) {
            ...
            strcpy(win->menu_options[2], "demo_world");
...
void draw_window(DesktopWindow* win) {
    ...
    if (strcmp(win->type, "terminal") == 0) {
        ...
        char coord_hud[256];
        snprintf(coord_hud, sizeof(coord_hud), "CAM: (%.1f, %.1f, %.1f) | XEL: (%d, %d, %d)", 
...
void keyboard(unsigned char key, int x, int y) {
    ...
    if (active_idx != -1 && strcmp(windows[active_idx].type, "gltpm_app") == 0) {
        ...
        if (in_map) {
            /* GL-OS Host: Handle Camera Fly Controls locally for smooth movement */
            float speed = 0.2f;
            int camera_moved = 0;
            ...
            else if (key == 'd' || key == 'D') { win->cam_pos[0] += speed; camera_moved = 1; }
            else if (key == 'z' || key == 'Z') { win->cam_pos[1] += speed; camera_moved = 1; }
            else if (key == 'x' || key == 'X') { win->cam_pos[1] -= speed; camera_moved = 1; }
            else if (key == 'q' || key == 'Q') { win->cam_rot[1] -= 5.0f; camera_moved = 1; }
            else if (key == 'e' || key == 'E') { win->cam_rot[1] += 5.0f; camera_moved = 1; }

            if (camera_moved) {
                /* Optional: Sync camera pos to Manager if needed, but Host-driven is fine for Fly */
                glutPostRedisplay();
                return; /* Yield: Don't inject fly keys to piece logic */
            }

            /* Numeric modes and other keys: Inject to history and let Manager be the truth */
            char *history_path = NULL;
            const char* target_src = (win->gltpm_scene.interact_src[0] != '\0') ? win->gltpm_scene.interact_src : "pieces/apps/player_app/history.txt";
