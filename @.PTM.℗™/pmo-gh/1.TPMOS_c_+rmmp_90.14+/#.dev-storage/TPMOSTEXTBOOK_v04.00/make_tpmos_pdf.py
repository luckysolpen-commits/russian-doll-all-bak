...
BASE_DIR = SCRIPT_DIR
OUTPUT_PDF = os.path.join(SCRIPT_DIR, "TPMOS_TEXTBOOK_v03.01.pdf")
...
class TPMOS_PDF(FPDF):
    ...
    def header(self):
        if self.page_no() > 1:
            ...
            self.set_x(0)
            header_text = clean_emojis("📚 TPMOS TEXTBOOK v03.01 🎓  ")
    ...
    def add_code_block(self, code_text):
        ...
        # Set code font (monospace)
        self.set_font("Hanzi", "", 9)
        ...
        if self.get_y() + block_height > 280:
            self.add_page()

        # Draw background rectangle
        x_start = self.l_margin
        width = 210 - self.l_margin - self.r_margin
        self.rect(x_start, self.get_y(), width, block_height, style='F')

        # Set text color for code
        self.set_text_color(20, 20, 120)  # Dark blue for code

        # Write code lines
        y_start = self.get_y() + 2
        for i, line in enumerate(lines):
            # Clean emojis but preserve code formatting
            ...
            self.set_xy(x_start + 2, y_start + i * line_height)
            self.cell(width - 4, line_height, clean_line)

        # Reset position and formatting
...
def create_pdf():
    ...
    pdf.cell(0, 10, "Version 03.01 (Code-Complete Edition)", align="C", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(80)
