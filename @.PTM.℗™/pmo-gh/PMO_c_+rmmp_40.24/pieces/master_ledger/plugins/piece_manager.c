...
int main(int argc, char *argv[]) {
    ...
    if (piece_file[0] == '\0') {
        ...
        if (strlen(current_proj) > 0) {
            snprintf(piece_file, sizeof(piece_file), "projects/%s/pieces/%s/piece.pdl", current_proj, piece_id);
            if (access(piece_file, F_OK) != 0) piece_file[0] = '\0';
        }
        
        // Try world/map_01 next for entities like zombie_01, pet_01, etc.
        if (piece_file[0] == '\0') {
            snprintf(piece_file, sizeof(piece_file), "pieces/world/map_01/%s/%s.pdl", piece_id, piece_id);
            if (access(piece_file, F_OK) != 0) {
                // Then try apps/fuzzpet_app for selector, fuzzpet
                snprintf(piece_file, sizeof(piece_file), "pieces/apps/fuzzpet_app/%s/%s.pdl", piece_id, piece_id);
                if (access(piece_file, F_OK) != 0) {
                    // Default fallback
