    ...
    if (method_trigger_key >= '2' && method_trigger_key <= '8') {
        ...
        if (strlen(method) > 0) {
            if (strcmp(method, "scan") == 0 || strcmp(method, "collect") == 0 || 
                ...
                FILE *cf = fopen(state_path, "r");
                if (cf) {
                    char line[128];
                    while (fgets(line, sizeof(line), cf)) {
                        line[strcspn(line, "\n\r")] = 0;
                        char *eq = strchr(line, '=');
                        if (eq && strncmp(line, "mode", 4) == 0) {
                            strncpy(current_mode, eq + 1, 63);
                        }
                    }
                    fclose(cf);
                }
                ...
                // Update via piece_manager
                pid_t p = fork();
                if (p == 0) {
                    freopen("/dev/null", "w", stdout);
                    freopen("/dev/null", "w", stderr);
                    execl(pm, pm, "clock_daemon", "set-state", "mode", new_mode, NULL);
                    exit(1);
                } else if (p > 0) {
                    waitpid(p, NULL, 0);
                }
                ...
                char msg[128];
                snprintf(msg, sizeof(msg), "Clock mode: %s", new_mode);
                log_resp(msg);
            } else {
...
int main() {
    ...
    // TRUNCATE VIEW FILE (Prevent Ghost Frame)
    char path_view[4096];
    snprintf(path_view, sizeof(path_view), "%s/pieces/apps/fuzzpet_app/fuzzpet/view.txt", project_root);
    FILE *f_v = fopen(path_view, "w"); if (f_v) fclose(f_v);
