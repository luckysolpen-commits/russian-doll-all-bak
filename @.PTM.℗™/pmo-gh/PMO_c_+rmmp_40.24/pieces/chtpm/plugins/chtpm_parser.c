    ...
    while (fgets(line, sizeof(line), f)) {
        if (strncmp(line, "METHOD", 6) == 0) {
            ...
            *val_start = '\0';
            ...
            // ARCHITECTURE FIX: Always use KEY:n format for all methods
            // The command handler in PDL is for the Manager to execute, not the Parser
            // When ENTER is pressed, Parser injects KEY:n into history, Manager reads it and executes the PDL handler
            char *btn = NULL;
...
void send_command(const char* cmd) {
    if (strncmp(cmd, "LOAD_PROJECT:", 13) == 0) {
        ...
        if (f) {
            ...
            if (strcmp(proj_name, "man-pal") == 0 || strcmp(proj_name, "man-ops") == 0 || strcmp(proj_name, "op-ed") == 0) {
        ...
        } else if (strcmp(proj_name, "fuzz-op") == 0) {
