...
typedef enum { OP_ADDI, OP_BEQ, OP_LW, OP_SW, OP_JALR, OP_J, OP_HALT, OP_CUSTOM, OP_READ_HISTORY, OP_EXEC, OP_HIT_FRAME, OP_READ_STATE, OP_READ_ACTIVE_TARGET, OP_READ_ENV_KEY } OpBase;
        ...
        if (ext) {
            ...
            if (project_root_env) {
                ...
                if (realpath(project_root_env, base_abs)) {
                    snprintf(full_script_path, sizeof(full_script_path), "%s/%s", base_abs, clean_script_path);
                } else {
                    snprintf(full_script_path, sizeof(full_script_path), "%s/%s", project_root_env, clean_script_path);
                }
            } else {
                strcpy(full_script_path, clean_script_path);
            }
            // fprintf(stderr, "[Prisc Debug] Executing Op: %s\n", full_script_path);
...
int main(int argc, char **argv) {
        ...
        if (len != -1) {
            ...
            if (last_slash) {
                *last_slash = '\0';
                ...
                snprintf(alt_path, sizeof(alt_path), "%s/default_op.txt", binary_path);
    ...
    while (pc < inst_count) {
        ...
        if (i.op == OP_CUSTOM) {
            ...
            char path[512];
            if (strlen(i.literal_arg) > 0) strcpy(path, i.literal_arg);
            else {
                char *proj_root = getenv("PRISC_PROJECT_ROOT");
                if (proj_root) snprintf(path, sizeof(path), "%s/pieces/apps/player_app/history.txt", proj_root);
                else strcpy(path, "pieces/apps/player_app/history.txt");
            }
            ...
            if (hf) {
                ...
                if (size > 0) {
                    ...
                    if (pos > 0) fseek(hf, pos + 1, SEEK_SET);
                    else fseek(hf, 0, SEEK_SET);
                    
                    int key;
                    if (fscanf(hf, "%d", &key) == 1) {
                        regs[i.rd] = key;
                        /* Consume it by truncating the file */
                        if (pos > 0) truncate(path, pos);
                        else truncate(path, 0);
                    }
                } else regs[i.rd] = 0; // No input
                fclose(hf);
            } else regs[i.rd] = 0;
            ...
            char *proj_root = getenv("PRISC_PROJECT_ROOT");
            char path[512];
            if (proj_root) snprintf(path, sizeof(path), "%s/pieces/display/frame_changed.txt", proj_root);
            else strcpy(path, "pieces/display/frame_changed.txt");
            FILE *f = fopen(path, "a");
            if (f) { fprintf(f, "X\n"); fclose(f); }
            ...
            char path[512];
            if (proj_root && proj_id) snprintf(path, sizeof(path), "%s/projects/%s/pieces/%s/state.txt", proj_root, proj_id, i.literal_arg);
            else snprintf(path, sizeof(path), "projects/template/pieces/%s/state.txt", i.literal_arg);
            FILE *f = fopen(path, "r");
            if (f) {
                char line[256];
                int found = 0;
