...
int main() {
    ...
    while (1) {
        ...
        /* For op-ed, always read from player_app/history.txt (where parser injects) */
                    ...
                    while (fscanf(hf, "%d", &key) == 1) {
                        ...
                        if (gui_focus_index == 1) {
                            ...
                            if (key == 'w' || key == 'W' || key == 1002) {
                                char cmd[MAX_CMD];
                                snprintf(cmd, sizeof(cmd), "%s/pieces/apps/playrm/ops/+x/move_selector.+x up op-ed > /dev/null 2>&1", project_root);
                                system(cmd);
                            }
                            else if (key == 's' || key == 'S' || key == 1003) {
                                char cmd[MAX_CMD];
                                snprintf(cmd, sizeof(cmd), "%s/pieces/apps/playrm/ops/+x/move_selector.+x down op-ed > /dev/null 2>&1", project_root);
                                system(cmd);
                            }
                            else if (key == 'a' || key == 'A' || key == 1000) {
                                char cmd[MAX_CMD];
                                snprintf(cmd, sizeof(cmd), "%s/pieces/apps/playrm/ops/+x/move_selector.+x left op-ed > /dev/null 2>&1", project_root);
                                system(cmd);
                            }
                            else if (key == 'd' || key == 'D' || key == 1001) {
                                char cmd[MAX_CMD];
                                snprintf(cmd, sizeof(cmd), "%s/pieces/apps/playrm/ops/+x/move_selector.+x right op-ed > /dev/null 2>&1", project_root);
                                system(cmd);
                            }
