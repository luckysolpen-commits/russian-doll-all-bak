...
void hit_markers() {
    ...
    if (asprintf(&path, "%s/pieces/apps/editor/view_changed.txt", project_root) != -1) {
        FILE *f = fopen(path, "a");
        if (f) { fprintf(f, "X\n"); fclose(f); }
        free(path);
    }
...
void write_editor_state() {
    ...
    if (asprintf(&path, "%s/pieces/apps/player_app/state.txt", project_root) != -1) {
        ...
        char stats[MAX_LINE];
        snprintf(stats, sizeof(stats), "[POS]: (%d,%d,%d) | [H-BEAT]: %d | [KEY]: %-10s", 
                 cursor_x, cursor_y, cursor_z, last_sync_idx, last_key_str);
...
void process_key(int key) {
    ...
    if (key == '9' || key == 27) {
        ...
        set_response("Returned to Selector");
    }
    ...
    if (key == 122) {
        char *cmd = NULL;
        if (asprintf(&cmd, "%s/pieces/apps/playrm/ops/+x/undo_action.+x > /dev/null 2>&1", project_root) != -1) {
            system(cmd); free(cmd);
    ...
    if (key == 127 || key == 8) {
        if (gui_focus_index == 1 && map_count > 0) {
            char *cmd = NULL;
            if (asprintf(&cmd, "%s/pieces/apps/editor/ops/+x/place_tile.+x %s %d %d . > /dev/null 2>&1",
                         project_root, project_maps[map_idx], cursor_x, cursor_y) != -1) {
                system(cmd); free(cmd);
    ...
    if (gui_focus_index == 1) {
        if (key == 'w' || key == 'W' || key == 1002 || key == 's' || key == 'S' || key == 1003 ||
            ...
            const char *dir = (key == 'w' || key == 'W' || key == 1002) ? "w" :
                              (key == 's' || key == 'S' || key == 1003) ? "s" :
                              (key == 'a' || key == 'A' || key == 1000) ? "a" : "d";
            char *cmd = NULL;
        ...
        if (strcmp(active_target_id, "selector") == 0 && (key == 10 || key == 13)) {
            char *pieces_dir_path = NULL;
            if (asprintf(&pieces_dir_path, "%s/projects/%s/pieces", project_root, current_project) != -1) {
                DIR *dir = opendir(pieces_dir_path);
                if (dir) {
                    struct dirent *entry;
                ...
                free(pieces_dir_path);
            }
        }

        // Sync local cursor vars
        char *s_path = NULL;
        ...
        if (strcmp(active_target_id, "selector") != 0) {
            ...
            if (px != -1 && py != -1) {
                char *cmd = NULL;
                if (asprintf(&cmd, "%s/pieces/master_ledger/plugins/+x/piece_manager.+x selector set-state pos_x %d > /dev/null 2>&1", project_root, px) != -1) { system(cmd); free(cmd); }
                if (asprintf(&cmd, "%s/pieces/master_ledger/plugins/+x/piece_manager.+x selector set-state pos_y %d > /dev/null 2>&1", project_root, py) != -1) { system(cmd); free(cmd); }
...
int main() {
    ...
    write_editor_state(); trigger_render(); hit_markers();
        ...
        if (stat(hist_path, &st) == 0) {
            if (st.st_size > last_pos) {
                ...
                if (hf) {
                    ...
                    if (processed) { trigger_render(); write_editor_state(); hit_markers(); }
