/*
 * editor_module.c - Minimal OPS Demo (Phase 2 FINAL: Synced Focus & Debug)
 */
...
int is_active_layout() {
    ...
    if (fgets(line, sizeof(line), f)) {
        fclose(f);
        int res = (strstr(line, "editor.chtpm") != NULL || strstr(line, "man-ops.chtpm") != NULL);
        free(path);
        return res;
    }
    fclose(f); free(path);
...
int main() {
    ...
    /* READ project_id from manager state (IMPORTANT!) */
    char path[MAX_CMD];
    ...
    /* Write back with confirmed project_id */
    snprintf(path, sizeof(path), "%s/pieces/apps/player_app/manager/state.txt", project_root);
    ...
    while (1) {
        ...
        /* For man-ops, always read from player_app/history.txt (where parser injects) */
                ...
                if (hf) {
                    ...
                    while (fscanf(hf, "%d", &key) == 1) {
                        ...
                        if (gui_focus_index == 1) {
                            /* Use SHARED move_selector op (legacy editor style) */
