    ...
    if (kvp) {
        ...
        while (fgets(line, sizeof(line), kvp)) {
            if (strncmp(line, "project_root=", 13) == 0) {
                ...
                v[strcspn(v, "\n\r")] = 0;
                if (strlen(v) > 0) {
                    strncpy(project_root_path, v, sizeof(project_root_path) - 1);
...
void load_dynamic_methods(const char* active_id) {
    ...
    // 1. Project-specific pieces
    asprintf(&path, "%s/projects/%s/pieces/%s/%s.pdl", project_root_path, get_var("project_id"), active_id, active_id);
    if (access(path, F_OK) != 0) {
        free(path);
        // 2. App-specific pieces (like loader)
...
void load_vars() {
    ...
    if (strstr(current_layout, "playrm") != NULL || strstr(current_layout, "player") != NULL || 
        strstr(current_layout, "editor") != NULL || strstr(current_layout, "man-add") != NULL || 
        strstr(current_layout, "man-ops") != NULL || strstr(current_layout, "man-pal") != NULL ||
        strstr(current_layout, "op-ed") != NULL) {
    ...
    if (strcmp(proj_id, "fuzzpet_v2") == 0) {
        set_var("module_path", "projects/fuzzpet_v2/manager/+x/fuzzpet_v2_module.+x");
        set_var("active_layout_id", "fuzzpet_v2.chtpm");
    } else if (strcmp(proj_id, "op-ed") == 0) {
        ...
        set_var("module_path", "pieces/apps/man-pal/plugins/+x/man-pal_module.+x");
    ...
    } else if (strcmp(proj_id, "fuzz_legacy") == 0) {
        set_var("module_path", "projects/fuzz_legacy/manager/+x/fuzz_legacy_manager.+x");
        set_var("active_layout_id", "fuzz_legacy.chtpm");
    } else {
...
void inject_raw_key(int code) {
    ...
    if (strlen(project_id) > 0 && strcmp(project_id, "template") != 0) {
        char *path = NULL;
        if (asprintf(&path, "%s/projects/%s/history.txt", project_root_path, project_id) == -1) return;
        fp = fopen(path, "a");
        if (fp) { fprintf(fp, "%d\n", code); fclose(fp); }
        free(path);
        return;
    ...
    if (strstr(current_layout, "editor") != NULL || strstr(current_layout, "playrm") != NULL || strstr(current_layout, "player") != NULL) {
...
void launch_module(const char* launch_str) { 
    ...
    if (current_module_pid == 0) { 
        ...
        if (strstr(args[0], "projects/") != NULL) {
            // Include project ID in name for uniqueness: projects/fuzzpet_v2/manager/... -> fuzzpet_v2_module
...
void send_command(const char* cmd) {
    if (strncmp(cmd, "LOAD_PROJECT:", 13) == 0) {
        ...
        if (f) {
            ...
            if (strcmp(proj_name, "fuzzpet_v2") == 0 || strcmp(proj_name, "man-ops") == 0 || strcmp(proj_name, "op-ed") == 0) {
        ...
        // Load project-specific layout
        if (strcmp(proj_name, "fuzzpet_v2") == 0) {
            strncpy(current_layout, "pieces/chtpm/layouts/fuzzpet_v2.chtpm", MAX_PATH-1);
        } else if (strcmp(proj_name, "op-ed") == 0) {
            strncpy(current_layout, "pieces/apps/op-ed/layouts/op-ed.chtpm", MAX_PATH-1);
        } else if (strcmp(proj_name, "man-ops") == 0) {
            strncpy(current_layout, "pieces/apps/man-ops/layouts/man-ops.chtpm", MAX_PATH-1);
        } else if (strcmp(proj_name, "man-pal") == 0) {
            strncpy(current_layout, "pieces/apps/man-pal/layouts/man-pal.chtpm", MAX_PATH-1);
        } else if (strcmp(proj_name, "man-add") == 0) {
            strncpy(current_layout, "pieces/apps/man-add/layouts/man-add.chtpm", MAX_PATH-1);
        } else if (strcmp(proj_name, "fuzz_legacy") == 0) {
            strncpy(current_layout, "pieces/apps/playrm/layouts/game.chtpm", MAX_PATH-1);
        } else {
            strncpy(current_layout, "pieces/apps/playrm/layouts/game.chtpm", MAX_PATH-1);
        }
        parse_chtm();
 initialize_focus(); return;
    }
