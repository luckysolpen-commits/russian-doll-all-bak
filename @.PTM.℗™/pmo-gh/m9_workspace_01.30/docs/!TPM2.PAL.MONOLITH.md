...
3. **RELAY:** If in **INTERACT** mode, Parser "injects" the key into the App's local `history.txt`.
4. **TICK:** The App Module (e.g., `playrm_module.c`) polls its local history.
5. **TRAIT/OP:** The Module calls a "Muscle" Op (e.g., `move_player.+x`) to process logic.
6. **SOVEREIGNTY:** The Op updates the Piece DNA (`.pdl`).
7. **MIRROR SYNC:** The update is flushed to the Piece Mirror (`state.txt`).
