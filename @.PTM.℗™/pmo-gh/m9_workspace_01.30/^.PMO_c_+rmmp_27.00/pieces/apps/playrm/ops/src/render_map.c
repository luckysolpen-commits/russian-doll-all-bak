...
int main() {
    ...
    /* 1. Load terrain */
    char *map_path = NULL;
    if (asprintf(&map_path, "%s/projects/%s/maps/map_0001_z%d.txt", project_root, current_project, current_z) != -1) {
    ...
    /* 3. Compose View */
    char *view_path = NULL;
    char *tmp_view_path = NULL;
    if (asprintf(&view_path, "%s/pieces/apps/player_app/view.txt", project_root) != -1) {
        asprintf(&tmp_view_path, "%s.tmp", view_path);
        FILE *vp = fopen(tmp_view_path, "w");
        if (vp) {
            for (int y = 0; y < MAP_ROWS; y++) {
                fprintf(vp, "║  ");
            ...
            fclose(vp);
            rename(tmp_view_path, view_path);
        }
        free(view_path);
        free(tmp_view_path);
    }
