...
// TPM Orchestrator (v2.2 - STABILITY RESTORED)
// Responsibility: Launch core components, register PIDs, and WAIT.
...
void proc_mgr_call(const char* action, const char* name, int pid) {
    ...
    if (project_root[0] == '/') {
        ...
        snprintf(pm_path, sizeof(pm_path), "./pieces/os/plugins/+x/proc_manager.+x");
    }
    ...
    if (p == 0) {
        if (pid >= 0) {
            ...
            execl(pm_path, pm_path, action, name, NULL);
        }
        exit(1);
    } else if (p > 0) {
        waitpid(p, NULL, 0);
...
void launch_and_register(const char* name, const char* path, const char* args, bool quiet) {
    ...
    pid_t pid = fork();
    if (pid == 0) {
        // Child Process: Change to project root for consistent relative paths
        if (project_root[0] != '\0') {
            chdir(project_root);
        }
        ...
        if (args && strlen(args) > 0) {
            execl(abs_path, abs_path, args, NULL);
        } else {
            execl(abs_path, abs_path, NULL);
        }
        exit(1);
    } else if (pid > 0) {
        // Parent Process: Register PID
        proc_mgr_call("register", name, pid);
        
        // CRITICAL: Block here to keep the thread alive, matching system() behavior.
        // This prevents the orchestrator from exiting prematurely.
        int status;
...
void* keyboard_thread_func(void* arg) {
    launch_and_register("keyboard_input", "pieces/keyboard/plugins/+x/keyboard_input.+x", NULL, true);
    return NULL;
}

void* joystick_thread_func(void* arg) {
    launch_and_register("joystick_input", "pieces/joystick/plugins/+x/joystick_input.+x", ".", true);
    return NULL;
}

void* response_thread_func(void* arg) {
    launch_and_register("response_handler", "pieces/master_ledger/plugins/+x/response_handler.+x", NULL, true);
    return NULL;
}
...
void* gl_render_thread_func(void* arg) {
    launch_and_register("gl_renderer", "pieces/display/plugins/+x/gl_renderer.+x", NULL, true);
    return NULL;
}

void* clock_daemon_thread_func(void* arg) {
    launch_and_register("clock_daemon", "pieces/system/clock_daemon/plugins/+x/clock_daemon.+x", NULL, true);
    return NULL;
}

void* chtpm_thread_func(void* arg) {
    launch_and_register("chtpm_parser", "pieces/chtpm/plugins/+x/chtpm_parser.+x", "pieces/chtpm/layouts/os.chtpm", true);
    return NULL;
}
