...
void load_vars() {
    ...
    // Load clock daemon variables
    snprintf(path, sizeof(path), "%s/pieces/system/clock_daemon/state.txt", project_root);
        ...
        while (fgets(line, sizeof(line), cf)) {
            ...
            if (eq) {
                *eq = '\0';
                char *k = line;
                char *v = eq + 1;
                if (strcmp(k, "turn") == 0) set_var("clock_turn", v);
                else if (strcmp(k, "time") == 0) set_var("clock_time", v);
                else if (strcmp(k, "mode") == 0) set_var("clock_mode", v);
            }
        }
        fclose(cf);
    } else {
        set_var("clock_turn", "0");
        set_var("clock_time", "08:00:00");
        set_var("clock_mode", "auto");
    }
