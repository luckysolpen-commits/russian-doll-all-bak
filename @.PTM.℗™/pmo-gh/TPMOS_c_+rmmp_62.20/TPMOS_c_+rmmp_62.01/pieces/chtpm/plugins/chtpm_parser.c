...
void load_vars() {
    ...
    if (strcmp(proj_id, "op-ed") == 0) {
        set_var("module_path", "projects/op-ed/manager/+x/op-ed_manager.+x");
    ...
    if (strcmp(el->type, "br") == 0) strcat(frame, "\n");
        ...
        if (is_active) {
            if (is_password) {
                ...
                asprintf(&line, "%s [^] %d. [%s%s_]                                              %s", BOX_V, *p_current_interactive, scratch_substituted, el->input_buffer, BOX_V);
            }
        } else if (is_focused) {
...
void process_key(int key) {
    ...
    if (active_index == -1) {
        if (key == ARROW_UP || key == 'w' || key == 'W' || key == JOY_UP) { 
            ...
            if (digit_accum == 0) {
                // First digit: always execute 1-9 immediately
                // Only wait if user might be typing a multi-digit number (second digit arrives in same poll cycle)
                if (d >= 1 && d <= 9) {
                    // Check if a second digit already accumulated (rare, only if rapid typing)
                    if (d * 10 <= max_projects) {
                        // Could be start of 2-digit number — accumulate but still try jump
                        digit_accum = d;
                        // Try jump anyway (single digit)
                        if (do_jump(d)) {
                            nav_buffer[0] = (char)key;
                            nav_buffer[1] = 0;
                            clear_nav_on_next = true;
                            export_active_index();
                            digit_accum = 0;  // Reset since we jumped
                        }
                    } else {
                        // d*10 > max_projects, can't be start of 2-digit — jump immediately
                        if (do_jump(d)) {
                            nav_buffer[0] = (char)key;
                            nav_buffer[1] = 0;
                            clear_nav_on_next = true;
                ...
                // d == 0: do nothing (zero is not a valid nav target)
            } else {
    ...
    /* NAV MARKER: For nav-only layouts (project loader, main menu), write the
     * marker so compose_frame() fires through the same single-trigger path as games.
