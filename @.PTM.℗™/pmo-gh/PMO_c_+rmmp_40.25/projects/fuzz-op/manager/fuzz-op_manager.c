...
static void ensure_selector_state_defaults(void) {
    char sel_state[4096];
    ...
    if (!has_name && lc < 128) snprintf(lines[lc++], sizeof(lines[0]), "name=Selector\n");
    if (!has_type && lc < 128) snprintf(lines[lc++], sizeof(lines[0]), "type=selector\n");
...
int main() {
    resolve_paths();
    ensure_selector_state_defaults();
