...
class DesktopWidget(QWidget):
    ...
    def add_widget_to_desktop(self, widget):
        """Add a widget directly to the desktop layout (embedding it)"""
        # Remove the background label if it's the only thing in the layout
        if self.layout.count() == 1 and isinstance(self.layout.itemAt(0).widget(), QLabel):
            self.layout.takeAt(0)  # Remove the background label
        
        # Add the widget to the desktop
        self.layout.addWidget(widget)
        
        # Add to windows list for tracking purposes
        if hasattr(widget, 'show') and callable(getattr(widget, 'show')):
            widget.show()
    ...
    def show_pyboard2d(self):
        ...
        # Position the window within the desktop
        pyboard2d_window.setParent(self)
        pyboard2d_window.show()
        
        # Add to windows list
        self.add_window(pyboard2d_window)
        
    def show_pyboard3d(self):
        """Show the 3D PyBoard app"""
        ...
        # Position the window within the desktop
        pyboard3d_window.setParent(self)
        pyboard3d_window.show()
        
        # Add to windows list
        self.add_window(pyboard3d_window)
