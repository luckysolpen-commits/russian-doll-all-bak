#!/usr/bin/env python3
"""
Convert Centroid Transition Session 5 (Physical Chemistry & Flexboxes) to multi-voice audio.
"""
...
INPUT_FILE = os.path.join(OUTPUT_DIR, "session-5-physical-chemistry-and-flexboxes.txt")
OUTPUT_MP3 = os.path.join(OUTPUT_DIR, "session-5-physical-chemistry-and-flexboxes.mp3")
...
async def main():
    ...
    for i, (speaker, seg_text) in enumerate(segments):
        ...
        seg_path = os.path.join(OUTPUT_DIR, f"centroid_s5_seg_{i:03d}.mp3")
        print(f"Generating segment {i+1}/{len(segments)}: {speaker} ({voice})")
        await generate_speech(seg_text, voice, seg_path)
        segment_files.append(seg_path)
    
    if segment_files:
        list_file = os.path.join(OUTPUT_DIR, "centroid_s5_filelist.txt")
