#!/usr/bin/env python3
"""
Convert Centroid Transition Session 10 (The SSOL Time Mission — The Stable Loop) to multi-voice audio.
"""
...
INPUT_FILE = os.path.join(OUTPUT_DIR, "session-10-the-ssol-time-mission.txt")
OUTPUT_MP3 = os.path.join(OUTPUT_DIR, "session-10-the-ssol-time-mission.mp3")

VOICE_MAP = {
    "MOTHER TMOJIZU": "zh-CN-XiaoxiaoNeural",
    "CHARA": "fr-FR-VivienneMultilingualNeural",
    "CHARA (PAST)": "fr-FR-VivienneMultilingualNeural",
    ...
    "LUCKY (PAST)": "en-US-GuyNeural",
    "ASTRA": "en-US-JennyNeural",
    "ASTRA (PAST)": "en-US-JennyNeural",
...
async def main():
    ...
    for i, (speaker, seg_text) in enumerate(segments):
        ...
        seg_path = os.path.join(OUTPUT_DIR, f"centroid_s10_seg_{i:03d}.mp3")
        print(f"Generating segment {i+1}/{len(segments)}: {speaker} ({voice})")
        await generate_speech(seg_text, voice, seg_path)
        segment_files.append(seg_path)
    
    if segment_files:
        list_file = os.path.join(OUTPUT_DIR, "centroid_s10_filelist.txt")
