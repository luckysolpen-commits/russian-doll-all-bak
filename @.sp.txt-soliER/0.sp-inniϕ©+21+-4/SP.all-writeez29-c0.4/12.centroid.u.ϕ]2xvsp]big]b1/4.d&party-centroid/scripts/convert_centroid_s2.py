#!/usr/bin/env python3
"""
Convert Centroid Transition Session 2 (The Centroid Tour) to multi-voice audio.
Voice mapping:
...
- Horli Qwin: fr-FR-DeniseNeural
- Professor Pylorus: en-GB-RyanNeural
...
OUTPUT_DIR = "/home/no/Desktop/Piecemark-IT/中ϕ_00.00/0.sp-inniϕ©+16/SP.all-writeez28-b2/12.centroid.u.ϕ]2xvsp]big]b1/4.d&party-centroid"
INPUT_FILE = os.path.join(OUTPUT_DIR, "session-2-centroid-tour.txt")
OUTPUT_MP3 = os.path.join(OUTPUT_DIR, "session-2-centroid-tour.mp3")
...
async def main():
    ...
    for i, (speaker, seg_text) in enumerate(segments):
        ...
        seg_path = os.path.join(OUTPUT_DIR, f"centroid_s2_seg_{i:03d}.mp3")
        print(f"Generating segment {i+1}/{len(segments)}: {speaker} ({voice})")
        await generate_speech(seg_text, voice, seg_path)
        segment_files.append(seg_path)
    
    if segment_files:
        list_file = os.path.join(OUTPUT_DIR, "centroid_s2_filelist.txt")
