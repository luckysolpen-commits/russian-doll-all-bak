#!/usr/bin/env python3
"""
Batch convert CHAR.A.NON Book 111 chapters to audio using edge-tts.
Chapters 1-12 have page_N.txt files (4 pages each).
Chapter 13 uses chapter-13-final-charanonbook.txt (special final chapter).
...
BOOK_DIR = "SP.all-writeez28-a1/-1.MyChara+21]b1/CHAR.A.NON]c3]++/-1.CHAR.A.NON]1111111111111111/#.book_template.scp]111"
VOICE = "en-US-AriaNeural"
OUTPUT_PREFIX = "ca111.chapter"
OUTPUT_SUFFIX = "-v1.0.mp3"

async def convert_chapter(ch_dir, output_path, ch_num, total):
    # Chapter 13 uses special final file
    if ch_num == 13:
        txt_file = os.path.join(ch_dir, "chapter-13-final-charanonbook.txt")
    ...
    print(f"[{ch_num:>2}/{total}] Converting chapter_{ch_num} ({page_count} pages, {len(text)} chars)...")
