#!/usr/bin/env python3
"""
Batch convert Tear-It Accidental Witch chapters to audio using edge-tts.
Each chapter has a single chapter_N.txt file.
Skips chapters that already have output files.
Uses en-US-AnaNeural (IQA voice — female, cute).
"""
...
BOOK_DIR = "SP.all-writeez28-a1/111.tear-it]accidental.witch]y3/book_template.scp"
VOICE = "en-US-AnaNeural"
OUTPUT_PREFIX = "tearit.chapter"
OUTPUT_SUFFIX = "-v1.0.mp3"

async def convert_chapter(ch_dir, output_path, ch_num, total):
    txt_file = os.path.join(ch_dir, f"chapter_{ch_num}.txt")
