...
# Output settings
OUTPUT_PREFIX = "ax-witch"
OUTPUT_SUFFIX = "-axwitch-v1.0.mp3"
...
async def convert_chapter(ch_dir: str, output_path: str, ch_num: int, total: int):
    ...
    # Find and sort page/chapter files
    page_files = sorted(
        [f for f in os.listdir(ch_dir) if (f.startswith("page_") or f.startswith("chapter_")) and f.endswith(".txt")],
...
async def main():
    ...
    print("\n" + "=" * 70)
    print("BATCH CONVERSION COMPLETE - ax-witch BOOK")
