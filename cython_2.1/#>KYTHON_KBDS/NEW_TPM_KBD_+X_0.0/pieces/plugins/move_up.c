...
int main() {
    ...
    sprintf(cmd, "./pieces/plugins/+x/piece_manager.+x mover set-state last_direction up");
    system(cmd);
    
    // Log to master ledger
    sprintf(cmd, "./pieces/plugins/+x/log_event.+x MovementEvent \"up | From: (%d,%d) To: (%d,%d) | Target: mover\"", pos_x, pos_y, pos_x, new_y);
    system(cmd);
    
    printf("Moved up from (%d,%d) to (%d,%d)\n", pos_x, pos_y, pos_x, new_y);
