...
// Function to parse piece file
int parsePieceFile(const char *filepath, PieceData *data) {
    FILE *file = fopen(filepath, "r");
    if (!file) {
        fprintf(stderr, "Error: Could not open piece file %s\n", filepath);
        return 0;
    }
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        // Skip empty lines and comments (we won't handle comments in this version)
        ...
        if (isSection(trim(line))) {
            strcpy(current_section, getSectionName(trim(line)));
        }
        // Handle content under sections
        else if (strchr(line, ':') != NULL && strcmp(current_section, "responses") == 0) {
            ...
            if (colon_pos != NULL) {
                *colon_pos = '\0';
                ...
                strcpy(data->responses[data->response_count][0], key);
                strcpy(data->responses[data->response_count][1], value);
                data->response_count++;
            }
        }
        // Handle non-section content (state values)
        else {
            char *space_pos = strchr(line, ' ');
            if (space_pos != NULL) {
                *space_pos = '\0';
                char *key = trim(line);
                char *value = trim(space_pos + 1);
                
                if (strcmp(current_section, "responses") == 0) {
                    strcpy(data->responses[data->response_count][0], key);
                    strcpy(data->responses[data->response_count][1], value);
                    data->response_count++;
                } else if (strcmp(key, "event_listeners") == 0) {
                    char items[10][100];
                    int count = splitCommaSeparated(value, items, 10);
                    for (int i = 0; i < count; i++) {
                        strcpy(data->event_listeners[data->listener_count++], items[i]);
                    }
                } else if (strcmp(key, "methods") == 0) {
                    char items[10][100];
                    int count = splitCommaSeparated(value, items, 10);
                    for (int i = 0; i < count; i++) {
                        strcpy(data->methods[data->method_count++], items[i]);
                    }
                } else {
                ...
                strcpy(data->state_keys[data->state_count], line);
                strcpy(data->state_values[data->state_count], "true");
                data->state_count++;
            }
        }
    }
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        ...
        fprintf(stderr, "Example: %s keyboard get-state session_id\n", argv[0]);
        return 1;
    }
    
    const char *piece_id = argv[1];
    char piece_file[500];
    snprintf(piece_file, sizeof(piece_file), "pieces/%s/piece.txt", piece_id);
    ...
    if (argc < 3) {
        ...
        if (strcmp(action, "get-state") == 0 && argc >= 4) {
            ...
            if (setStateValue(&data, key, value)) {
                // Also write changes back to file
                ...
                if (file) {
                    ...
                    if (data.method_count > 0) {
                        ...
                        for (int i = 0; i < data.method_count; i++) {
                            if (i > 0) fprintf(file, ", ");
                            fprintf(file, "%s", data.methods[i]);
                        }
                        fprintf(file, "\n");
                    }
                    
                    if (data.listener_count > 0) {
                        fprintf(file, "event_listeners ");
                        for (int i = 0; i < data.listener_count; i++) {
                            if (i > 0) fprintf(file, ", ");
                            fprintf(file, "%s", data.event_listeners[i]);
                        }
                        fprintf(file, "\n");
                    }
