...
# Master Ledger Monitor - constantly monitors the master ledger for new entries
class MasterLedgerMonitor:
    ...
    def start_monitoring(self):
        """Start monitoring the master ledger in a separate thread"""
    ...
    def _monitor_loop(self):
        """Main monitoring loop - polls the master ledger for new entries"""
        while self.monitoring:
            self.process_new_ledger_entries()
    ...
    def _handle_method_call(self, line):
        ...
        print(f"[LEDGER MONITOR] Processing method call: {line}")
        ...
        try:
            ...
            print(f"[LEDGER MONITOR] Error processing method call: {e}")
    
    def _handle_event_fire(self, line):
        """Handle event fire entries in the master ledger"""
        print(f"[LEDGER MONITOR] Processing event fire: {line}")
        ...
        try:
            ...
            print(f"[LEDGER MONITOR] Error processing event fire: {e}")
    
    def _handle_state_change(self, line):
        """Handle state change entries in the master ledger"""
        print(f"[LEDGER MONITOR] Processing state change: {line}")
...
ledger_monitor = MasterLedgerMonitor()
...
# Start the master ledger monitor
ledger_monitor.start_monitoring()

print("\n=== TPM Virtual Pet – FuzzPet ===")
print("Commands: feed, play, sleep, status, evolve, quit")
print("Shortcuts: 1=feed, 2=play, 3=sleep, 4=status, 5=evolve, 6=quit")
...
while True:
    ...
    if cmd == 'quit':
        print("Bye bye~ FuzzPet waves goodbye!")
        break
    if not cmd:
        continue

    # Log the user command to the master ledger as a MethodCall (this will be processed by ledger monitor)
    timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
    log_entry = f"[{timestamp}] MethodCall: {cmd} on fuzzpet | Caller: user_input\n"
    with open('pieces/master_ledger/master_ledger.txt', 'a') as ledger:
        ledger.write(log_entry)

    # Fire command_received event (this can happen immediately)
