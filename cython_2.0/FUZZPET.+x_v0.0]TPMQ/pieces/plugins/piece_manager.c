...
int parsePieceFile(const char *filepath, PieceData *data) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        // Handle top-level methods and event_listeners
        else if (strncmp(line, "methods:", 8) == 0) {
            char *values_part = line + 8;
            char items[10][100];
            int count = splitCommaSeparated(values_part, items, 10);
            for (int i = 0; i < count; i++) {
                strcpy(data->methods[data->method_count++], items[i]);
            }
        }
        else if (strncmp(line, "event_listeners:", 16) == 0) {
            char *values_part = line + 16;
            char items[10][100];
            int count = splitCommaSeparated(values_part, items, 10);
            for (int i = 0; i < count; i++) {
