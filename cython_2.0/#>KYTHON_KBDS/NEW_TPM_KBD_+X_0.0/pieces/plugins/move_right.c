...
int main() {
    ...
    sprintf(cmd, "./pieces/plugins/+x/piece_manager.+x mover set-state last_direction right");
    system(cmd);
    
    // Log to master ledger
    sprintf(cmd, "./pieces/plugins/+x/log_event.+x MovementEvent \"right | From: (%d,%d) To: (%d,%d) | Target: mover\"", pos_x, pos_y, new_x, pos_y);
    system(cmd);
    
    printf("Moved right from (%d,%d) to (%d,%d)\n", pos_x, pos_y, new_x, pos_y);
