...
/* Dynamic Project Mapping (A29 Decoupling) */
typedef struct {
    char id[64];
    char label[128];
} GLProject;

GLProject gl_projects[20];
...
void load_gl_projects(void) {
    char *kvp_path = NULL;
    if (asprintf(&kvp_path, "%s/pieces/apps/gl_os/gl_os_projects.kvp", project_root) == -1) return;
    
    FILE *f = fopen(kvp_path, "r");
    if (!f) {
        /* Fallback if KVP missing */
        gl_project_count = 0;
        free(kvp_path);
        return;
    }
    ...
    gl_project_count = 0;
    char line[MAX_LINE];
    while (fgets(line, sizeof(line), f) && gl_project_count < 20) {
        if (line[0] == '#' || line[0] == '\n') continue;
        char *eq = strchr(line, '=');
        if (eq) {
            *eq = '\0';
            strncpy(gl_projects[gl_project_count].id, trim_str(line), 63);
            strncpy(gl_projects[gl_project_count].label, trim_str(eq + 1), 127);
            gl_project_count++;
        }
    }
    fclose(f);
    free(kvp_path);
    printf("[GL-OS] Loaded %d projects from KVP decoupling.\n", gl_project_count);
}
...
void execute_option(DesktopWindow* win, int index) {
    ...
    /* Standard Terminal Contexts */
        ...
        if (index == 0) {
            /* Switch to Desktop Apps Context (A29 Decoupling) */
            win->menu_context = CTX_DESKTOP_APPS;
            win->selected_index = 0;
            
            int i = 0;
            for (i = 0; i < gl_project_count && i < MAX_TERM_OPTIONS - 3; i++) {
                strncpy(win->menu_options[i], gl_projects[i].label, MAX_OPTION_LEN - 1);
            }
            
            /* Append fixed options */
            strncpy(win->menu_options[i++], "GLTPM Apps ->", MAX_OPTION_LEN - 1);
            strncpy(win->menu_options[i++], "3D Cube Demo", MAX_OPTION_LEN - 1);
            strncpy(win->menu_options[i++], "back", MAX_OPTION_LEN - 1);
            ...
            strcpy(win->menu_options[3], "back");
        ...
        if (index < gl_project_count) {
            char launch_msg[256];
            snprintf(launch_msg, sizeof(launch_msg), "Launching %s...", gl_projects[index].id);
            add_history(win, launch_msg);
            create_mirrored_window(gl_projects[index].id, gl_projects[index].label);
            ...
            create_cube_window();
        } else if (index == gl_project_count + 2) {
            /* Back to Main Menu */
            win->menu_context = CTX_MAIN;
            win->selected_index = 0;
            win->option_count = 3;
...
void draw_window(DesktopWindow* win) {
    ...
    if (strcmp(win->type, "terminal") == 0) {
        ...
        /* Draw Xelector (Artifact-XEL) - Sovereign Position */
        ...
        /* A29 FIX: Force 2D Top-Down for aow-2d */
        if (strcmp(win->project_id, "aow-2d") == 0) {
            win->cam_rot[0] = -90.0f; /* Pure Top-Down */
            win->cam_rot[1] = 0.0f;
            win->camera_mode = 4;     /* Force Free Camera context */
            glTranslatef(0, 0, -18.0f); /* Zoom out slightly for grid view */
            glRotatef(win->cam_rot[0], 1, 0, 0); /* Pitch DOWN */
        } else {
            if (win->camera_mode == 1) { /* 1st Person - Close/Low */
                glTranslatef(0, -0.5f, -1.0f);
            } else if (win->camera_mode == 3) { /* 3rd Person - Behind/High */
